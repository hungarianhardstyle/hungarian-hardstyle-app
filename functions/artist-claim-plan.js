 ...
 *     független.
 *  3. **A ház saját címe (`info@hungarianhardstyle.hu`) soha nem claimelhet** —
 *     az a `booking_via_huhs` esetén kerül a nyilvános adatlapra, nem egy DJ
 *     személyes címe.
 *  4. **A döntés nem szivárogtat e-mail címet.** A felület csak annyit kap, hogy
 *     `claimed` / `mine` / `canClaim` — a címek a szerveren maradnak.
 */

/** A ház saját címe: ezzel SOHA nem lehet claimelni. */
 ...
 *
 * A ház saját címét kihagyjuk (nem személyes cím), az üreseket szintén — így nem
 * lehet „véletlenül" egyezni egy hiányzó mezőn.
 */
function claimEmailsFor(artist) {
  ...
  for (const value of [artist?.booking_email, artist?.contact_email]) {
    ...
    if (!email || email === HOUSE_EMAIL) continue;
...
function artistClaimState({ email, emailVerified, artist, claim, uid } = {}) {
  ...
  else if (normalized === HOUSE_EMAIL) reason = 'house-email';
  else {
