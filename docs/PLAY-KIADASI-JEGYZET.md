...
- **A mért gyökér:** a kiírás `'${_downloaded.length} letöltött zene'` volt, azaz **nyers, interpolált** szöveg. Ezt **egyetlen kapu sem látta**: az i18n-extraktor a `$`-t tartalmazó literált kihagyja, a raw-label audit regexe pedig eleve kizárja. Ezért **kapu is épült** (`test/services/i18n_untranslated_ui_test.dart`): a szonda **95** további ugyanilyen helyet talált (tárolt állapotüzenetek, tooltipek, űrlap-ellenőrzők, e-mail-űrlapok tárgya).
- **Ami épült:** minden érintett felirat **szótár-kulcs** lett (a szótár **1147 → 1300 kulcs**), a megjelenítés pedig fordít (`tr`/`trArgs`/`AppText`, illetve tárolt állapotnál `Text(tr(context, _message!))`). Angolul mostantól **DELETE** a törlés-megerősítés szava (eddig a felirat DELETE-et kért, de a kód a magyar `TÖRLÉS`-t várta — így angolul **nem is lehetett** törölni).
- **Mérve:** `flutter analyze lib test` → **No issues found!**; `flutter test` → **1154/1154** (ebből új: `test/services/i18n_untranslated_ui_test.dart` **3/3** — a forrás **nem tartalmaz** fordítatlan magyar feliratot, a megjelenítéskor fordított kulcsok bent vannak a szótárban, és a tulajdonos konkrét kulcsa angolul szól); **mutációs bizonyíték 5/5** (`tmp/mutation-proof-untranslated-ui.mjs`: a nyers „letöltött zene", egy nyers megjelenítési felirat, egy új tárolt üzenet, egy nyers gombfelirat és egy hiányzó szótár-kulcs is **elkapva**, bájtazonos visszaállítással); `node tmp/verify-aab.mjs build/HUHS-v1.0.0+374-release.aab 374` → **versionCode 374**, **1300 kulcsú szótár**, a „letöltött zene" kulcs **a csomagban is angolul**, mind a **huszonnégy** changelog-sor mindhárom ABI-ban.
- **⚠️ ŐSZINTE KORLÁT:** a **chat** és a felhasználók **saját szövegei** továbbra sem fordulnak (szándékos), és a `RegExp`-minták, illetve a **szerveroldali válaszra illesztő** rész-szövegek szándékosan magyarul maradnak (ezek nem feliratok; a kapu névre szóló kivétellistán tartja őket).

### 373 — a hír-kategóriák, a `&amp;` kódolási hiba, az angol feliratok és a pont-értesítés szóismétlése
- **A tulajdonos észrevétel-listája** (képernyőkép a jegyzeteiről): *„a híreknél a kategóriák is magyar"*, `&amp;` a DJ-leírásokban (Goze, Nu-Clear, Subrage), magyar feliratok angol módban (esemény ismerős-sora, éves név-/e-mail-módosítás, „Reklámmal feloldva", „Ismerősök: n", értesítés-kijelölés), és a magyar pont-értesítés szövege („ez a magyarnál javítandó").
- **A mért gyökér (mind mérve):** (1) a **kategória-sor** a kártyákon és a cikk-fejlécben **nyersen** ment ki (`post.articleCategories.join(' · ')`) — a hírek fül szűrő-chipjei viszont már fordítottak; éles szonda (`tmp/probe-news-categories.mjs`): a **11 élő kategóriából 10-nek** van fordítása, a `#TBT` eleve angol; (2) a DJ-/szervező-adatlap a **sima szöveget még egyszer escape-elte**, ezért a képernyőn `&amp;` látszott (éles mérés: a magyar leírások **2/17**, az angolok **3/17** tartalmaz entitást — a hír, az esemény és a GYÍK ezt már feloldotta); (3) az öt felirat **nyers** (interpolált vagy ternary-ág) volt, ezért angol módban magyarul jelent meg, és az i18n-extraktor **nem is látta**; (4) a magyar sablonban **szóismétlés** volt („Új össz**össz**pontszámod").
- **Ami épült:** közös `articleCategoriesLabel()` (`lib/core/i18n/content_labels.dart`, a négy kiíró helyen) és `decodeHtmlEntities()` (`lib/core/content/html_text.dart`, a DJ- és szervező-modellben); a feliratok **szótár-kulccsá** váltak (`Ismerőseid is jönnek: {names}`, `Közös esemény: te és {names} is ott lesztek.`, `Éves névmódosítási lehetőség: {n} maradt`, `Éves e-mail-módosítási lehetőség: {n} maradt`, `Reklámmal feloldva`, `Ismerősök: {n}`, `Jelölj ki értesítéseket`, `Kijelölve: {n}`, `1 értesítés törölve.`, `{n} értesítés törölve.`) — mind angol fordítással; a `notificationSelectionKey`/`notificationDeletedKey` **kulcsot** ad (nem kész szöveget), így a szótár kapuja is számon kéri; a szerveroldali pont-sablon javítva (**telepítve**).
- **Mérve:** `flutter analyze lib test` → **No issues found!**; `flutter test` → **1150/1150** (ebből új: `test/services/html_entities_and_labels_test.dart` **9/9** — entitás-feloldás a DJ- és szervező-leírásban, a HTML-tagek érintetlenül hagyása, a feliratok kulcsa + a szótárbeli fordítás, a pont-sablon szóismétlése, és a kategória-sor fordítása angol/magyar módban); `node tools/run-function-tests.mjs --pure` → **289/289**; **mutációs bizonyíték 7/7** (`tmp/mutation-proof-labels-entities.mjs` — az entitás-feloldás, a kulcs, a „Reklámmal feloldva", az „Ismerősök: n", a szóismétlés, a kategória-sor és a kategória-címke fordítása: mind **elkapva**, bájtazonos visszaállítással); `node tmp/verify-aab.mjs build/HUHS-v1.0.0+373-release.aab 373` → **versionCode 373**, **1147 kulcsú szótár**, mind a **húsz** changelog-sor mindhárom ABI-ban.
...
- **A mért gyökér:** a `tr(context, …)` a `Localizations` függőséget regisztrálja, ezért a **feliratok** azonnal átfordultak — a képernyő viszont a **betöltött példányt** (`_post` / `_event` / `_release`) tartotta, és a **tartalom** (cím, szöveg, leírás) csak újranyitáskor frissült. A DJ- és szervező-adatlap **már jó volt**: azok `ConsumerWidget`-ek, és a részlet-providerük figyeli a jelzést (`ref.listen(publicContentRefreshProvider, … invalidateSelf())`).
- **Ami épült:** új, újrahasznosítható minta (`lib/core/i18n/content_language_reload.dart`): a képernyő megjegyzi, **milyen nyelven** töltötte be a tartalmat, és a WordPress-jelzésre **nyelvváltást** észlelve **egyszer** újratölti (**ugyanazon** a meglévő `_loadFull…` úton, ezért nincs második kóri út). A régi tartalom a betöltés alatt **látható marad** (nincs üres villanás), hiba esetén pedig a régi marad. **Tartalom-frissülésre nem tölt újra** (az a jelzés a háttér-frissítésnél is felhúz), és a lebontott képernyő **leiratkozik** (nincs szivárgás). Bekötve: cikk-, esemény- és kiadvány-adatlap.
- **Mérve:** `flutter analyze lib test` → **No issues found!**; `flutter test` → **1141/1141** (ebből új: `test/services/content_language_reload_test.dart` **5/5** — nyelvváltáskor **egyszer** tölt, tartalom-frissülésre **nem**, lebontás után **nem**, és forrás-lint a három adatlap bekötésére); **mutációs bizonyíték 4/4** (`tmp/mutation-proof-content-language-reload.mjs` — a figyelő elvétele, a nyelv-összehasonlítás elvétele, a mixin levétele a cikk-adatlapról, az esemény-út kiürítése: mind **elkapva**, bájtazonos visszaállítással); `node tmp/verify-aab.mjs build/HUHS-v1.0.0+372-release.aab 372` → **versionCode 372**, **1133 kulcsú szótár**, mind a **tizenhat** changelog-sor mindhárom ABI-ban.
- **Amit ez a kiadás is tartalmaz:** a 371 (a Chat **@mindenki** push-ja, a küldő visszajelzése, a hírlista azonnali átállása), a 370 (az értesítésbe kerülő cikk címe is a választott nyelven), a 369, a 368, a 367, a 366, a 365, a 364 és a 363–359 minden újdonsága.
- **⚠️ ŐSZINTE KORLÁT:** a nyitott adatlap a nyelvváltás után **hálózatot** kér az új tartalomért; ha épp nincs kapcsolat, a **régi nyelvű** tartalom marad (nem törünk el semmit), és a következő jelzésre újrapróbáljuk. A **Chat** és a felhasználók saját szövegei továbbra sem fordulnak.

### 371 — a Chat **@mindenki** értesítéséhez push is jön + a küldő visszajelzése + a hírlista nyelvváltása
...
- **Mérve:** `flutter analyze lib test` → **No issues found!**; `flutter test` → **1135/1135** (ebből új: `test/services/chat_mention_wiring_test.dart` **2/2** a visszajelzésre és a push-útvonalra, `test/services/news_freshness_test.dart` **2/2** a nyelvváltásra, `functions/chat-mention-plan.test.cjs` **2/2** a tiszta döntésre); `node tools/run-function-tests.mjs --pure` → **289/289**; **mutációs bizonyíték 12/12** (`tmp/mutation-proof-everyone-push.mjs` — a push-hívás elvétele, a `created`/preferences/token kapuk, a nyelvi cím, a visszajelzés és a koppintás-útvonal elvétele, valamint a **cím-összehasonlítás** és a **nyelvváltás-ág** elvétele is **elkapva**, bájtazonos visszaállítással); `node tmp/verify-aab.mjs build/HUHS-v1.0.0+371-release.aab 371` → **versionCode 371**, **1132 kulcsú szótár**, mind a **tizenöt** changelog-sor mindhárom ABI-ban.
- **Javítva — a hírlista nyelvváltása (a tulajdonos jelzése: *„a legutolsó hír valamiért nincs fent angolul"* → *„újraindítás után jó"*):** a hírek fül háttér-frissítése **azonosítók** alapján döntött („ugyanaz a lista"), a nyelvváltás viszont **ugyanazokat** az azonosítókat adja **más nyelvű** címekkel — így a régi nyelvű lista a helyén maradt; ráadásul a háttér-út csak az első oldalon futott, ezért egy továbblapozott lista **újraindításig** a régi nyelven maradt. Mostantól az összehasonlítás a **címet is** nézi, és **nyelvváltáskor** a lista minden betöltött oldalát eldobjuk, és az első oldaltól töltünk újra.
