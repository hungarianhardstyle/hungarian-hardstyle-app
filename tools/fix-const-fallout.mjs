...
export function analyzeErrors(target = 'lib') {
  ...
  for (const line of output.split('\n')) {
    const match = /^\s*error\s+-\s+(.*?)\s+-\s+(.*?):(\d+):(\d+)\s+-\s+(\S+)\s*$/.exec(line);
    if (!match) continue;
    errors.push({
      message: match[1],
      file: match[2].replaceAll('\\', '/'),
      line: Number(match[3]),
...
function main() {
  ...
  for (let round = 1; round <= maxRounds; round += 1) {
    ...
    console.log(`  tervezett const-eltávolítás: ${planned}`);
    if (!planned) {
    ...
    for (const [file, offsets] of removals) {
      const source = fs.readFileSync(file, 'utf8');
      let updated = source;
      for (const at of [...offsets].sort((a, b) => b - a)) {
        // A `const` + az utána következő szóköz eltávolítása.
