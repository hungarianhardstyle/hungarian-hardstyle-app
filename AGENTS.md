...
- v1.0.0+2 keeps configured Label prices visible when Play product details are unavailable, reports Play purchase failures, and gives the rewarded-ad button a loading state and timeout/error feedback.
...
- Backend package 2.4.31, 2.4.33, 2.4.36 and 2.4.37 are historical; 2.4.42 is the last previously prepared voting package and 2.4.45 is the current locally prepared v1 package awaiting deployment.
...
- Current 1.0 live status (2026-08-12): all four Wellerman Play Billing products are active in Hungary with the requested 700/550 HUF base prices. WordPress release 12123 returns the four configured products. The `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` secret is enabled, the Play service account has active app-level financial-data access, `verifyLabelPurchase`/`getLabelDownloadUrl`/`admobRewardedSsv` are deployed, and production AdMob Banner/Rewarded units plus the SSV callback are configured. The production ARM64 APK/AAB are built; do not close v1.0 until the project owner completes the final phone verification of the production artifact.
