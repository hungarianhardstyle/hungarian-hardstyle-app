...
- **Amit a tulajdonosnak kell tennie:** **semmit az appban** (nem kell új AAB, a javítás a szerveren van). Egyedül a Cloudinary **API Secret**-et érdemes rendbe tenni a Cloudinary Console → Settings → API Keys alatt, hogy a törölt felhasználók **képei** is eltűnjenek — addig a képek a Cloudinaryn maradnak, de a fiók és a profil **már törlődik**.
- **Ellenőrzések:** `functions/rules.test.cjs` **18/18**, `functions/account-deletion.test.cjs` **5/5**, `functions/prize-draw.test.cjs` **24/24**, `functions/security-permissions.test.cjs` **22/22**, `node --check functions/index.js` tiszta.
