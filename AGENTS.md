...
### Az edge-to-edge kártya **MÉRVE**: Android 14-en a `enableEdgeToEdge()` nem változtat a felületen (2026-09-24, mérőváltozat; **a main NEM változott**)

- **A TULAJDONOS DÖNTÉSE (kérdező ablak):** *„Mérd meg előbb API 34 emulátoron, aztán döntünk"* → **ez a szakasz a mérés**, a döntés ezután jön.
...
- **A JAVASLAT KÓDJA (ha a tulajdonos igent mond):** `android/app/src/main/kotlin/hu/hungarianhardstyle/app/MainActivity.kt` — `import androidx.activity.enableEdgeToEdge`, majd
  `override fun onCreate(savedInstanceState: Bundle?) { enableEdgeToEdge(); super.onCreate(savedInstanceState) }` (**~5 sor**, a meglévő `AudioServiceFragmentActivity` bázis változatlan).
- **⚠️ ŐSZINTE KORLÁT:** a mérés **emulátoron** történt (x86_64, API **34**, Pixel 8 profil) — **valódi régi készüléken nem**, és **API 33 vagy alatta nem** (ahhoz külön rendszerkép kellene). Azt, hogy a Play **leveszi-e a kártyát** egy új csomagtól, csak a következő **feltöltés után** lehet látni. A mérőváltozat **nincs a repóban** (visszavontam), a **355-ös AAB változatlan** (`F8D2132D…`), a Dart-tesztekhez nem nyúltam. **A bizonyíték a repóban:** `tmp/edge34/base/*.png` (alap), `tmp/edge34/e2e/*.png` (mérőváltozat), `tmp/edge34/dump-base.txt` + `tmp/edge34/e2e/dumpsys.txt` (ablak-attribútumok), valamint a futtatható szkriptek (`flow-short.ps1`, `find-close.ps1`, `colscan.ps1`, `edge-compare.ps1`).
