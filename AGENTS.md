...
### A GYIK (Segítség) rendberakva — tárgyi hiba, duplikáció, zaj és hiányzó témák (2026-09-18, plugin **2.5.4**)
...
- **Új, önmagát bizonyító ellenőrzés: `tools/verify-faq-content.mjs`** (**37/37**):
  ...
  - **A detektor bizonyítottan működik — két hibamódot is kimértem:** a hibás „naponta legfeljebb **öt** alkalommal" szöveggel **elhasal** (a kódhoz kötött érték-ellenőrzés bukik), és a **gyenge nyugdíjazást** (csak az 5 slug) is elbuktatja; a valódi forráson **37/37** átmegy.
- **Csomag:** `build/huhs-mobile-api-2.5.4.zip` — 44 bejegyzés, 137,2 KB, SHA-256 `A0A892452334C325364A83957AC1DAD2B8291C798D6588883F3823B14944064D`.
- **ÉLŐ ÁLLAPOT (2026-09-18, mérve):** élesben a **2.5.3** van fent (`X-HUHS-Health: api=2.5.3`), és a migráció **már lefutott** — a `/faq?per_page=100` **52 kiadott** bejegyzést adott (a 31 új a helyes `menu_order`-ekkel 101/201/301/401/501/601/701…, plusz a **14 db `order=0`** és a **8** régi „App és közösség" maradék). A hibás „naponta legfeljebb öt" szöveg **már nincs benne**. A **2.5.4** ezt a 22 maradékot teszi vázlatba; a migráció akkor fut le, amikor a feltöltés után megnyitod a WordPress adminfelületet (admin_init + verziójelző). Utána a GYIK **7 témakörre bontva, 31 kérdéssel** jelenik meg az appban.
- **Ellenőrzések:** 43/43 PHP parse, `verify-faq-content.mjs` **37/37**, `check-wp-admin-menu.mjs` **8/8**, `check-wp-meta-json.mjs` zöld, `verify-prize-draw.mjs` **44/44**, `verify-poll-status.mjs` **25/25**.
