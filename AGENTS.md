...
- **⏭️ AMI MÉG HÁTRA VAN (a következő részek):** (2) a **claim-állapot** cache-first (a DJ-adatlap „ez az enyém / átvehető" sora és a claim-gomb azonnal a mentett válaszból, háttérben egyeztetve), (3) a profil „DJ-adatlap" szekciója (`getClaimedArtistsForUser`, ma `FutureBuilder`), (4) saját profil adatai, (5) a listák (hírek/közösségi — a `newsProvider` ma `keepAlive` nélkül), (6) kiadványok/„Megvásárolt zenéim", (7) előtöltés bejelentkezés után. **Ehhez kell a közös cache-réteg** (`AsyncCacheStore` + tiszta `cache_plan` modul), amit még meg kell írni.
...
- **Csomag:** `build/HUHS-v1.0.0+353-release.aab` (versionCode **353**, 81,25 MB, SHA-256 `77A86BFB2D5E747815933101C3573F07A1F63452AC1C9D403A82538A5EE01589`). A `docs/PLAY-KIADASI-JEGYZET.md` **frissítve** (meta: currentBuild 353 / lastPublishedBuild 352; AAB-tábla; 1. rövid blokk = a 353; **1b. blokk = 352–353 összesítő a nyilvános kiadáshoz**, mert a nyilvános felhasználók a 329–351 szöveget látták; 3. pont = 353 bejegyzés), és a `lib/data/app_changelog.dart` is megkapta a 353-at; `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN**.
