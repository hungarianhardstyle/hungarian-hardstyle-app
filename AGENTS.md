...
### AZ ACHIEVEMENT-SÚGÓ LEÍRÁSA + A „DJ-k" FELIRAT — a tulajdonos képernyőképe (2026-09-26)

- **A TULAJDONOS JELZÉSE (képernyőkép az iPhone-ról, „Achievement system" képernyő):** *„szted ez angol?"* — a **nevek angolul** voltak (`Hardstyle Face`, `Community Member`, …), a **leírások viszont magyarul** (`700 points • Láthatóan aktív HUHS-közösségi tag.`), plusz: *„a menüben a DJ-k az elég magyar, lehetne az Dj's vagy ilyesmi"*.
- **A MÉRÉS — két különböző gyökér, mindkettő mérve:**
  1. **A leírás a SABLON helyőrzőjében ment ki nyersen:** `achievement_guide_screen.dart` → `trArgs(context, '{n} pont • {d}', {'n': …, 'd': level.description})`. A **sablon** fordítva volt (ezért lett „700 points"), a **`{d}` értéke** viszont **fordítás nélkül** került a helyőrzőbe. ⚠️ **EZ A HIBAOSZTÁLY FONTOS:** a `trArgs` önmagában nem fordítja a paramétereket — ha a paraméter **szerverről/adatból jövő szöveg**, külön kell fordítani (`'d': tr(context, level.description)`).
  2. **A „DJ-k" felirat a More menüben nyersen állt:** `more_screen.dart` `_item(icon, 'DJ-k', …)` — ez volt az **egyetlen** menüpont, aminek a felirata **nem** `tr(...)`-ben volt (a szótárban már benne volt: `DJ-k → DJs`). Ugyanez a nyers alak a **DJ-lista fejlécében** (`artists_screen.dart`: `const Text('DJ-k')`) és a **műfaj-felfedező** szekció-címében (`genre_discovery_screen.dart`).
- **AMI ÉPÜLT:** (1) a leírás a helyőrzőben is fordítva (`'d': tr(context, level.description)`); (2) `tr(context, 'DJ-k')` a menüben, `const AppText('DJ-k')` a DJ-lista fejlécében, `tr(context, 'DJ-k')` a műfaj-felfedezőben. **Az angol alak `DJs`** (a `DJ's` az angolban **birtokos** lenne: „a DJ-é") — ezt a szótári érték adja, és a teszt őrzi.
- **ÚJ ESZKÖZ — `tools/audit-raw-labels.mjs` (triage):** megkeresi a UI-rétegben azokat a literálokat, amelyek **nem fordító hívásban** állnak (`tr`/`trArgs`/`AppStrings.tr*`/`AppText`). **Mért eredmény:** **82 találat** („a szótárban VAN fordítás, de a hely nyersen írja ki") + 16 `Text('…')` + 34 gyanús, és **58 „by design"** const térkép/lista érték (azokat a megjelenítés fordítja, pl. `AppText(option.$2)`). ⚠️ **A 82 FELSŐ KORLÁT, nem hibalista:** a naiv változatom **474 hamis pozitívot** adott (a `tr(context, 'X')` belsejét is nyersnek hitte — ezért a szkript a **körülvevő hívást** keresi meg zárójel-számlálással), a maradékban pedig **adatlisták** is vannak (pl. a szerepkör-opciók `const [...]` listája), amiket a megjelenítés fordít. **Ellenőrzött, valódi találat ebből a körből:** a műfaj-felfedező „DJ-k" szekciója.
- **BIZONYÍTÉK (mind mérve):** `flutter analyze lib test` → **No issues found!**; `flutter test` → **1072/1072**; az Achievement/menü teszt **10/10** (új: „a leírás a SABLON paramétereként is fordítva megy be", „a menü »DJ-k« felirata is a fordítón megy át"); **mutációs bizonyíték 3/3** (`tmp/mutation-proof-achievement-ux.mjs`, bájtpontos visszaállítással: a nyers `{d}`, a nyers menü-felirat és a nyers fejléc → mind **elkapva**).
- **AZ AAB ÚJRAÉPÜLT + AZ IPHONE FRISS (2026-09-26):** **`build/HUHS-v1.0.0+364-release.aab`** — **85 640 300 bájt**, SHA-256 **`AB788F453DD4BABA4000862CCE1714BE8BF958FC92FC351D643488758E56CDB1`** (versionCode 364, 966 kulcs, production AdMob ID, aláírás, changelog mindhárom ABI-ban). Az iPhone: CI **`36233789123`** → IPA **`build/ios-ipa-364d/Runner-unsigned.ipa`** (26 355 178 bájt, SHA-256 `3699A8BD0CD531E09BA9346F21830225B8245A00C11942577471B1F91085B4C6`, ellenőrizve **12/12**) → sideload **id 22** (`2026-09-26 11:59`, ugyanaz a bundle-utótag, `failures_count 0`).
- **⚠️ ŐSZINTE KORLÁT:** a **teljes angolságot nem állítom** — az audit szerint **maradnak** olyan helyek, ahol egy felirat nyersen megy ki (főleg **ternary-ágak** és `tooltip:`/`label:` mezők, pl. `tooltip: currentVisible ? 'Elrejtés' : AppStrings.tr('Megjelenítés')`). Ezek egy része **nem látszik** (adatlista, amit a megjelenítés fordít), más része **látszik** (pl. hosszú nyomásra a tooltip). A következő kör ezeket viheti át egy menetben; a méréshez az audit eszköz adja a listát.

### A JELVÉNY-NEVEK A TOPLISTÁBAN IS ANGOLUL + A KVÍZ KÉRDÉSEI IS FORDULNAK (2026-09-26, plugin **2.14.2** + app)

- **A TULAJDONOS JELZÉSEI (mind a három mérve, nem tippelve):**
  1. *„az achievement leírása még magyar az angol verzóiban"*;
  2. *„az achievement nevei magyarul vannak az angol verzióban a huhs legenda toplistában"*;
  3. *„ha felkerül Poll/Kérdőív, jó lenne ha a válaszok is lefordulnának angolul, vagy lenne lehetőség megadni angol választ is, kviz meg nyereményjáték dettó"*.
- **A MÉRÉS (mi volt tényleg magyar és hol):**
  - A **HUHS Legenda toplista** (`achievement_leaderboard_screen.dart`) a szerverről jövő `badgeName`-et **nyersen** írta ki (`Text(badge!)`), a **pont-egység pedig be volt égetve** (`Text('$points pont')`) → angol módban magyar jelvénynév **és** magyar „pont". A **közösségi lista** sora (`community_screen.dart`) ugyanígy nyersen (`'${achievement.badgeName} • '`).
  - A **leírás** a jelvény-kártyán már fordítva volt (`tr(context, …)`), de a **szerver-katalógus** (`includes/achievements.php`, 7 jelvény) szövegei közül **kettő hiányzott** a szótárból: `Közösségi aktivitással megszerzett jelvény.` (a modell tartaléka) és `HUHS jelvény` (a plugin tartaléka) — ezeken a helyeken maradt magyar.
  - A **kérdőív és a nyeremény válaszai** már a 2.14.0/2.14.1 óta **fordulnak** (élesben mérve: `Yes`/`No`/`I love it`/`I don't like it`, illetve a nyeremény kérdése/neve/leírása) — a **kvíz** kérdés-szerkezete viszont **nem** (beágyazott JSON, eddig szándékos korlát volt).
- **AMI ÉPÜLT (app-oldal):** a toplista és a közösségi lista a **megjelenítés helyén** fordít (`tr(context, badge)`, `tr(context, achievement.badgeName)`), a pont-egység `trArgs(context, '{n} pont', …)`; a szótár **+2 kulccsal** (**966**).
- **AMI ÉPÜLT (plugin 2.14.2 — a kvíz is):** a `huhs_translation_source_fields()` a beágyazott kérdés-szerkezetet **lapos kulcsokra** bontja (`_huhs_game_questions.0.prompt`, `…0.option.1`), ezért a szolgáltató **csak szövegeket lát**, és a **`correct` index hozzá sem kerül a kéréshez** (a helyes válasz nem tud elcsúszni). A kiolvasó (`huhs_translation_game_questions($id, $lang)`) **elemenként** esik vissza a magyarra; a nyilvános payload a fordított kérdéseket adja (`correct` **nélkül**), a **privát proxy-útvonal** viszont a `correct`-tel együtt (ott kell a kiértékeléshez).
- **BIZONYÍTÉK (mind mérve):** PHP-viselkedésteszt **83/83 × 2 futás** (a **16. szakasz**: laposítás, `correct`-mentesség, teljes fordítás, `correct` változatlan, magyar ág bájtazonos, nyilvános payload angolul `correct` nélkül, privát útvonal `correct`-tel); **PHP-mutációs bizonyíték 25/25** (`tmp/mutation-proof-php-plugin.mjs`); a **lint** mutációs bizonyítéka **12/12** (`tmp/mutation-proof-game-branch.mjs`); `node --test functions/game-payload-language.test.cjs` → **7/7**; `node tools/verify-plugin-package.mjs` → **41 ellenőrzés** (önteszt 7/7); `flutter analyze lib test` → **No issues found!**; `flutter test` → **1070/1070**; i18n-kapu zöld (**966 kulcs, 901/901 célzott, 0 duplikált**).
- **⚠️ SAJÁT HIBA, amit a mutációs bizonyíték fogott meg (ismét ugyanaz az osztály):** a lint első változata csak azt kérte számon, hogy a laposító függvény **létezik** — ezért a „laposítás elvétele" mutációt **nem kapta el** (0 bukó). A javítás: a lint a **kulcs-alakot** és a **forrás-beolvasó bekötését** is megköveteli. **A tanulság (harmadszor is): a „létezik" nem bizonyíték — a használatot és az alakot kell mérni.**
- **CSOMAG:** **`build/huhs-mobile-api-2.14.2.zip`** — **50 fájl**, **182 455 bájt**, SHA-256 **`DAF57407C964DD0E2ED24AAD34A5BAE6A52B4EA442B6A1BAAA351227D279B96E`**; patch: **`docs/plugin-2.14.2-quiz-translations.patch`** (3 fájl, **+101/−9**, SHA-256 `49916C6753760882BBCA927F7D18752D6E6994C51C014A2DC506C3DBEDF8B81E`), a **kiadott 2.14.1** csomagból kibontott bázishoz mérve (`git apply --check` → **exit 0**).
- **AZ AAB ÚJRAÉPÜLT (2026-09-26):** **`build/HUHS-v1.0.0+364-release.aab`** — **85 639 441 bájt**, SHA-256 **`A60C729BED47A926995135A48016C8F8F20A80539AB5E60B349C327CCA98BD2C`**; versionCode **364**, 966 kulcsú szótár, production AdMob ID, aláírás, és a **toplistát is említő új changelog-sor mindhárom ABI-ban** megvan (`node tmp/verify-aab-364.mjs`).
- **AZ IPHONE IS FRISS (2026-09-26 11:33):** az új app-kóddal (toplista + közösségi lista fordítás) a CI **`36232495800`** lefutott (`success`), az IPA **`build/ios-ipa-364c/Runner-unsigned.ipa`** — **26 355 129 bájt**, SHA-256 `47DD5E35B98E9C1617C2799725576ECDBDA2410BAB88A5732BA40422E66A72A9`, ellenőrizve **12/12** (`tmp/verify-ios-ipa.mjs`: build 364, **966 kulcsú szótár** az Achievement-kulcsokkal, mindkét 364 changelog-sor). Sideload: Sideloadly `installations.db` **id 21**, `2026-09-26 11:33`, ugyanaz a bundle-utótag → valódi frissítés, `failures_count 0`. ⚠️ **MÉRT ESZKÖZ-TANULSÁG:** a `sideloadly.exe` **nem** indítja el magától a telepítést minden alkalommal (a délelőtti futásnál igen) — a `Start` gombot **UIAutomation `InvokePattern`-nel** indítottam (`tmp/start-sideloadly.ps1`), egér-kattintás nélkül, majd a `Done. / 100%` és az adatbázis-sor igazolta az eredményt.
- **📌 A TULAJDONOS LÉPÉSE:** (1) a **2.14.2** plugin feltöltése (a 2.14.1-et cseréli) — utána a **kvíz kérdései is angolul** szólnak; a fordítás a **pótló körben** magától elkészül (a kérdések forrás-ujjlenyomata megváltozott → a két játék újra várólistás), vagy azonnal: `node tools/run-content-translation-sweep.mjs --rounds=5 --type=huhs_game`; (2) az **AAB (364)** feltöltése — ebben van a toplista/közösségi lista javítása (`A60C729B…BD2C`).
- **⚠️ ŐSZINTE KORLÁT:** a `timeline_items` (`artist`, `track_title`) **nem** fordul (tulajdonnevek); a **válaszok kézi angol felülírása** (WP-admin mező) továbbra sincs — a fordítás **automatikus**, viszont a tárolt térkép (`_huhs_translation_fields_en`) kézzel is javítható, és a magyar ág mindenhol bájtazonos. A **gépi** fordítás minőségét (főleg a kvíz-válaszoknál) érdemes a tulajdonosnak átnéznie.

### A JÁTÉK IS ANGOLUL — PLUGIN **2.14.1** (2026-09-26, a 2.14.0 éles mérése után)

- **A MÉRÉS (éles, nem tipp):** a 2.14.0 feltöltése után azonnal mértem: `GET /huhs/v1/faq?lang=en` → **`"How can I register?"`** (magyarul `Hogyan regisztrálhatok?`), `GET /poll/active?lang=en` → **`"Do you like the App?"`** + `Yes`/`No`/`I love it`, `GET /prize/active?lang=en` → **`"What is Noisecontrollers' real name?"`** — a **GYÍK, a kérdőív és a nyereményjáték tehát tényleg angolul megy**. A `translations/status` a feltöltés után **40 várólistás** elemet mutatott (`huhs_faq: 33`, `huhs_poll: 2`, `huhs_prize: 3`, `huhs_game: 2`), a pótló kör után **0**; a magyar ág mindenhol magyar maradt.
- **🔴 ÉLES HIBA, amit ez a mérés talált:** `GET /games/results/latest?lang=en` → **magyar** összefoglaló (*„Teszteld a hardstyle tudásod!…”*). **A gyökér (kettős):** (1) a `huhs_game_public_payload()` **nem kapott nyelvet** (a `/games/active`, `/games/results/latest`, `/games/{id}` closure-ök `$request` nélkül hívták), ezért a 2.14.0 által **lefordított és eltárolt** összefoglalót **nem olvasta ki** — ugyanaz a hibaosztály, mint a 2.13.0 „kész" jelzésénél: *a fordítás megvan, az olvasó nincs*; (2) a `title`/`type_label` a **magyar** `HUHS_GAME_TYPES`-ból ment ki, és ezek a címkék **az app szótárában sem** voltak benne (mérve: `Hardstyle kvíz` → `null`).
- **AMI ÉPÜLT (2.14.1):** (1) `huhs_game_public_payload($post, $lang = 'hu')` — az összefoglaló a **nyelvi olvasón** át megy ki (a valódi magyar meta a tartalék), a `title`/`type_label` pedig **névtárból** (`huhs_game_type_label($type, $lang)` + `huhs_game_type_labels_en()`, mind a **9** típus); (2) **mind a négy** nyilvános játék-végpont átadja a kért nyelvet (`huhs_request_lang($request)`); (3) a `$lang` **opcionális** (alap `hu`), ezért egyetlen régi hívó sem tört el, és a **magyar ág bájtazonos**.
- **BIZONYÍTÉK (mind mérve):** a PHP-viselkedésteszt **74/74 × 2 futás** — a **15. szakasz** a **tényleges** végpontot futtatja (a `rest_api_init` hookok regisztrálják az útvonalakat, majd a callbacket hívjuk `lang=hu`/`en`/nélkül: magyar bájtazonos, angolul a fordítás + `Hardstyle Quiz`, a `correct` nem szivárog); `node tools/run-php-plugin-tests.mjs` → **MINDEN ELLENŐRZÉS RENDBEN** (49 PHP fájl, 74/74, 74/74, push-lánc 12/12); **mutációs bizonyíték 22/22** bájtpontos visszaállítással (benne a névtár elvétele, az olvasó elvétele, a `lang` elvétele — mind **elkapva**), és **ÚJ** `tmp/mutation-proof-game-branch.mjs` **6/6** a forrás-lintre; `node tools/verify-plugin-package.mjs` → **36 tartalmi ellenőrzés** (önteszt 7/7); `node --test functions/game-payload-language.test.cjs` → **6/6**.
- **SAJÁT HIBA, amit a mutációs bizonyíték fogott meg:** a lint első változata csak azt kérte számon, hogy a névtár-függvény **létezik**, és hogy a magyar ág rendben van — ezért a „névtár elvétele" mutációt **nem kapta el** (0 bukó). A javítás: a lint a **használatot** is megköveteli (`return isset($names[$type]) ? $names[$type] : $fallback;`). **Tanulság (ismét): a „létezik" nem bizonyíték — a használatot kell mérni.**
- **✅ A 2.14.1 FELTÖLTÉSE UTÁNI ÉLES MÉRÉS (2026-09-26, minden mérve — `node tmp/verify-2140-live.mjs`, 12/12):** GYÍK **33/33 angolul** (`"How can I register?"`, kategória `"Getting Started"`; magyarul `"Első lépések"`), kérdőív `"Do you like the App?"` + `Yes/No/I love it/I don't like it` (magyarul `"Tetszik az Applikáció?"` + `Igen/Nem/Imádom/Nem tetszik`), nyeremény `"What is Noisecontrollers' real name?"` + `"Couple entry to the Hard Base event!"` (magyarul `"Mi Noisecontrollers igazi neve?"` + `"Páros belépő a Hard Base rendezvényre!"`), és a **játék** `"Test your hardstyle knowledge!…"` + **`Hardstyle Quiz`** (magyarul `"Teszteld a hardstyle tudásod!…"` + `Hardstyle kvíz`) — vagyis a 2.14.1 javítása **élesben is működik**, a magyar ág pedig mindenhol változatlan. A `/games/active` **üres törzzsel** válaszol, ha nincs aktív játék (`rest_ensure_response(null)`) — ez **nem hiba**, hanem a végpont szándékos viselkedése; a nyelvi bizonyítékot ezért a zárt játék eredmény-végpontján mértem.
- **⚠️ SAJÁT MÉRÉSI HIBA (a 12. ellenőrzés első bukása):** a „van ékezet? = magyar" heurisztika **nem elég** — a `"Mi Noisecontrollers igazi neve?"` ékezet nélkül is magyar, ezért a magyar ág ellenőrzése **hamisan bukott**. A javítás: a magyar ágat a **különbözés** (magyar ≠ angol) + a valóban ékezetes mezők (`prize_type`, `prize_description`) igazolják. **Tanulság (ismét): a heurisztika nem bizonyíték — a konkrét értéket kell mérni.**
- **CSOMAG:** **`build/huhs-mobile-api-2.14.1.zip`** — **50 fájl**, **181 693 bájt**, SHA-256 **`C122409E356337BCFAC20FD47A20DED418DAB1B75D59CC902BAC9EE446CD969A`**; patch: **`docs/plugin-2.14.1-game-language.patch`** (2 fájl, **+69/−12**, SHA-256 `6285B8B0C74741A97E079AEA2E44F8CD84690DA06B016BA29794562EF0EF4047`), a **kiadott 2.14.0** csomagból kibontott bázishoz mérve — `git apply --check` a kiadott fára **exit 0**, alkalmazás után **49/50 fájl bájtazonos**, a `games.php` pedig **LF-normalizálás után azonos** (CRLF, nem tartalom).
- **📌 A TULAJDONOS LÉPÉSE:** a **2.14.1** feltöltése — ✅ **megtörtént (2026-09-26)**, és az éles mérés igazolja (lásd fent). Az **AAB (364)** viszont **még nincs fent a Playen** — az a következő lépés (`build/HUHS-v1.0.0+364-release.aab`, SHA-256 `D65C0EB2…C9A0`). Az **iPhone** a mai app-kódot futtatja: **2026-09-26 11:08-kor újrasideloadolva** (Sideloadly `installations.db` **id 20**, `final_bundle_id hu.hungarianhardstyle.app.JQPJ793V65` — valódi frissítés, `failures_count 0`; az IPA `53BBAE09…`, CI-futás `36201481958`). Az **Achievement-nevek** app-oldaliak (lásd a következő szakaszt).
- **⚠️ ŐSZINTE KORLÁT:** a játék `questions` és `timeline_items` mezője **továbbra sem fordul** (beágyazott szerkezet; a lapos mező-térkép nem tudja biztonságosan leképezni — ez a következő kör), és a `guess_*` típusoknál a **trackcímek/előadónevek** tulajdonnevek maradnak az angol módban is.

### A TESZT-KAPU HELYREÁLLÍTVA + A 364 ÚJRAÉPÍTVE (2026-09-26)

- **A KIINDULÁS (mérve, nem tipp):** a teljes függvényteszt-kör **3 bukóval** állt (`node tools/run-function-tests.mjs` → *„BUKÓ: tiszta tesztek (3 bukó)"*, 271 teszt / 268 pass), miközben a PHP- és a Flutter-kör zöld volt. A bukó a **`functions/prize-active-payload.test.cjs` forrás-lintje** volt: a 2.14.0-ban a nyitott nyeremény-ág a mezőket a **nyelvi olvasón** keresztül adja ki (`'prize_type' => huhs_translation_text($prize_id, $lang, '_huhs_prize_type', (string) get_post_meta(…))`), a lint viszont a **régi** alakot kereste; emellett a végpont aláírása is megváltozott (`WP_REST_Request $request = null`), ezért a `function huhs_prize_api_active()` keresés **nem talált semmit** (hamis bukás).
- **A JAVÍTÁS (nem elfedés):** a lint **szigorúbb** lett — megköveteli, hogy a mező a **nyelvi olvasón** át menjen ki, és hogy a **tartalék a valódi `get_post_meta(...)`** legyen (üres string nem lehet: ez volt a 2026-09-24-i élő hiba); méri a **`has_en`** jelzőt és a **válaszlista elemenkénti** fordítását; a `correct` szivárgás tiltása megmaradt. A fix karakterszámú ablak (`slice(-900, +1200)`) helyett **függvényhatárok** — a hosszabb, több soros hívások levágása ugyanis **hamis bukást** adott.
- **ÚJ VALÓDI VISELKEDÉSMÉRÉS (nem lint):** a `tools/php/plugin-translation-test.php` **14. szakasza** a **tényleges** `huhs_prize_api_active()`-ot futtatja (a `posts.php`/`poll.php`/`prize.php` betöltésével, ezért a `lang` a **valódi** `huhs_request_lang()`-ból jön): a magyar kérés bájtazonos, az angol a fordítást adja (kérdés, válaszok, nyeremény neve/leírás, `has_en`), fordítás nélkül a magyar megy ki `has_en=false`-szal, és a **helyes válasz egyik ágban sem szivárog ki**. Ezzel a PHP-kör **63 → 68 ellenőrzés** (68/68 × 2 futás).
- **MUTÁCIÓS BIZONYÍTÉKOK:** a PHP-kör **20/20** (`tmp/mutation-proof-php-plugin.mjs`; **+2 új**: a nyitott ág üres tartaléka és a `lang` beégetése — mindkettő **elkapva**, bájtpontos visszaállítással), és **ÚJ** `tmp/mutation-proof-prize-branch.mjs` **5/5** a lintre (tartalék elvétele, olvasó elvétele, `has_en` beégetése, lista fordítása nélkül, `correct` kiszivárogtatása — mind **elkapva**, SHA-256-tal igazolt visszaállítással).
- **⚠️ TANULSÁG (a saját mérésem hibája):** a `tools/run-php-plugin-tests.mjs` és a `tmp/mutation-proof-php-plugin.mjs` **ugyanazt a `tmp/php-plugin` könyvtárat** használja, ezért **együtt futtatva** a runner újrakibontása elviszi a mutációt → a proof „NEM KAPTA EL"-t, a runner pedig „67/68"-at mért. **Sorban kell futtatni őket** — a hamis eredmény nem a kód hibája volt.
- **SZÓTÁR:** az Achievement-kör **duplikált kulcsot** hagyott (`Sokat tesz a közösségi jelenlétért.` kétszer; a JSON csendben az utolsót használja — a kapu jelzi). **Egy példány törölve**, a kulcslista frissítve (`node tools/extract-ui-strings.mjs --write`) → **964 szótár-kulcs, 901/901 célzott szöveg (100%), duplikált kulcs 0**.
- **A 364 ÚJRAÉPÍTVE (2026-09-26):** **`build/HUHS-v1.0.0+364-release.aab`** — **85 639 370 bájt** (81,67 MB), SHA-256 **`D65C0EB2CD71A9299C7B3E9177958BD7B42EAD13CBC174979E40A5CC0C24C9A0`**; a csomagban (mérve, `node tmp/verify-aab-364.mjs`): **versionCode 364**, `versionName 1.0.0`, production AdMob App ID bent, **teszt ID nem**, aláírás bent, **964 kulcsú szótár** (benne `Kezdő ütem → First Beat`, `{n} pont → {n} points`, `Közösség → Community`, `Partyajánló → Party Guide`), és az **új changelog-sor mindhárom ABI-ban** megvan. ⚠️ **MÉRT AOT-TANULSÁG:** a beépített (konstans argumentumú) fejléc-függvények neve **nincs** a snapshotban (`homeHeaderFitsCommunityLabel` = NINCS), a `homeHeaderLabelWidth` viszont megvan — ez a mérhető bizonyíték az új fejléc-kódra; a viselkedést a widget-tesztek mérik.
- **A Play-jegyzet a 364-re állítva:** az 1. blokk **353/500**, az 1b. (nyilvános, 359–364 összesítő) **471/500** (mindkettő a 480-as biztonsági sáv alatt), a meta `sha256` és az AAB-tábla az új hasonra; `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN**.

### A GYÍK, A KÉRDŐÍV ÉS A NYEREMÉNYJÁTÉK IS ANGOLUL — PLUGIN **2.14.0** (2026-09-26)

- **A TULAJDONOS JELZÉSE:** *„a Segytség/GYÍK magyar maradt / a poll tartalma magyar maradt / a nyereményjáték tartalma magyar maradt"* — vagyis angol módban ezek a **tartalmak** magyarul jöttek.
- **A MÉRÉS (éles, nem tipp):** `GET /huhs/v1/faq?lang=en` → `question`/`answer`/`category` **magyarul** (a végpont a `lang`-ot **egyáltalán nem olvasta**); `GET /poll/active?lang=en` → `question: „Tetszik az Applikáció?"`, `options[].label: Igen/Nem/Imádom`; `GET /prize/active?lang=en` → `question`, `answers[].label`, `prize_type`, `prize_description` **magyarul**.
- **A GYÖKÉR (két különböző ok):** (1) a **GYÍK** szövege a **címben és a törzsben** van (`post_title` = kérdés, `post_content` = válasz), de a `huhs_faq` típus **nem volt benne** a fordítási listában; (2) a **kérdőív** és a **nyereményjáték** szövege **post metában** él (`_huhs_poll_question`, `_huhs_poll_options`, `_huhs_prize_question`, `_huhs_prize_answers`, `_huhs_prize_type`, `_huhs_prize_description`), amit a 2.12/2.13-as mechanizmus **egyáltalán nem látott**.
- **AMI ÉPÜLT:**
  1. **`includes/translation-fields.php` (ÚJ)** — meta-alapú szövegek fordítása. Mező-térkép típusonként (`text` / `list`), a forrás beolvasása, a fordítás tárolása **egy** JSON metában (`_huhs_translation_fields_en`) + forrás-ujjlenyomat (`_huhs_translation_fields_hash`), **saját** hiba-jelölő (nem blokkolja a cím/törzs ágat). A szolgáltató kérése ugyanaz (szigorú JSON), de `{"fields": {…}}` alakban; a feldolgozó **megköveteli a lista hosszának egyezését**, ezért félkész fordítás nem íródhat be. A kiolvasó (`huhs_translation_text` / `huhs_translation_list`) **elemenként** esik vissza a magyarra.
  2. **GYÍK a közös kapun:** a `huhs_faq` bekerült a `huhs_translation_post_types()`-ba (most **6** típus), a `/faq` végpont pedig a `huhs_translation_meta_values()`-ot használja (kérdés = cím, válasz = törzs) + `has_en` jelzőt ad. A **kategória-nevek** külön névtárból fordulnak (a mért 8 kategória) + term-meta felülírással (`_huhs_name_en`).
  3. **Minden típus egy listában:** új `huhs_translation_all_post_types()` (a 6 cím/törzs típus + `huhs_poll` + `huhs_prize` + `huhs_game`), ezt használja a mentés-ág, a pótló kör és a `status` végpont. A `huhs_translation_post_needs_work()` **egy helyen** dönti el, hogy egy elem várólistás-e (cím/törzs VAGY meta-szöveg hiányos, és nem hibázott nemrég) — így a korábbi „üres törzs = nem várólistás" szabály nem zárja ki a meta-szöveges típusokat.
  4. **A végpontok** (`poll/active`, `prize/active` mindkét ága) a mező-fordítást olvassák; a magyar ág **bájtazonos** (nem-angol kérésre az eredeti érték).
- **BIZONYÍTÉK (mind mérve):** a PHP-viselkedésteszt **63/63 × 2 futás** (a kibontott ZIP-en) — benne a mező-térkép, a forrás-beolvasás, a teljes fordítás + naprakészség, a hibaág, a **lista-hossz elutasítása**, a GYÍK-névtár és a pótló kör meta-szöveges ága; `node tools/run-php-plugin-tests.mjs` → **MINDEN ELLENŐRZÉS RENDBEN** (49 PHP fájl lintje, 63/63, 63/63, push-lánc 12/12); **mutációs bizonyíték 18/18** bájtpontos visszaállítással (`tmp/mutation-proof-php-plugin.mjs`, benne a mező-fordítás elvétele, a hossz-ellenőrzés elvétele, a GYÍK-névtár elvétele); `node tools/verify-plugin-package.mjs` → **31 tartalmi ellenőrzés** + a ZIP fájllistája bájtazonos (önteszt 7/7); `node tools/check-plugin-encoding.mjs` → 50/50 fájl UTF-8.
- **SAJÁT HIBA, amit a teszt fogott meg:** a `huhs_translation_stored_fields()` először **kérés-szintű `static` gyorsítótárral** dolgozott, ezért a **beíró ugyanabban a kérésben** (a pótló kör ír, majd rögtön számol) a régi, üres térképet látta → a frissen lefordított elem `partial` lett és **újra várólistásnak** látszott (minden körben felesleges API-hívás). A gyorsítótár **kivéve**, mert a WordPress meta-cache így is olcsó.
- **CSOMAG:** **`build/huhs-mobile-api-2.14.0.zip`** — **50 fájl**, **180 572 bájt**, SHA-256 **`597C1B24A2E26481A9CA668D0CABA80825262D9B2D5F1679F15323F7F5696B1C`**; patch: **`docs/plugin-2.14.0-content-translations.patch`** (8 fájl, **+701/−47**, SHA-256 `682846036CEE519B53E5C2B1A14EA12D045522F29DA4EF18EC10BFDB5D68F1AC`, a **kiadott 2.13.0** csomagból kibontott bázishoz mérve).
- **📌 A TULAJDONOS LÉPÉSE:** a **2.14.0** feltöltése (a 2.13.0-t cseréli) — ✅ **megtörtént (2026-09-26)**, és az éles mérés szerint a GYÍK, a kérdőív és a nyereményjáték angolul megy; a **játék** végpontját a **2.14.1** javítja (lásd a legfelső szakaszt). Utána a hiányzó fordítások **magától** pótlódnak (óránkénti pótló kör), vagy egy paranccsal: `node tools/run-content-translation-sweep.mjs --rounds=20`. Az app **nem** igényel frissítést ehhez: a `?lang=en` paramétert már minden kérésre elküldi, és a cache is nyelvenként külön válik.
- **⚠️ ŐSZINTE KORLÁT:** a `huhs_game` `_huhs_game_questions` mezője (a kvíz kérdés-szerkezete) **szándékosan kimarad** ebből a körből (beágyazott szerkezet) — a játék **összefoglalója** viszont fordul. A GYÍK-névtár a mért 8 kategóriát fedi; **új** kategóriát a term-meta `_huhs_name_en`-nel lehet angolosítani. A **jelvények** neve/leírása **nem** itt fordul: az **app-oldali szótárból** (lásd a következő szakaszt), mert a katalógust a Firebase-függvény egy nyelven cache-eli.

### AZ ACHIEVEMENT-NEVEK ÉS -LEÍRÁSOK IS ANGOLUL + a „Közösség" gomb kifér (2026-09-26, app-oldal)

- **A TULAJDONOS KÉRÉSE:** *„az Achievement nevek is lehetnének angolul az angol verzióban és a leírása is"*, illetve korábban: *„Magyarul »Közösség«, Angolul »Community«, de férjen ki"*.
- **A MÉRÉS (nem tipp):** a jelvény-katalógus **két helyről** jut el a felületre: a szerverről (`/achievements/badges` → a Firebase-függvény katalógusa) és az app **tartalék listájából** (`achievement_service.fallbackLevels`). A megjelenítés (`AchievementBadgeCard`, `achievement_guide_screen`) a szótárból fordít — **a 7 jelvénynév viszont egyáltalán nem volt a szótárban** (`Kezdő ütem`, `Első lépés`, …), a leírások közül **egy** hiányzott, a kártya pedig a `pont` egységet **beégetve** írta ki.
- **AMI ÉPÜLT:** (1) `assets/i18n/en.json` **+9 kulcs** (7 jelvénynév, a hiányzó leírás, `{n} pont`); (2) az `AchievementBadgeCard` a nevet `AppText`-tel, a leírást és a pont-egységet `tr`/`trArgs`-tal fordítja; (3) **valódi hiba javítva:** a `static final` adatlisták **betöltéskor** fordítottak (`AppStrings.tr(...)`), ezért **rögzítették a nyelvet** — ha az app angolul indult, magyar módban is angol szöveg maradt. Mostantól az adat **magyar kulcs**, a fordítás a megjelenítés helyén (`AppText`/`tr(context, …)`).
- **AMIÉRT NEM szerveroldali:** a jelvény-katalógust a Firebase-függvény **egy nyelven** cache-eli (30 s), ezért egy plugin-oldali `name_en` önmagában nem érne el az appba — ahhoz a függvényt is át kellene írni és deployolni. A szótári út **azonnal** működik, és a 7 jelvény stabil; ha később **új** jelvény jön, annak a neve a következő app-verzióig magyar marad (dokumentált korlát).
- **A „Közösség" gomb:** a fejléc küszöbe korábban **fix 360 px** volt, a magyar feliratra mérve — angolul a hosszabb „Community" miatt a sor nem fért ki, és a tartalék `FittedBox` **lekicsinyítette az egész fejlécet**. Mostantól a döntés a **tényleges (lefordított) felirat szélességéből** jön (`homeHeaderFitsCommunityLabel`), a gomb belső margója 12 → 10 px, a hézagok 8 → 6 px, és a felirat csak akkor jelenik meg, ha **valóban kifér** (különben ikonná válik) — így **semmi nem zsugorodik**.
- **BIZONYÍTÉK (mind mérve):** `flutter analyze lib test` → **No issues found!**; `flutter test` → **1068/1068** (a kör előtt 1052; a fejléc **mindkét nyelven, 7 szélességen** nem csordul túl és **nem zsugorodik**, + az Achievement-teszt 6 ellenőrzése); `node tools/check-i18n.mjs --strict` → **MINDEN ELLENŐRZÉS RENDBEN** (964 szótár-kulcs, 901/901 célzott szöveg, duplikált kulcs 0); **mutációs bizonyíték 3/3** a címke-fordításra (`tmp/mutation-proof-content-labels.mjs`). ⚠️ **A korábbi „1080/1080" elírás volt** (nem futott le a teljes kör) — a **mért** érték ez.
- **⚠️ ŐSZINTE KORLÁT:** a **szerverről** jövő jelvénynév/-leírás a szótárból fordul, ezért ha a tulajdonos a WordPress-adminban **átírja** a jelvény nevét/leírását, az új szöveg a következő app-verzióig magyarul látszik. A fejléc döntése a **valós betűtípus** szélességét méri; a tesztekben az Ahem betűtípus ~2× szélesebb, ezért a tesztek a **font-független** tulajdonságokat kérik számon (nincs túlcsordulás, nincs zsugorodás, bő helyen látszik a felirat).
- **📌 A TULAJDONOS LÉPÉSE:** ezek **app-oldali** változások → a **364-es AAB 2026-09-26-án ÚJRAÉPÜLT** ezekkel együtt (`build/HUHS-v1.0.0+364-release.aab`, SHA-256 `D65C0EB2…C9A0`) — a feltöltés a tulajdonos dolga, és utána az iPhone-sideload is frissíthető.

### AZ IPHONE-RA FELKERÜLT A MAI KÓD (iOS sideload, 2026-09-26)

- **A TULAJDONOS KÉRÉSE:** *„sideloadold iphone-ra"* — vagyis a mostani kód (a hírek címke-/kategória-fordításával) kerüljön fel a **fejlesztői iPhone-ra**. Ez **nem** Play/App Store kiadás: a szabad Apple ID-s út (CI → aláírás nélküli IPA → **Sideloadly**).
- **A MEGMÉRT ÚT (nem emlékezet):**
  1. **Commit + push** (`904b7527`, branch `codex/v1.0`) — **ez indítja a CI-t**: a workflow `push` szűrője a `lib/**`-ra is figyel („a telefonos teszthez MINDEN kódváltozásból új IPA kell"), a `workflow_dispatch` gombot pedig a GitHub csak az **alapértelmezett** (`master`) branch-en mutatja. **Futás: `36198530160`, `success`, ~10,5 perc.**
  2. **Letöltés:** `gh run download 36198530160 --name HUHS-ios-unsigned-ipa --dir build/ios-ipa` → **`Runner-unsigned.ipa`**, **26 356 756 bájt** (25,14 MB), SHA-256 **`9A7D856A7384870331BF64FB04222224962EDF986EC23E07795ED384F67E5A3E`**.
  3. **Ellenőrzés a csomagban (ÚJ eszköz: `tmp/verify-ios-ipa.mjs`, 11/11):** `Payload/Runner.app` megvan; `Info.plist` (nyers bájt-keresés, mert **bináris plist**): `1.0.0`, build **364**, `hu.hungarianhardstyle.app`, `UIBackgroundModes`, ATT-leírás, AdMob iOS app ID; **a szótár az `Frameworks/App.framework/flutter_assets/assets/i18n/en.json`-ban van (956 kulcs)** és a **címke-fordítások is** (`Partyajánló → Party Guide`, `fesztivál → festival`, `Zene → Music`); a 364 changelog-sora bent van (UTF-16LE az `App.framework/App`-ban). ⚠️ iOS-en a Flutter-assets az **`App.framework`** alatt van, nem a `Runner.app` gyökerében (ez az első mérésem hibája volt).
  4. **Sideload:** a `sideloadly.exe` **az IPA útvonalával elindítva** magától végigfutott — az ablak **`Installing… 0%` → `Done. 100%`** (~40 s). A UI-t **UIAutomation**-nel olvastam ki (nem kattintottam): az IPA betöltve, az iDevice `Imre iPhone-ja (26.7) 00008030-001475413ED2402E @USB`, az Apple ID **emlékezett** (`djdeeroy@gmail.com [9 App IDs Remaining, updates 3d]`), a `Start` gomb a folyamat közben letiltva.
  5. **Bizonyíték a Sideloadly saját adatbázisából** (`%LOCALAPPDATA%\Sideloadly\installations.db`, csak olvasva — új eszköz: `tmp/check-sideloadly-installs.mjs`): új sor **id 18**, `updated_at 2026-09-26 01:03:54`, `final_bundle_id hu.hungarianhardstyle.app.JQPJ793V65` (**ugyanaz, mint korábban → valódi FRISSÍTÉS, az app-adatok megmaradnak**), `failures_count 0`, `last_error` üres, `known_ttl 7` nap, `refresh_at_hours 96`.
- **⚠️ AMIT NEM TUDTAM MEGMÉRNI (őszintén):** a **készüléken való elindulást** nem — a `pymobiledevice3` ezen a gépen **nem használható** (a Python-telepítés elromlott: *„Executable … pythoncore-3.14-64\python.exe is for a different kind of processor architecture"*, és csak a WindowsApps-alias van). A korábbi kör ezzel tudott launch-olni és képernyőképet készíteni; ez most **kiesett**. A telepítés tényét a Sideloadly adatbázisa igazolja, a megnyitás a tulajdonos egy koppintása. **A profil-megbízás** (`Beállítások → Általános → VPN és eszközkezelés`) azért nem kell újra, mert **ugyanaz a tanúsítvány** (`cert-djdeeroy@gmail.com.pem`) írta alá, mint a délelőttit — ha az app mégsem indul, az az első dolog, amit nézni kell.
- **A sideloadolt build jellemzői (a CI zászlai, mérve a workflow-ból):** `HUHS_APP_CHECK_DEBUG_IOS=true` (a debug App Check token a készüléken keletkezik és a Firebase-konzolban regisztrálva van → a védett callable-ok 200-at adnak) és `HUHS_ENABLE_TEST_ADS=true` (a Google **teszt**-egységei, mert az iOS AdMob app még nincs jóváhagyva). Az **éles** TestFlight-build egyik zászlót sem kapja meg.
- **⚠️ Az ingyenes Apple ID korlátai (tények, nem hibák):** az aláírás **7 napig** érvényes (utána újra át kell húzni), egyszerre max. 3 app, a bundle ID **utótagot** kap (`.JQPJ793V65` — állandó, ezért a frissítés valódi frissítés), és a **push** (`aps-environment`) így nem működik.

### A HÍREK KATEGÓRIA-/CÍMKE-NEVEI IS ANGOLUL — **app-oldali** megoldás (2026-09-25)

- **A TULAJDONOS DÖNTÉSEI:** (1) *„label maradhat úgy"* — a kiadvány/`huhs_release` **marad**, ahogy van (nincs benne fordítható szöveg: a cím név, leírás-mező nincs); (2) a hírek **kategória-/címke-neveit** angolosítani akarja; (3) a **címkéknél** csak a **magyar közneveket** fordítjuk — a tulajdonnevek (`Adam Bass`, `adaro`, `#TBT`), a márkák és az **elírások** (`festiva`, `akvérium`) maradnak.
- **A MÉRT KÉSZLET (éles WordPress, 2026-09-25):** **13 kategória** (szerkesztői, stabil) és **600 címke** — a címkék túlnyomó része tulajdonnév/márka/angol szó, a **magyar köznevek** a legnépszerűbbek között vannak (`elektronikus zene` 201, `hirek` 148, `buli` 83, `fesztivál` 68, `zene` 25, `interjú` 16, `magyar hardstyle` 9, `Jövő` 7, `képek` 7, `fesztiválszezon` 7, `Hollandia` 6, `partyképek` 4, `horvátország` 3, `megjelenés` 3).
- **MIÉRT APP-OLDALI (és nem plugin):** a kategória-nevek a `/wp/v2/categories` válaszból jönnek, amit az app **egyszer** letölt és **nyelvfüggetlenül cache-el** (`huhs.wp.categories`) — ha a plugin fordítaná őket `lang=en` alapján, a cache miatt a magyar felület is angol nevet kapna. A **megjelenítésnél** fordítani viszont mindkét problémát megoldja: nincs új WordPress-mező, nincs plusz hálózat, és nyelvváltáskor **azonnal** vált a felirat. A `huhs/v1` payload `categories`/`tags` mezői ugyanígy a szótárból fordulnak.
- **AMI ÉPÜLT:** (1) `assets/i18n/en.json` **+22 kulcs** (9 kategória + 14 magyar címke-köznév; a `Hírek` már benne volt) — **955 kulcs**; (2) a megjelenítési helyek: `news_screen` szűrő-chip (`tr(context, category.name)`), `news_detail_screen` címke-chip (`'#${tr(context, tag)}'`), `tagged_news_screen` cím (`'#${tr(context, widget.tag)}'`) — a `#` szándékosan a **kulcson kívül** van; (3) új teszt **`test/services/i18n_content_labels_test.dart` (7/7)**: a mért nevek benne vannak, a fordítások **valódiak** (nem üresek, nem a kulccsal azonosak), angol módban fordít, **magyar módban változatlan**, a tulajdonnevekhez **nincs** fordítás, és forrás-lint a három megjelenítési helyre.
- **BIZONYÍTÉK (mind mérve):** `flutter analyze lib test` → **No issues found!**; `flutter test` → **1052/1052** (a kör előtt 1045; **+7** az új teszttel); `node tools/check-i18n.mjs --strict` → **MINDEN ELLENŐRZÉS RENDBEN** (a 22 új kulcs „extra"-ként jelenik meg, ami **nem** hiba — a kapu csak a célzott szövegeket méri); `node tools/extract-ui-strings.mjs --write` → **900 kulcs** (a dinamikus kulcsok szándékosan nem célok); **mutációs bizonyíték 3/3** (`tmp/mutation-proof-content-labels.mjs`, bájtpontos visszaállítással): a `Zene` kulcs törlése, a szűrő-chip és a címke-chip fordításának elvétele → mind **elkapva**.
- **⚠️ SAJÁT HIBA, amit a saját tesztem fogott meg:** az első szótár-bővítésből **kimaradt a `Zene`** (a kisbetűs `zene` címkéhez bekerült, a nagybetűs kategóriához nem) — a „mind benne vannak" és a „valódi fordítások" ellenőrzés **azonnal elbukott**. Ezért van a teszt.
- **⚠️ A MEGOLDÁS KORLÁTAI (őszintén):** (1) a **tulajdonnevek és az elírások** szándékosan nem fordulnak (a tulajdonos döntése), és egy **új** magyar címke a következő app-verzióig magyarul látszik; (2) a szótár **pontos egyezéssel** keres, ezért egy más írásmódú címke (`Fesztivál` nagybetűvel) a listában marad — a kis/nagybetűt nem kezeljük (nem akartunk újabb mechanizmust a szótárba); (3) a magyar oldal **bájtazonos** (ezt a teszt is őrzi).
- **📌 A TULAJDONOS LÉPÉSE:** ez **app-oldali** változás, ezért **AAB** kell hozzá (a WordPressen nincs teendő). A verzió attól függ, hogy a **364** felkerült-e már a Play-re. **A tulajdonos válasza: a 364 még NINCS fent**, ezért a csomag **364-ként épült újra** (nem vész el verziókód).
- **✅ AZ ÚJRAÉPÍTETT 364 (2026-09-25, minden mérve):** **`build/HUHS-v1.0.0+364-release.aab`** — **85 637 701 bájt** (81,67 MB), SHA-256 **`3D8C260B04EC72C01FA3AA842FDA31C5AC02D4B0E921B54020495C193CD2AF60`**; a merge-elt manifestből visszaolvasva **versionCode 364**, `versionName 1.0.0`; a **szótár (956 kulcs)** benne van a csomagban (`base/assets/flutter_assets/assets/i18n/en.json`, ellenőrizve: `Partyajánló → Party Guide`, `fesztivál → festival`, `Zene → Music`); az **új 364 changelog-sor mindhárom ABI** `libapp.so`-jában megvan (UTF-16LE); production AdMob App ID bent, **teszt ID nem**; **1 aláírási identitás** (`META-INF/HUHS-UPL.RSA`). A `docs/PLAY-KIADASI-JEGYZET.md` meta + az AAB-tábla az új hasonra állítva, és a **Play-jegyzet 1. blokkja 462/500** (a biztonsági sáv 480 alatt) — `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN**.
- **⚠️ A Play-jegyzet régi „A feltöltendő AAB" táblája 360-as volt** (elavult a 364-es körből) — most a 364-re állítva, és a 360-ról szóló bekezdések **történetiként** megjelölve. **Tanulság:** a táblát is frissíteni kell, nem csak a fejléc-meta blokkot (a `check-play-notes.mjs` a meta hasht ellenőrzi, a táblát nem).

### A DJ / SZERVEZŐ / ESEMÉNY / **LABEL** ANGOLJA IS AUTOMATIKUS — PLUGIN **2.13.0** (2026-09-25)

- **A TULAJDONOS KÉRÉSE:** *„jó lenne ha a dj, szervező, esemény és a label is így működne, hogy automata felkerül angolul is"* — vagyis ugyanaz, mint a cikkeknél. (Az app „Label" füle a `ReleasesScreen`, tehát a **kiadvány** = `huhs_release`.)
- **A MÉRÉS ELŐBB (nem tipp).** A 2.12.0 cronja már mind az öt típust fordítja, de **csak mentéskor** (`save_post` + `publish`) — ezért a **plugin előtt létrejött** tartalom angol nélkül maradt. Új, csak olvasó szonda **`tmp/probe-translation-coverage.mjs`** (REST `context=edit` + nyilvános `?lang=`): cikk **30/100** (a régi 70-en nincs), esemény **3/9** (a 6 régi eseményen nincs — és ezek az appban **elérhetők**, mert az `events_provider`/`content_target`/`chat_mention_source` `include_past=1`-et használ), DJ **12/20** (a 8-on **magyar szöveg sincs** → nincs mit fordítani), szervező **3/4** (1-nek nincs szövege), **kiadvány 0/22 — a kiadvány egyetlen szövege a cím (név), a payloadban nincs leírás, a `huhs_release` csak `title`-t támogat**, tehát ott nincs fordítanivaló.
- **A MÁSODIK MÉRÉS — mi marad magyarul angol módban? (`tmp/probe-english-mode-leftovers.mjs`, csak olvas):** a prózai szövegek mind angolul mennek, DE `country`/`venue_country` = **„Magyarország"** (17 DJ, 4 szervező, minden esemény), `city` = „Bécs", valamint a **múltbeli események** címe/leírása. A **Label** fülön 1 elem jelzett magyar szöveget: az a **dal címe** (`… BAZ+ (Túlvagyok Rajtad) …`) — az tulajdonnév, nem fordítjuk.
- **AMI ÉPÜLT (plugin 2.13.0, három rész):**
  1. **`includes/translation-sweep.php` (ÚJ) — önjavító pótlás.** Óránként (WP-cron `huhs_translation_sweep_event`) végigmegy a publikált elemeken, és amelyiken van magyar szöveg, de **nincs** `_huhs_content_en`, azt lefordítja. Típusonként max. **3 elem / 20 másodperc** futásonként (`huhs_translation_sweep_limit`, `huhs_translation_sweep_budget`, `huhs_translation_sweep_enabled` szűrő), plusz admin végpontok: **`GET /huhs/v1/translations/status`** (mi vár még) és **`POST /huhs/v1/translations/sweep`** (`type`, `limit`, `budget`; `manage_options`). ⚠️ **Nem hoz létre értesítést:** közvetlenül a fordítást hívja (meta-írás), nem `wp_update_post`-ot — így a `save_post`-lánc és a push sem fut le (ezért nem spammel 90 régi cikk átfordításakor).
  2. **Ujjlenyomat + hibajelző** (`_huhs_translation_hash`, `_huhs_translation_failed[_at]`): egy díszítő mentés nem költ API-hívást, a **megváltozott** magyar szöveg viszont újrafordítódik, a tartós hiba pedig 6 óráig nem próbálkozik (`huhs_translation_retry_delay`).
  3. **Hely-névtár `includes/translation-places.php` (ÚJ)** — az ország/város **nem prózai szöveg**, ezért determinisztikus (nincs API-hívás): `Magyarország → Hungary`, `Ausztria → Austria`, `Bécs → Vienna`. A **magyar ág bájtazonos** (nem-angol kérésre az eredeti érték), ismeretlen névre **nem tippel**. ⚠️ A **„Velence" szándékosan nincs** a listában: a magyar Velence (Velencei-tó, a Hard Lake helyszíne) ↔ olasz Venice — egy vak névtár **félrefordítaná** a magyar eseményt.
- **KÉT VALÓDI HIBA, amit ez a kör talált (mindkettő javítva, mérve):**
  - **A DJ/szervező adatlapján a közelgő események angol módban is magyarul jöttek:** a `huhs_get_artist_upcoming_events()`-ben a `$lang` **nem is volt paraméter** (undefined változó → `hu`), a szervezőnél pedig az `array_map('huhs_build_event_response', …)` **csak egy argumentumot** adott, ezért a nyelv az alapértékre esett vissza. Javítva: a nyelv átadása mindkét úton (`use ($lang)`).
  - **A „kész" jelzés a MEGLEVŐ metára épült** a `huhs_run_translation`-ban: ha a magyar szöveg megváltozott, de a válasz fél volt (nincs törzs), a **régi** angol meta miatt a kör késznek jelölte → az elavult angol maradt volna. A javítás **a válaszból** számol (ezt a saját mutációs bizonyíték kapta el, 1 bukó).
- **BIZONYÍTÉK (mind mérve):** PHP-viselkedésteszt a **kibontott ZIP-en 45/45 × 2 futás** (kulcs nélkül/örökölt kulccsal és a `HUHS_OPENAI_API_KEY` konstans ágban); `node tools/run-php-plugin-tests.mjs` → **MINDEN ELLENŐRZÉS RENDBEN** (48 PHP fájl `php -l`, 45/45, 45/45, push-lánc 12/12); **mutációs bizonyíték 14/14** bájtpontos visszaállítással (`tmp/mutation-proof-php-plugin.mjs`); **`node tools/verify-plugin-package.mjs` (ÚJ eszköz, önteszt 7/7)** → a ZIP gyökere, **fájllistája és bájtjai azonosak a forrással** + 21 tartalmi ellenőrzés; `node tools/check-plugin-encoding.mjs` → 49/49 fájl érvényes UTF-8; `node --test functions/post-translation-meta.test.cjs` → **26/26**; `node tools/run-function-tests.mjs` → **„MINDEN TESZT RENDBEN."** (tiszta **271/271**, emulátoros **9 suite 111/111**).
- **MIÉRT LÁTSZIK EZ A FELÜLETEN (mérve a kódban):** az `Artist.location` és az `Organizer.location` a `[city, country].join(', ')` (artist.dart:124, organizer.dart:80), és **meg is jelenik** (artist_detail 300, artists_screen 255, organizer_detail 217, organizers_screen 267) — vagyis angol módban eddig **„Budapest, Magyarország"** állt ott; az eseménykártya a `venueCity`-t mutatja (event_card:139). Az `AppStrings.tr('Magyarország')` a beküldő űrlapokon már szótári kulcs volt, a **szerverből jövő** érték viszont nem — ezt zárja be a névtár.
- **⚠️ KÉT MÉRÉSI TANULSÁG (mindkettő a saját eszközeim hibája volt):** (1) a **költségvetés-teszt** a fordítás **után** mért (üres várólistán), ezért a kapu elvétele nem látszott — „nem kapta el" (a mutációs bizonyíték fogta meg; a teszt azóta a várólista **meglétekor** mér); (2) a csomag-ellenőrző tiltó keresése a **megjegyzésben** szereplő `wp_update_post` szövegre is bukott — a tiltást a **kódra** kell mérni (`stripPhpComments()`).
- **CSOMAG:** **`build/huhs-mobile-api-2.13.0.zip`** — **49 fájl**, **173 198 bájt**, SHA-256 **`357D496D16E95D8605989B154637B23BA24F16597120D3872191CC0D2D4D0585`**; patch: **`docs/plugin-2.13.0-translation-sweep.patch`** (7 fájl, **+672/−18**, SHA-256 `49681F445874FC017E5068F71E6E40CA8E83340613A59594CE38F6AC380B2CE7`, a **kiadott 2.12.0** csomagból kibontott bázishoz mérve — `tmp/plugin-212/`).
- **📌 A TULAJDONOS LÉPÉSE:** a **2.13.0** feltöltése (a 2.12.0-t cseréli). Utána a hiányzó angol **magától** pótlódik (óránként 3/típus a WP-cron körben), vagy egy paranccsal felgyorsítható a `POST /huhs/v1/translations/sweep` végponttal (`limit` 20-ig). A **Label/kiadvány** tartalmában ma **nincs fordítható szöveg** — a típus a pótlásban benne van, ezért ha később leírás kerül a kiadványba, az is automatikusan angol lesz (a payloadban való megjelenítéséhez app-kör kell). **A tulajdonos döntése: a Label marad úgy, ahogy van.**
- **✅ A 2.13.0 FELTÖLTÉS UTÁNI ÉLES MÉRÉS (2026-09-25, minden mérve):**
  - **A végpont él:** `GET /huhs/v1/translations/status` → `enabled: true`, szolgáltató **OpenAI gpt-4o-mini** (az örökölt kulcs), várólista a feltöltés után: `{post: 200 (a 200-as plafonnal), huhs_event: 6}`.
  - **A hely-névtár ÉL:** DJ/szervező/esemény `?lang=en` → **`Budapest / Hungary`, `Vienna / Austria`, `Hungary`**; `?lang=hu` → **`Budapest / Magyarország`, `Bécs / Ausztria`** (a magyar ág bájtazonos — ez a lényeg). Az `Artist.location`/`Organizer.location` ezt **meg is jeleníti** (artist.dart:124, organizer.dart:80).
  - **A 6 régi esemény pótolva:** célzott hívással (`--type=huhs_event`, 2 kör) → `?lang=en&include_past=1` **9/9 angol** (`?lang=hu` **0/9** angol, a magyar változat változatlan). A nevek (`Hard Lake - Gárdony`) helyesen **nem** fordultak le.
  - **Egy gép-fordítási hiba javítva:** a #12342 esemény magyar címe `(MÁSNAP MUNKASZÜNETI NAP)` volt, a gép `(MONDAY WORKING DAY)`-t írt — **ellenkező értelmű**. A `_huhs_title_en` metát kézzel javítottam `(NEXT DAY IS A PUBLIC HOLIDAY)`-ra (`tmp/fix-one-event-title-en.mjs`, írás + visszaolvasás **ok**); a magyar oldalhoz nem nyúltam, és a cron **nem** írja felül (a tartalom ugyanaz → az ujjlenyomat egyezik).
  - **A cikk-hátralék valódi mérete (mérve, nem tippelve):** a `post` típusban **346 publikált** cikk van, de a pótlásra váró (nem üres törzsű, angol nélküli) elem **272** — a maradék **üres törzsű** (galéria-/kép-bejegyzések, pl. Partyképek), azokat a szűrő helyesen kihagyja. A 200-as **plafon** miatt a `status` végpont először „200"-at mutat, és csak akkor kezd csökkenni, amikor a valódi szám a plafon alá ér.
  - **ÚJ ESZKÖZ: `tools/run-content-translation-sweep.mjs` (önteszt 6/6)** — a pótlás futtatása a plugin végpontján (`--status`, `--limit`, `--budget`, `--rounds`, `--type`), a WordPress-jelszóval a Secret Managerből. Nem fordít és nem ír metát: **csak indít és mér**. Mért átbocsátás: **~3 elem / 28 s** (a `budget` a PHP futásidő-korlát miatt 25 s), tehát ~6-7 cikk/perc.
  - **⚠️ MÉRT GYENGESÉG A PÓTLÓ KÖRBEN (a 2.14.0-ban javítandó):** a kör a típusok **rögzített sorrendjében** halad (`post` az első), ezért egy nagy cikk-hátralék **kiéhezteti** a kisebb típusokat — a 6 esemény csak célzott `type=huhs_event` hívással került be. A javítás: futásonként **típusonként legalább 1 elem**, és csak utána a maradék sorrend; vagy a típusok **növekvő várólista** szerinti sorrendje.
- **⚠️ ŐSZINTE KORLÁT:** a pótlás a **következő órás WP-cron körben** indul (a WP-cron látogatásra fut: forgalmas oldalon gyors, csendesnél később), vagy azonnal a végpont hívásakor. A pótlás **nem** fordítja újra a megváltozott magyar szöveget (azt a mentés teszi) — szándékos, hogy ne költsön feleslegesen. A hely-névtár **csak a biztos** neveket tartalmazza; a `venue_name` és a dalcímek **tulajdonnevek** maradnak. A 2.13.0 **éles viselkedését** a feltöltés után mértem (status → sweep → `?lang=en`); a **fordítás minősége gépi** — egy félrefordítást találtam és javítottam, a többit érdemes a tulajdonosnak néznie.
- **✅ A PÓTLÁS LEFUTOTT (2026-09-25, éles, mérve):** három adagban (**60 + 60 + 10 kör**, `tools/run-content-translation-sweep.mjs`, ~2,5 óra, **0 hiba** a 3 átmeneti szolgáltató-hibán kívül) → **`MINDEN HIÁNYZÓ ANGOL PÓTOLVA`** (várólista **0** minden típusra). **A mért eredmény:** a **346 publikált cikkből 343-nak van angolja** (a 3 kimaradó a mérés szerint **átmenetileg hibázott** — a `hiba 1` számláló a 8., 24. és 52. körben —, ezért a 6 órás késleltetés után a cron **magától újrapróbálja**), a **9 esemény mind angolul** (a 6 múltbelivel együtt), a **12 DJ** és **3 szervező** (amelyiknek van magyar szövege) szintén. A nyilvános hírlista **50/50 angolul** (`?lang=en`), a magyar ág **0/50 angol** (nincs beszólás), a weboldal HTML-jében továbbra sincs angol. A 3 **piszkozat** szándékosan marad angol nélkül (a pótlás csak publikált tartalomra megy).
- **✅ A TULAJDONOS DÖNTÉSEI (kérdező ablak, 2026-09-25):** (1) a pótlás **mindent** lefordíthat, a **régi cikkeket is** (kb. 90 elem; a kulcs költsége ~0,1–0,2 USD) — ez a `huhs_translation_sweep()` alapértéke, nem kell hozzá állítás; (2) a hírek **kategória-/címke-neveit** is angolosítani akarja → **következő kör (2.14.0)**.
- **📄 A KÖVETKEZŐ KÖR — a hírek taxonómia-nevei:** a **terv megváltozott** a mérés után: nem plugin-oldali, hanem **app-oldali szótár** (lásd a legfelső szakaszt). **MIÉRT:** a kategória-nevek a `/wp/v2/categories` válaszból jönnek, amit az app **egyszer** és **nyelvfüggetlenül** cache-el — egy plugin-oldali `lang`-fordítás a cache miatt a magyar felületet is elrontaná. A címkék (`post_tag`) pedig **felhasználói** halmaz (600 db), ezért a tulajdonos döntése szerint csak a **magyar köznevek** kapnak fordítást.

### AZ ÉRTESÍTÉSEK IS A VÁLASZTOTT NYELVEN + AZ ANGOL FELÜLET TELJES — AAB **364** (2026-09-25)

- **A TULAJDONOS KÉRÉSE:** *„maradék 132 szöveg + az értesítések is"* — vagyis (1) a `context` nélküli helyek bekötése és (2) az értesítések angolul. Mindkettő kész, **AAB 364**-ben.
- **1) A MARADÉK BEKÖTVE (147 hely) — KÉT eszközzel, mérve:**
  - Új eszköz **`tools/wire-skipped-sites.mjs`** (önteszt **10/10**): a kihagyott helyeket `AppStrings.tr(/*i18n-skip*/…)`-tal köti be, majd az **analyzer** mondja meg, melyik áll **konstans kifejezésben** (`const_eval_method_invocation` stb.) — azokat **visszaállítja**, és a visszaállítottakhoz a fordítás a **megjelenítés helyén** történik. Mérve: **101 hely bekötve**, **46 visszaállítva** (4 fájl: `community_screen` 16, `wordpress_admin_screen` 14, `achievement_guide_screen` 6, `spotify_playlists_screen` 10).
  - ⚠️ **A VISSZAÁLLÍTÁS POZÍCIÓ-KEZELÉSE** (ez a kör tanulsága): a beszúrt hívások **eltolják** a pozíciókat, ezért minden bekötés **egyedi jelölőt** kap (`/*i18n-skip*/`), és az analyzer hibájából a **legközelebbi jelölőig** lépünk vissza — nem számolunk pozíciót. A hívás végét **zárójel-számlálással** (string-literálok átugrásával) kell megtalálni: a literál maga is tartalmazhat `)`-t (mérve: `'Ez (zárójel) van benne'`).
  - A **megjelenítés helyén** fordítunk ott, ahol a szöveg `const` térképben/listában él: `AppText(entry.value)` (ChoiceChip), `AppText(option.$2)`, `AppStrings.tr(rule)`, `AppText(playlist.$1/$2)`, `AppStrings.tr(_roleLabel(...))` és `trArgs(context, 'Szerkesztés: {n}', {'n': AppStrings.tr(...)})` (a `{n}` sablon **új szótár-kulcs**: „Editing: {n}").
  - **A szabályok pontosítva** (`tools/lib/i18n-targets.mjs`): a **kereső-/hibahívások** argumentuma **nem** felirat — `contains|startsWith|endsWith|indexOf|lastIndexOf|replaceAll|replaceFirst|split|allMatches|RegExp|debugPrint|print|StateError|Exception|ArgumentError|AssertionError|FormatException|Error` (`NON_UI_CALLS`). Mérve **5 hamis pozitív** esett ki (`startsWith('Unknown User ')`, `contains('felhasználónév már foglalt')`, `throw StateError('Profile unlock cancelled')`).
  - ⚠️ **A `looksTechnical` MÉRÉSI HIBÁJA:** a **prózai** szöveget nem szabad technikainak hinni — az adatkezelési bekezdés `@`-ot (e-mail) tartalmaz, ezért a szűrő kidobta. A javítás: **3+ szavas** szöveg soha nem technikai (az azonosítók és útvonalak nem tartalmaznak szóközt).
- **2) AZ ESCAPE-ELT (TÖBBSOROS) BEKEZDÉSEK — valódi, mért rés, javítva:** a `'Hatályos: …\n\nAdatkezelő: …'` típusú szövegek **ki sem kerültek** a célok közül (a nyers alakban `\n` áll, a futásidejű szövegben valódi új sor). Ezért az **adatkezelési tájékoztató**, a rádió jogi szövege és a súgók **angol módban magyarul maradtak**. A javítás: a **kulcs az escape-ek feloldásával** készül (`unescapeDart`), a forrásban a nyers literál marad; **12 új kulcs** fordítva (a jogi szövegek hű fordítása — a tulajdonosnak érdemes átnéznie). A szótárból **10 szemét kulcs törölve** (a saját extraktorom korábbi hibájából: regex-minta és `] ?? option[` típusú interpolációs töredék).
- **3) AZ ÉRTESÍTÉSEK NYELVE — új tiszta modul + szerveroldali bekötés:**
  - **`functions/notification-texts.js`** (ÚJ): `NOTIFICATION_LANGUAGES`, `normalizeNotificationLanguage` (ismeretlen/hiányzó → **magyar**), `TEXTS` (**27 kind**, mind `hu`+`en`), `achievementReasonKey` (a **14** pontforrás-ág), `achievementReasonText`, `notificationText(kind, language, params)`. A `{helyőrző}` hiányzó értékre a **nyelvfüggő alapértéket** kapja (`name`: „Egy HUHS tag" / „A HUHS member") — eddig ez a tartalék **magyarul volt beégetve a hívókba**, ezért az angol szövegbe magyar név került volna. A `reasonKey` paraméter **beágyazott kulcsot** old fel (a pont-indoklás így nem kényszeríti a hívót nyelv-választásra).
  - **`createNotification`** mostantól `kind` + `params`-ot is elfogad: a szöveget a **címzett nyelvén** oldja fel (`recipientLanguage(uid)` a `community_profiles/{uid}.language`-ből, **futásonként egyszer** olvasva, best-effort). A kész `title`/`body` átadása **változatlanul működik** (néhány hívó dinamikus szöveget ad). A **push** ugyanazt a szöveget kapja (`notificationTextFor`), ezért a lista és a push nem tud széthúzni.
  - **Bekötve:** cikk-komment + válasz, Chat-lájk/-válasz, `@`-megemlítés, `@mindenki`, esemény-értékelés kérés, új tartalom (5 végpont), nyeremény-nyertes, ismerősnek jelölés, Meetup-érdeklődés, chatjelentés (a **több admin** miatt a push **nyelvenként csoportosítva** megy) és a **pont-értesítés**.
  - **Kliens:** új `lib/providers/profile_language_provider.dart` + `CommunityService.saveProfileLanguage` — a nyelv a **nyelvváltáskor és bejelentkezéskor** a profilba kerül (best-effort, nem blokkol). A **`firestore.rules`** `community_profiles` create/update whitelistje megkapta a `language` mezőt, érték-ellenőrzéssel (`in ['hu','en']`).
- **BIZONYÍTÉK (mind mérve):** `flutter analyze lib test` → **No issues found!**; `flutter test` → **1045/1045**; `node tools/run-function-tests.mjs` → **„MINDEN TESZT RENDBEN."**: tiszta **269/269** (ebből az új `notification-texts.test.cjs` **9/9**), emulátoros **9 suite 111/111** (a `rules.test.cjs` **24/24** — **+3** új teszt a `language` mezőre: írható `hu`/`en`, elutasítja a `de`-t, és **más** profiljáét nem lehet átírni); `node tools/check-i18n.mjs --strict` → **MINDEN ELLENŐRZÉS RENDBEN**, lefedettség **900/900 (100%)**, szótár **934 kulcs**; `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN** (1. blokk **332/500**, 1b. **429/500**, 42 build-bejegyzés, a hash valós).
- **MUTÁCIÓS BIZONYÍTÉKOK (mind bájtpontos visszaállítással):**
  - **A nyelvi katalógus** (`tmp/mutation-proof-notification-texts.mjs`, **4/4**): a nyelvfelismerés elvétele → **7 bukó**; egy angol szöveg visszaállítása magyarra → **1 bukó**; a `{name}` alapérték elvétele → **4 bukó**; a `chat_mention` kind törlése → **4 bukó**; visszaállítás után **49/49 zöld**.
  - **A mérés vakság-őrszemei** (`tmp/mutation-proof-i18n-blindness.mjs`, **2/2**): `isUiLayerFile → false` → célszám **795**, a kapu **elbukik és jelzi a vakságot**; az `AppStrings.tr(` ismeret elvétele → célszám **884**, az **önteszt** bukik. ⚠️ **A kulcslista is visszaáll** — a kapu a `tmp/i18n/keys.json`-t olvassa, ezért a szabály mutálása **önmagában nem elég** (ezt a saját mérésem buktatta meg először: „892 → 892", mert a kulcslista a régi volt). Ezért lett **elavult-kulcslista őrszem** is a kapuban.
  - **A második kör forrás-lintje** (`tmp/mutation-proof-i18n-round2.mjs`, **2/2**): `AppStrings.tr( → AppStrings.tx(` **11 fájlban** → bukik; a szótár **200 kulcsos csonkítása** → bukik.
- **CSOMAG:** **`build/HUHS-v1.0.0+364-release.aab`** — versionCode **364** (merge-elt manifest), **85 635 619 bájt** (81,67 MB), SHA-256 **`39DC24F3F82D9916E155D2975A66EC6E2BD179C2395DDA6A0A1DD73EBA30E368`**; production AdMob App ID bent, teszt ID nem; aláírás bent; **szótár 934 kulcs** (benne az új, új sorokat tartalmazó bekezdés-kulcs és a `Szerkesztés: {n}`), és **mindhárom ABI** `libapp.so`-jában megvan az `AppText`, `AppStrings`, `profileLanguageSyncProvider`, `saveProfileLanguage` szimbólum, valamint a 364 changelog-sora.
- **TELEPÍTVE (élő):** `npx firebase deploy --only functions,firestore:rules` → **„Deploy complete!"**, minden függvény „Successful update operation" — vagyis a **többnyelvű értesítések élesek**, és a `language` mező írása a szabályokkal együtt él.
- **⚠️ MIÉRT 364:** a Play-en a feltöltött AAB-ek listája **360-nál végződik** (production **358**, zárt teszt **360**, beta 354) — a **361, 362 és 363 elkészült, de egyik sem került fel**, ezért azokat **ne** töltsd fel; a 364 mindegyiket tartalmazza.
- **⚠️ ŐSZINTE KORLÁT:** aki **még nem nyitotta meg** a 364-es appot (nincs `language` mezője a profiljában), az **magyar** értesítést kap — az első indítás/nyelvváltás után ez magától helyreáll. A **jogi szövegek** fordítása gépi (a tulajdonosnak érdemes átnéznie). A **tartalom** (esemény/DJ/szervező/kiadvány) angol mezői továbbra is a plugin **2.12.0** feltöltésére várnak, és az automatikus fordítás (WP-cron) **API-kulcsot** igényel. Az értesítések **éles viselkedését** (nyelvhelyesség, push) eszközön még **nem mértem** — a következő lépés a tulajdonos feltöltése a zárt tesztre és egy angol nyelvű kör a készüléken.

### AZ ANGOL FELÜLET MÁSODIK KÖRE **BEKÖTVE** — AAB **363** (2026-09-25)

- **A TULAJDONOS KÉRÉSE:** *„fent a 2.11.0 és így mindent megkéne csinálni"* — vagyis az angol nyelv **minden** része készüljön el. Az első kör (AAB 361/362) a **szabályalapú** helyeket fordította; ez a kör a **mért maradék** bekötése.
- **AMIT EZ A KÖR MEGMÉRT ÉS BEKÖTÖTT (nem tippelve):**
  - **327 új bekötés, 44 fájlban** (a `tools/wrap-ui-strings.mjs` hatókör-felismerő körével): feltételes ágak (`cond ? 'A' : 'B'`), lista-/térkép-értékek, `??` alapértékek, állapotüzenetek. Az `AppText`-csere itt **0** volt (azt az első kör már elvégezte) — az új helyek mind `tr(...)`/`AppStrings.tr(...)` alakot kaptak.
  - **A bekötött helyek száma mérve:** `AppText(` **596**, `tr(context,` **458**, `trArgs(context,` **10**, `AppStrings.tr(` **59**, `AppStrings.trArgs(` **1** (a 783-as első kör után).
  - **A látható felület maradéka mérve:** a „se be nem kötött, se le nem fordított” magyar szövegek **983 → 618** (ebből `screens`+`widgets` **538 → 192**, `services` 210, `data`/changelog 105).
- **A HIBAOSZTÁLY, AMI EBBEN A KÖRBEN IS ELKAPOTT (ez a lényeg):** a bekötés új alakot hozott (`AppStrings.tr(…)` a `context` nélküli helyeken), és **a mérőeszköz ezt nem ismerte fel** → az így bekötött szövegek **kiestek a célok közül**, a lefedettség pedig **zölden hazudott volna** (ugyanaz a hibaosztály, mint a körbefordításnál). **A mérés pontos:** e forma nélkül a célszám **892 → 884** (8 kulcs tűnt volna el észrevétlenül), az `isUiLayerFile` kikapcsolásával pedig **892 → 795**. A javítás: az extraktor **minden** bekötött alakot ismer (`tr(context, …)`, `AppStrings.tr/trArgs(…)`, `AppText(…)`), **és** az interpolált sorokban (`'${tr(context, 'Nyelv')}: …'`) is gyűjti a bekötött kulcsokat — a `stringLiterals` ugyanis a beágyazott idézőjel miatt **egyetlen literál-párba sem** teszi bele a `Nyelv`-et.
- **ÚJ ŐRSZEMEK (mindegyik mutációval igazolva, bájtpontos visszaállítással):**
  - `tools/check-i18n.mjs`: **`MIN_TARGETS = 880` padló** — ha az extraktor megvakul, a kapu **elbukik** a célszám miatt, nem azért, mert hiányzó fordítást talált. **Mutáció:** `isUiLayerFile → false` → célszám **795**, a kapu **elbukik és jelzi a vakságot**; bájtpontos visszaállítás után újra zöld.
  - `tools/check-i18n.mjs`: **elavult-kulcslista őrszem** — a kapu a `tmp/i18n/keys.json`-t olvassa, ezért ha a kód **újabb**, mint a kulcslista, a mérés elavult lehet (a mutációs bizonyíték közben **pont ez** buktatta meg a mérést: a szabály mutálása nem látszott, mert a kulcslista a régi volt). `--strict`-ben **hiba**, a javítás egy sor: `node tools/extract-ui-strings.mjs --write`.
  - `tools/extract-ui-strings.mjs` öntesztje **21/21**: új esetek az `AppStrings.tr(…)` kulcsra, az interpolált sorban lévő bekötött kulcsra, és arra, hogy a bekötetlen interpolált szöveg **nem** cél.
  - `test/services/i18n_wiring_test.dart`: új forrás-lint a második körre (`AppStrings.tr` > 40, `trArgs` ≥ 1, > 5 fájlban) **és** a szótár-csonkulás ellen (`dictionarySize > 800`; mért 921). **Mutáció:** `AppStrings.tr( → AppStrings.tx(` **11 fájlban** → a lint elbukik; a szótár **200 kulcsos csonkítása** → a küszöb elbukik; mindkét visszaállítás **bájtpontos**, utána zöld.
- **TOOLING-VÁLTOZÁSOK (mind saját önteszttel):** `tools/fix-context-fallout.mjs` (**10/10**, új fájl a repóban) — a `context`-hibák mellett a **`use_build_context_synchronously`** infókat is kezeli (12 hely: `await` utáni `tr(context, …)` → `AppStrings.tr(…)`), és **eltávolítja a feleslegessé vált `tr.dart` importot** (mért hiba: a `AppStrings.tr(` is illeszkedett a `\btr\(`-re, ezért az első változat **semmit** nem távolított el; a fájlok egy része **CRLF**, ezért a sor-egyezés `trim()`-mel megy). `tools/fix-const-fallout.mjs` (**8/8**) — a deklarációs `const` → `final`, a kimaradt `final` beszúrása, és a két javítás **egy menetben, jobbról balra** (különben az első írás eltolja a második pozícióit).
- **A SZÓTÁR RENDBEZÉSE:** a saját extraktorom korábbi hibájából a szótárban **10 szemét kulcs** maradt (regex-minta és `] ?? option[` típusú interpolációs töredék) — **törölve**; a szótár **931 → 921 kulcs**, a lefedettség **892/892 (100%)**, a „szótáron kívüli kulcs” 41 → 29 (a maradék **valódi**: fűzött töredékek és `lib/core/**`-ban élő, de a UI-szabályon kívüli szövegek, pl. a nyelvnevek).
- **BIZONYÍTÉK (mind mérve):** `flutter analyze lib test` → **No issues found!** (a kör közben **402 → 99 → 46 → 1 → 0** hiba, mind a saját eszközökkel); `flutter test` → **1045/1045** (a kör előtt 1044); `node tools/check-i18n.mjs --strict` → **MINDEN ELLENŐRZÉS RENDBEN**, exit 0; a hat eszköz öntesztje **21/21, 18/18, 12/12, 7/7, 8/8, 10/10**.
- **CSOMAG:** **`build/HUHS-v1.0.0+363-release.aab`** — versionCode **363** (merge-elt manifest: `build/app/intermediates/merged_manifests/release/processReleaseManifest/`), **85 622 263 bájt** (81,66 MB), SHA-256 **`47AD8FD9EE4AA534ED9EF1AB068C31667D69C87FFA92EEE99734818996C559F9`**; a **szótár (921 kulcs)** benne van, a **363 changelog-sor** és a második kör kulcsai (`NYEREMÉNYJÁTÉK`, a megerősítő e-mail mondatai, `Mindenki értesítést kap`) **mindhárom ABI** `libapp.so`-jában megvannak; production AdMob App ID bent, teszt ID nem; **1 aláírási identitás** (62 csomag). `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN** (1. blokk **358/500**, 1b. blokk **429/500**, a dokumentumban szereplő AAB-hash **valós**, 41 build-bejegyzés).
- **⚠️ MIÉRT 363 (mérve, nem tippelve):** a Play-en a feltöltött AAB-ek listája **360-nál végződik** (production **358**, zárt teszt **360**, beta 354) — a **361 és a 362 elkészült, de nem került fel**, ezért azokat **nem kell feltölteni**; a 363 mindkettőt tartalmazza. A `pubspec.yaml` `1.0.0+363`, a `docs/PLAY-KIADASI-JEGYZET.md` fejléce/meta/1./1b./3. blokkja a **363-ra** állítva, és a Névjegy (`lib/data/app_changelog.dart`) is megkapta a 363 bejegyzését (a Play-szöveggel egyezően, **a build ELŐTT**, mert a changelog a csomagba fordul).
- **⚠️ ŐSZINTE KORLÁT:** **132 egyedi szöveg (157 hely)** továbbra sem angolul jelenik meg, mert a bekötés `BuildContext`-et kíván, és az adott helyen (osztály-szintű adatlista, statikus tábla) nincs — a **fordításuk kész**, csak a megjelenítésnél kell fordítani (**ez a következő kör**; a legnagyobb érintettek: `community_screen` 41, `achievement_guide_screen` 31, `wordpress_admin_screen` 17). Az **értesítések** szerveroldali szövegei továbbra is magyarul mennek. Az **esemény/DJ/szervező/kiadvány** angol mezői a plugin **2.12.0** feltöltésére várnak. A **mérés statikus** (kódból), nem eszközön látott szöveg: a napló-/belső szövegek egy része sosem látszik. **A 363-at eszközön még nem próbáltam** — a következő lépés a tulajdonos feltöltése a zárt tesztre és egy angol nyelvű kör a készüléken.
...
- **AMI ÉPÜLT (`.tmp-api-260/huhs-mobile-api/`, gitignore-olt forrás):**
  - `includes/post-translation-meta.php`: a három rejtett meta (`_huhs_title_en`, `_huhs_excerpt_en`, `_huhs_content_en`) regisztrációja **egy** listából (`huhs_translation_post_types()`: `post`, `huhs_event`, `huhs_artist`, `huhs_organizer`, `huhs_release`), plusz a **közös kapu** `huhs_translation_meta_values($post_id, $lang, $title_hu, $content_hu, $excerpt_hu = '')` — ugyanaz a szabály, mint a cikkeknél: angolra csak akkor váltunk, ha a **cím ÉS a törzs** is megvan, különben magyar + `has_en = false`.
  - `api-events.php`, `api-artists.php`, `api-organizers.php`, `api-releases.php`: a payload-építők **opcionális `$lang` paramétert** kaptak (a meglévő paraméterek mögé, ezért egyetlen hívó sem tört el), a végpontok `huhs_request_lang($request)`-ből adják tovább, a payload pedig `has_en`-t kap. ⚠️ **A closure-ök nem látják a `$request`-et** — a nyelvet a `array_map` ELŐTT kell kiszámolni és `use ($lang)`-gal átadni (ez az első változatban hiba volt).
  - `includes/translation-cron.php` (ÚJ): `save_post` → `wp_schedule_single_event('huhs_translate_post')` → `huhs_run_translation()`. **API-kulcs nélkül szándékosan nem csinál semmit** (`huhs_translation_enabled()`), a kulcs a `huhs_translation_api_key` opció/szűrő, a szolgáltató a `huhs_translation_provider` szűrő (alap: DeepSeek csevegő-végpont, szigorú JSON ki/be). A válasz feldolgozása (`huhs_translation_parse_response`) **hálózat nélkül** olvasható, és hibás válaszra üres mezőket ad (nem ír félkész fordítást).
- **BIZONYÍTÉK:** `node tools/run-function-tests.mjs` → **tiszta 260/260**, emulátoros **9 suite 108/108, 0 bukó** (a 2.11.0 óta **+3** teszt a `functions/post-translation-meta.test.cjs`-ben: a típuslista, a közös kapu a négy végponton, a WP-cron kulcs-kapuja és a válasz-feldolgozó). **Két mutációs bizonyíték:** (1) a kapu „csak cím" feltételre gyengítése → **1 bukó**; (2) a `huhs_translation_enabled()` `return true;`-ra cserélése → **2 bukó**; mindkét visszaállítás **bájtpontos** (SHA-256 `8F5F4361…` és `2DD1A50E…` egyezik), utána **24/24**.
- **Csomag (ÚJRAÉPÍTVE 2026-09-25-én, az örökölt kulcs javításával):** **`build/huhs-mobile-api-2.12.0.zip`** — **47 fájl**, **163 632 bájt** (159,8 KB), SHA-256 **`C39D2BDF2AF179AF89CCD22610E2F7A37209B388EBC9541536B14FD6C500EF9B`** (a 2.11.0 46 fájlja + a `translation-cron.php`); a patch: **`docs/plugin-2.12.0-english-fields.patch`** (7 fájl, **+453/−49** sor, SHA-256 `BC594D58D2065DF37C7A0849E58031027EE264E82402587540A159CF16410208`, a **kiadott 2.11.0 ZIP-ből** kibontott bázishoz mérve, nem emlékezetből). ⚠️ **A korábbi `7AAD9A34…` hash és a 167 005 bájtos méret ELAVULT** (a régi csomagot felülírta az újraépítés — semmi nem került fel belőle, ezért nincs kockázat); a kibontott csomag **fájllistája bájtra egyezik a forrással** (47/47, nincs hiányzó és nincs plusz), és a tartalma ellenőrizve (`tmp/verify-plugin-212-zip.mjs`, **12/12**).
- **⚠️ ÖRÖKÖLT API-KULCS (2026-09-25) — a WP-cron nem vár új kulcsra:** ha a `huhs_translation_api_key` üres, de a **korábbi OpenAI-próba** kulcsa megvan (`huhs_openai_api_key` opció vagy `HUHS_OPENAI_API_KEY` konstans — ezt használta a régi, **be nem töltött** `includes/translations.php`), akkor a cron **azt** használja, és a szolgáltató automatikusan az **OpenAI** csevegő-végpontja lesz (`gpt-4o-mini`); az elsődleges kulcs mindig nyer. Új teszt őrzi (`functions/post-translation-meta.test.cjs`, **25/25** — benne a sorrend, a végpont-váltás és az, hogy **kulcs nincs a forrásban**). **Mérve:** a `huhs/v1` élő útvonallistájában **nincs** `/huhs/v1/translations` (48 útvonal), tehát a régi futásidejű fordítás élesben **nem él** — a `translations.php` halott kód, nem is töltődik be.
- **📌 ÉLŐ ÁLLAPOTMÉRÉS EGY PARANCCSAL (ÚJ eszköz, 2026-09-25): `node tools/check-content-english-live.mjs`** (önteszt **8/8**, csak olvas). Hármat mér: (1) **plugin-támogatás** — a `context=edit` válasz `meta` kulcsai között ott van-e a `_huhs_title_en` típusonként; (2) a **nyilvános végpontok** `?lang=hu`/`?lang=en` `has_en` száma; (3) a **weboldal** — az angol cím/kivonat megjelenik-e a nyilvános HTML-ben. **A friss mérés eredménye:** `post` **TÁMOGATOTT** (14 meta-kulcs, 3 `_huhs_*`), `huhs_event`/`huhs_artist`/`huhs_organizer` **nem támogatott** (0 meta-kulcs) → **a 2.12.0 nincs fent**; cikkek `?lang=en` → **30/50** angol (`?lang=hu` → **0/50**), esemény `0/3`, DJ `0/17`, szervező `0/4`; a weboldal HTML-jében az angol cím és kivonat **nincs benne** (a „nem jelenik meg a weboldalon" követelmény **élőben igazolva**).
- **⚠️ A PATCH ELLENŐRZÉSE (mért, 2026-09-25):** a `docs/plugin-2.12.0-english-fields.patch` **tisztán alkalmazható** a kiadott 2.11.0 fára (`git apply --check` → **exit 0**), és az alkalmazás eredménye **41/47 fájlban bájtazonos** a 2.12.0 forrással, a maradék **6 fájl pedig LF-normalizálás után azonos** (a különbség a git `core.autocrlf=true`-ja, nem tartalom). ⚠️ **MÓDSZERTANI TANULSÁG:** a `git apply` a **repo gyökeréhez** képest oldja fel az útvonalakat (nem a cwd-hez), ezért a próbát `--directory=tmp/patchcheck`-kel kell futtatni — a `tmp/patchcheck`-be `cd`-zve a git „Skipped patch"/„0 files changed" lett, ami **hamis negatív** volt (a patch végig helyes).
- **✅ A PHP MOSTANTÓL **FUT** — a 2.12.0 valódi PHP-teszttel ellenőrizve (2026-09-25; a korábbi „nem futtatható" korlát ezzel megszűnt):** új eszköz **`tools/run-php-plugin-tests.mjs`** (önteszt **9/9**) + **`tools/php/plugin-translation-test.php`** + **`tools/php/run-plugin-tests.sh`**. A Docker (`php:8.2-cli`) konténerben fut, és **a kibontott ZIP-et** méri (nem a forráskönyvtárat), négy szakaszban: (1) **46 PHP fájl `php -l`** szintaxisa; (2) **19 viselkedés-ellenőrzés** WordPress-stubokkal; (3) ugyanaz a **`HUHS_OPENAI_API_KEY` konstans** ágban (külön futás, mert a konstans nem definiálható kétszer); (4) a **meglévő** `tools/verify-push-dedupe.php` (**12/12**) — ez eddig azért nem futott, mert nem volt PHP. **Eredmény: `MINDEN ELLENŐRZÉS RENDBEN — lintelt fájl: 46, viselkedés-körök: 19/19, 19/19, push-lánc: 12/12`.** Amit a stubolt WordPress-függvényekkel **tényleg lefuttat**: a típuslista (mind az 5), a **fallback-kapu** (`hu` → magyar meta-olvasás nélkül; `en` + cím ÉS törzs → angol; csak cím vagy csak törzs → **marad a magyar**), a kulcs-ágak (nincs kulcs → nincs hívás/írás; örökölt opció- és konstans-kulcs → OpenAI végpont; elsődleges kulcs nyer → DeepSeek), a **teljes fordítás** (érvényes válaszból a három helyes meta; a címből a tag kimegy, a HTML marad), a hibaágak (HTTP 500 és értelmezhetetlen JSON → **nincs meta-írás**, nincs félkész fordítás), a részleges válasz, a rossz post-típus, az üres cím, és a `save_post`/`huhs_translate_post` hookok. Docker nélkül **KIHAGYVA** (indoklással), `--strict`-tel **hiba** — hogy a zöld ne csússzon el. **Mérve:** a Docker Desktop indítása után a démon **~6 s alatt** elérhető volt, a `php:8.2-cli` kép letöltése egyszeri.
- **MUTÁCIÓS BIZONYÍTÉK A PHP-TESZTHEZ (`tmp/mutation-proof-php-plugin.mjs`, 4/4, bájtpontos visszaállítás a ZIP-ből újrakibontva):** a fallback-kapu gyengítése → **1 bukó**; **mindkét** kulcs-kapu kivétele → **1 bukó**; az örökölt kulcs felismerésének elvétele → **4 bukó**; a `_huhs_content_en` írás elhagyása → **1 bukó**; utána újra **19/19 zöld**. ⚠️ **KÉT MÉRÉSI TANULSÁG:** (1) a két kulcs-kapu (a cron külső kapuja és a kérés belső ellenőrzése) **szándékosan redundáns** — bármelyik önmagában **maszkolt** (ekvivalens mutáns), ezért csak a kettő együtt változtat viselkedést; (2) a **string-csere mutációt ankerezni kell**: a fájlban **két** ugyanilyen kapu van, és a naiv `replace` az elsőt (a `save_post` ágban) módosította → a teszt „nem kapta el" eredményt adott, ami **hamis negatív** volt. Emellett a valódi futás talált egy hiányzó stubot is (`wp_strip_all_tags`), amit a forrás-lint természetesen nem láthatott.
- **🔴 ÉLES HIBA A 2.12.0-BAN, JAVÍTVA — a REST-en küldött angol meta CSENDBEN elveszett (mérve 2026-09-25, a tulajdonos első feltöltése után):** a `/wp/v2/huhs_event|huhs_artist|huhs_organizer|huhs_release` válaszában a **`meta` mező egyáltalán nem volt benne** (0 kulcs), miközben a `post`-nál **14 kulcs, ebből 3 `_huhs_*`** volt — vagyis a `POST {meta:{…}}` **200-at adott, de a visszaolvasás üres lett** (a `tools/write-content-translations.mjs` ezt **kimérte**: `Írás: 0 elem, hiba 18`). **A gyökér:** a WordPress a `meta` mezőt csak akkor teszi a REST-válaszba (és csak akkor fogadja írásra), ha a post-típus támogatja a **`custom-fields`**-et — a 2.12.0 `register_post_meta(..., show_in_rest => true)`-ja ehhez **nem volt elég** ezeknél a típusoknál. **A javítás:** `huhs_enable_translation_meta_custom_fields()` + `add_post_type_support($post_type, 'custom-fields')` a **késői `init`-en (99)**, `post_type_exists` őrrel. A szerveroldali utakra (a WP-cron `update_post_meta`-ja és a végpontok `get_post_meta`-ja) a hiba **nem** hatott — csak a kívülről (REST) írt fordításokra. **Új ZIP:** `build/huhs-mobile-api-2.12.0.zip` — 47 fájl, **164 151 bájt**, SHA-256 **`FABC459B3BA81F96D3421633AF102284704A06FA132C1F21C6D1EA82F31B768A`**; a patch 7 fájl, **+483/−51**, SHA-256 `54D09AB3B76076B4274866BE0777E7CB359EE94C150A0FE298EBFDE3C88B16A7`. **Migráció:** a 2.12.0-t **még egyszer fel kell tölteni** (a régi csomag `C39D2BDF…` volt).
- **⚠️ KÉT SAJÁT HIBA, amit ez a kör talált (mindkettő a MI oldalunkon):** (1) a `tools/write-content-translations.mjs` a `events`/`artists`/`organizers` REST-útvonalat használta — a valódi útvonal a **post-típus neve** (`huhs_event`, …), amit a `/wp/v2/types` igazol; ebből `rest_no_route` **404** lett (a hiba csak `--confirm`-nál látszott, mert a száraz futás nem hívja az API-t). A javítás: `POST_TYPE_BY_KEY` + `restBaseFor()` a `/wp/v2/types` válaszából (önteszt **13/13**). (2) A valódi PHP-teszt (19 → **21** ellenőrzés) **nem** fedte le a WordPress REST-integrációt — ezért a hiba csak **éles méréssel** derült ki. A tanulság: a stubolt WordPress-függvényekkel futó teszt a **logikát** méri, az integrációs viselkedést (REST-séma, `custom-fields`) **élesben** kell megmérni.
- **A PHP-TESZT BŐVÜLT (21 ellenőrzés × 2 futás + push-lánc 12/12):** két új ellenőrzés a **REST-meta írás előfeltételére** (mind az öt típus megkapja a `custom-fields` támogatást, és a **késői init (99)**-en) — a mutációs bizonyíték **5/5** (a `custom-fields` elvétele → **1 bukó**, bájtpontos visszaállítással).
- **⚠️ A KIADVÁNY KIMARAD (méréssel):** a kiadvány egyetlen szöveges mezője a **cím**, ami **név** (kiadvány/szám címe) — ezt nem fordítjuk —, és a payloadban **nincs leírás** sem. Mérve (`tools/fetch-content-for-translation.mjs`): 21 kiadvány, **0 fordítható prózai szöveg**. A meta regisztrálva van (egy jövőbeli leíráshoz), de a végpont **nem** hívja a fordítást (ezt forrás-lint is őrzi: a `huhs_translation_meta_values(` nem szerepel az `api-releases.php`-ban).
- **A FORDÍTÁS ELŐKÉSZÍTVE (18 elem, 9 864 karakter):** `tools/fetch-content-for-translation.mjs` (önteszt **5/5**) begyűjti a listából + a részlet-végpontokból a magyar szöveget (esemény **3**, DJ **12**, szervező **3**; 5 DJ és 1 szervező adatlapján nincs szöveg), chunkokba osztja, és a fordítások a `tmp/content-en/en/chunk-*.json`-ba kerülnek (a HTML-szerkezetet **gépi** ellenőrzés kíséri: a tag-szekvencia bájtazonos). A beíró: `tools/write-content-translations.mjs` (önteszt **9/9**), ami a plugin 2.12.0 fentlétekor egy paranccsal feltölti a metákat.
- **📌 A TULAJDONOS LÉPÉSE:** a **2.12.0 ZIP feltöltése** a WordPressre (a 2.11.0-t cseréli). ⚠️ **Feltöltés után** lehet a meglévő **esemény/DJ/szervező/kiadvány** tartalmat angolra fordítani és beírni (ugyanaz az eszköz, mint a 30 cikknél: `tools/write-post-translations.mjs` a cikkekre; a többi típusra a meta-írás ugyanígy megy, a REST `context=edit` + alkalmazás-jelszó).
- **⚠️ ŐSZINTE KORLÁT:** a PHP **nem futtatható** ezen a gépen (`php -l` nincs), ezért a 2.12.0 viselkedése **forrás-lintekkel** és a kiadott ZIP-fel ellenőrzött, nem futással; az éles működést a feltöltés után a `?lang=en` kérések mérik (a cikkeknél ez a mérés 30/30 volt). Az automatikus fordítás **élesben még nem futott** (nincs API-kulcs). Az ágens, amit a 2.12.0-ra kértem, **hosszú idő után sem írt fájlt** (mint a körbefordításnál) — leállítottam, és magam csináltam meg.

### ANGOL NYELV AZ APPBAN — a felület kész, **AAB 361** (2026-09-25; a tartalom fordítása még hátra)

- **A TULAJDONOS DÖNTÉSE:** *„fent a 2.11.0 és így mindent megkéne csinálni"* — a plugin **2.11.0 élesben fut** (mérve: a `/huhs/v1/posts` válaszban megjelenik a `has_en` mező), és a **30 legutóbbi cikk angol változata élesben van**: `?lang=en` → **30/30 angolul** (`has_en: true`, a szöveg a fordításunkkal egyezik), `?lang=hu` → **30/30 magyarul, változatlanul**; az újrafuttatás **idempotens** (0 írás, 30 kihagyva, 0 hiba).
- **A SZÓTÁR-KAPU MÉRVE:** `tools/check-i18n.mjs` → **599/599 célzott szöveg lefordítva (100%)**, 0 hiányzó, 0 üres, 0 magyarul maradt érték, 0 helyőrző-hiba. A szótár **be van építve** az appba: `assets/i18n/en.json` (**609 kulcs**), és a **361-es AAB-ban is benne van** (`base/assets/flutter_assets/assets/i18n/en.json`).
- **A FELÜLET SZÖVEGEI BEKÖTVE — 783 hely, két biztonságos módon:**
  - **`Text('…')` → `AppText('…')` (581 hely).** Azért így, mert a `tr(context, …)` **nem konstans**, és a 783 hely **~44%-a `const` sorban** van — az `AppText` viszont a **konstruktorban hagyja a szöveget**, ezért a `const` felület **nem törik el**, a fordítás a `build`-ben történik (`lib/widgets/app_text.dart`; teljes `Text`-API, `Localizations.maybeLocaleOf` a nyelvváltás-újrarajzoláshoz).
  - **Címkék/tooltipek → `tr(context, '…')` (202 hely),** az interpolált feliratok **`trArgs(context, '… {n} …', {…})`** (9 hely, pl. `'{n}. lehetőség'` → „Option {n}").
  - **A `const`-utómunka MÉRVE, nem tippelve:** `tools/fix-const-fallout.mjs` az **analyzer hibahelyeiből** keresi meg a hozzá tartozó, még nyitott `const` kulcsszót és **csak azt** veszi ki (44 bontás, 86 → 0 hiba).
- **⚠️ HÁROM HELY, AHOL NINCS `context` — a fordítás a MEGJELENÍTÉSNÉL van:** a `achievement_service.dart` (szintek adatai), a `chat_mention_plan.dart` (tiszta modul: a `@mindenki` sor felirata) és az `achievement_guide_screen.dart` **top-level adatlistája**. Ezeket **visszaállítottam** magyar kulcsra, és a **renderelésnél** fordítok (`AppText(level.name)`, `AppText(activity.title)`, `AppText(suggestion.subtitle)`). **A tanulság: a „UI-kontextus" szabály (pl. `title: '…'`) a service-rétegen is talál, ahol nincs `context` — ott a szöveg kulcs, a fordítás a nézetben történik.**
- **AZ ESZKÖZÖK (mind saját önteszttel):** `tools/lib/i18n-targets.mjs` (a szabályok egy helyen), `tools/extract-ui-strings.mjs` (**19/19**, a kulcsokat a KÓD adja: 786 literál / **589 egyedi**), `tools/wrap-ui-strings.mjs` (**18/18**, abszolút pozíciók, mert a `Text(` gyakran **korábbi sorban** van; idempotens: a bekötött alakokon a második futás **0 változás**), `tools/fix-const-fallout.mjs` (**7/7**), `tools/merge-i18n.mjs` (**7/7**), `tools/check-i18n.mjs` (**12/12**).
- **A FORDÍTÁS MÓDJA:** 10 párhuzamos ágens, chunkonként 60 szöveg, **közös glosszáriummal** (`docs/I18N-GLOSSARY.md`), mindegyik gépi ellenőrzéssel (`missing 0 / extra 0 / empty 0`). A döntések (pl. `Elutasítás` → „Reject" gomb-feliratként, `adatlap` → „profile", `Meetup` és a márkanevek maradnak) a glosszáriumban dokumentálva.
- **⚠️ KÉT SAJÁT HIBA, amit a saját kapum fogott meg (ezért van a kapu):** (1) a körbefordítás **megváltoztatta a kontextust**, ezért az extraktor a 783 helyből csak **21-et** látott, és a lefedettség-mérés **hamis 100%-ot** mutatott — a javítás: a **már bekötött** szöveg (`tr(context, …)`, `AppText(…)`) is **kulcs**; (2) a `tools/write-post-translations.mjs`-ben a „van-e már angol" a WP **összes** meta-kulcsát nézte (a Yoast/egyéb kitöltött meta miatt **minden** cikket kihagyott volna), a visszaolvasás pedig a nyers HTML-t hasonlította a WP-normalizálthoz — mindkettő javítva, és a titok-olvasás már **egyszer** fut (60 `npx` helyett 2).
- **BIZONYÍTÉK:** `flutter analyze lib test` → **No issues found!**; `flutter test` → **1029/1029** (a kör előtt 1022; **+6** `AppText`-teszt és **+1** forrás-lint, ami a bekötés mértékét is őrzi: >450 `AppText`, >150 `tr(`); `node tools/check-i18n.mjs --strict` → **MINDEN ELLENŐRZÉS RENDBEN**, exit 0. **Mutációs bizonyíték:** a `Localizations`-feliratkozás kivétele elbuktatja a nyelvváltás-tesztet (a teszt **első változata átengedte**, mert `Builder`-t használt a `const` képernyő helyett — megerősítve), bájtpontos visszaállítás (SHA-256 `F83DFE1E…`).
- **CSOMAG:** **`build/HUHS-v1.0.0+361-release.aab`** — versionCode **361** (merge-elt manifest), **85 564 463 bájt**, SHA-256 **`5B117577B4A7A5932DD06F2EC8ABF0BC1CE84EA241E30C7418A070E90A28E671`**; a **szótár benne van** (609 kulcs), és **mindhárom ABI** `libapp.so`-jában megvan az `AppText`, `AppStrings`, `LanguageSwitchButton`, `languageProvider` szimbólum, valamint a 361 changelog-sora (UTF-16LE). `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN** (1. blokk **216/500**, 1b. blokk **323/500** karakter, 39 build-bejegyzés); `check-signing-identity.mjs` → **1 aláírási identitás**.
- **📌 A TULAJDONOS LÉPÉSE:** a **361 feltöltése a zárt teszt sávra** a `docs/PLAY-KIADASI-JEGYZET.md` **1. blokkjával**; a **nyilvános** kiadáshoz az **1b. blokk** (359–361 összesítő). **Frissen mérve:** a zárt teszten a **360** (`completed` 100%), a production sávon a **358**, a `beta`-n a **354**.
- **⚠️ AMI MÉG HÁTRA (a következő körök):** (1) a **tartalom** nyelve — az app a WordPress-tartalmat még **magyarul** kéri (a cikkek angol változata a szerveren már él): a `lang` paraméter átadása és a `has_en` szerinti választás; (2) **esemény/DJ/szervező/kiadvány** angol mezői — ehhez **plugin 2.12.0** kell (a 2.11.0 csak a cikkeket tudja), utána a meglévő tartalom fordítása; (3) **automatikus fordítás új tartalomnál** (WP-cron) — **API-kulcs kell a tulajdonostól**; (4) iOS-sideload a 361-gyel (kérésre).

### A piros node-tesztek RENDBE TÉVE — 8 emulátoros suite + App Check (2026-09-25; **app-kód NEM változott**)

- **A TULAJDONOS KÉRÉSE:** *„tedd rendbe"* — a `functions/` tesztjei közül **8 fájl** pirosan állt, mert a valódi callable-okat/Admin SDK-t mérik, és ehhez Firestore/Auth/Functions **emulátor** kell.
- **1) EMULÁTOR NÉLKÜL NE HAZUDJON PIROSAT (8 fájl):** `account-deletion`, `achievement-daily-limit`, `achievement-guided-points`, `achievement-restore`, `label-purchase-limit`, `prize-draw`, `push-dedupe`, `registration.integration` — mindegyik **emulátor-host skip-burkolót** kapott (`FIRESTORE_EMULATOR_HOST` nélkül minden teszt „kihagyva", a suite zölden lefut). ⚠️ **A hookokat is védeni kell** (`before`/`beforeEach`/`after` `guardHook`-kal), különben a `push-dedupe` `before`-ja emulátor nélkül **továbbra is hibázott**. Mérve: `node --test` a 8 fájlra → **skipped, 0 fail**; friss emulátorral fájlonként: account-deletion **6/6**, achievement-daily-limit **9/9**, achievement-guided-points **18/18**, achievement-restore **7/7**, label-purchase-limit **5/5**, prize-draw **24/24**, push-dedupe **7/7**, rules **21/21** = **97 teszt, 0 bukó**. A tiszta csoport ugyanekkor **257/257**.
- **2) A `registration.integration` 6 PIROS TESZTJE — A GYÖKÉR MEGMÉRVE, NEM A MI KÓDUNK:** a bukó hívások a `checkRegistrationEligibility` és a `checkDisplayNameAvailability` voltak, **token nélkül** hívva, és a válasz **`401 {"error":{"message":"Unauthenticated","status":"UNAUTHENTICATED"}}`** volt. A `firebase-functions` HTTP-rétege (`functions/node_modules/firebase-functions/lib/common/providers/https.js`): `const appCheckToken = req.header("X-Firebase-AppCheck"); if (!appCheckToken) return "MISSING";` … majd `if (tokenStatus.app === "MISSING" && options.enforceAppCheck) throw new HttpsError("unauthenticated", "Unauthenticated")` — a két callable pedig szerveroldalon `runWith({enforceAppCheck: true})` (`functions/index.js` 163. és 2203. sor). **Vagyis a 401 az App Check hiánya volt (emulátorban is), nem az Auth-é és nem a regisztrációs logika hibája.** Az „App Check kikapcsolva, Auth továbbra is szükséges" teszt (a `claimDisplayName` `enforceAppCheck: false`) **végig zöld volt** — ez a kontroll.
- **A JAVÍTÁS (teszt-harness, nem termékkód):** `functions/registration.integration.test.cjs` — ha `FIREBASE_DEBUG_MODE=true`, akkor a `callable()` **aláíratlan, csak dekódolt** App Check-tokent küld (`X-Firebase-AppCheck`, base64url JWT `{sub}`), a `skipTokenVerification` debug-szolgáltatás miatt ez érvényesnek számít; **debug-mód nélkül** a 6 érintett teszt **kihagyva** fut, egyértelmű indokkal („App Check-hez kötött callable — emulátorban FIREBASE_DEBUG_MODE=true kell"), tehát nem lesz belőle hamis zöld. A `tools/run-function-tests.mjs` a `registration.integration` suite-ot **saját környezettel** indítja (`env` mező a suite-listában: `FIREBASE_DEBUG_MODE=true`, `FIREBASE_DEBUG_FEATURES={"skipTokenVerification":true}`), és az önteszt erre **külön ellenőrzést** kapott.
- **BIZONYÍTÉK (mind mérve):** `node tools/run-function-tests.mjs --self-test` → **8/8**; a suite **debug-környezet nélkül** → `tests 11 | pass 5 | fail 0 | skipped 6`; **debug-környezettel** (`FIREBASE_DEBUG_MODE=true`) → **`tests 11 | pass 11 | fail 0 | skipped 0`** (ugyanaz a kód, ugyanaz a nap: 401 → a várt 200/403 válaszok, a két App Check-es callable tényleg lefutott). **A teljes futtató (`node tools/run-function-tests.mjs`) → „MINDEN TESZT RENDBEN.", exit 0:** tiszta csoport **257/257**, majd **9 emulátoros suite fájlonként külön emulátorral, 108/108, 0 bukó** (6+9+18+7+5+24+7+11+21).
- **⚠️ ŐSZINTE KORLÁT:** az emulátoros futás az App Check-t **szándékosan nem hitelesíti** (a `skipTokenVerification` pontosan ezt jelenti) — a valódi App Check-kényszerítés **élesben** mérhető, nem itt. A teszt ezért **nem** azt bizonyítja, hogy az App Check működik, hanem azt, hogy a regisztrációs logika App Check **mögött** helyesen dönt. A 6 teszt debug-mód nélkül **kihagy**, nem fut — ez tudatos, hogy a „zöld" jelentése ne csússzon el.

### KÉRDÉS ELŐRE: a Chat-`@`hivatkozás (mention) — MEGVALÓSÍTHATÓ, terv kész (2026-09-24, kutatás; **kód NEM változott**)

- **A TULAJDONOS KÉRÉSE:** *„Kéne olyan, hogy egy @xy betűvel tudjak hivatkozni a chaten cikkre, djre, szervezőre, eseményre, kiadványra vagy személyre/userre, ha személyre hivatkozok kapjon róla notifyt és nyissa meg a chaten neki azt az üzenetet és azt is jelezze hogy ki hivatkozott rá. Elkezdem irni a betűket és dobja fel a lehetőségeket. A személyre/userre hivatkozás legyen elérhető mindenkinek, többi csak admin/moderátornak. Természetesen mindegyik kattintható legyen és a megfelelő helyre vigyen."* — **és a kérés: először csak a megvalósíthatóság + javaslatok.**
- **A VÁLASZ: IGEN, megvalósítható** — és ami a legfontosabb: a kérés **négy részéből kettő már készen van**. A mért állapot (kódból): a chat-üzenet a `live_feed_posts/{postId}` dokumentumban van (`publishChatPost` callable írja, `accessRole ∈ {admin, moderator, none}`); a **hat célpont közül mind a hat** elérhető képernyővel bír, és a koppintás útvonala **már létezik** az értesítés-központban (`profile` / `article` / `artist` / `organizer` / `event` / `release`); a **személy-lista** élő streamből jön (`getRegisteredPublicProfiles`), a **tartalom-listák** (cikk/DJ/szervező/esemény/kiadvány) pedig a **mentett** WordPress-listákból; a **„nyissa meg azt az üzenetet"** a 357-es körben elkészült (`targetType: 'chat'` + `targetId` → görgetés + kiemelés), a **„ki hivatkozott rá"** pedig a szintén most bevezetett `resolveActorName()`-nel megoldott.
- **A HIÁNYZÓ KÉT RÉSZ:** (a) az `@` beírása közbeni **javaslatlista** (a chat beviteli mezőjében), és (b) maga a **hivatkozás** — tárolás (`mentions` tömb a `live_feed_posts`-on), kattintható megjelenítés (`Text.rich`; ma egyszerű `Text(post.text)`) és az **értesítés** (`chat_mention`, `targetType: 'chat'`, `dedupeKey: chat-mention:{postId}:{uid}`).
- **⚠️ AMIT A MEGVALÓSÍTÁS KÖTELEZŐEN IGÉNYEL:** a Firestore-szabály bővítése — a `live_feed_posts` create-whitelistje `keys().hasOnly([...])`, ezért a `mentions` mező **nincs benne**, és anélkül a kliens-oldali írás elutasításra kerülne (+ `functions/rules.test.cjs` bővítés). A **jogosultságot a szerver** kényszerítse (a kliens-oldali szűrés csak UX): nem admin/moderátor tartalom-hivatkozásait a `publishChatPost` **kihagyja** a `mentions` tömbből (a szöveg marad), és visszaadja a kihagyottak számát.
- **📄 A RÉSZLETES TERV: `docs/CHAT-MENTION-TERV.md`** — mért állapot-táblázat, a javasolt szintaxis (egy `@` mindenre, típus szerint csoportosított találatokkal), az adatmodell (`{type, id, label}` + korlátok: max 10 hivatkozás, max 5 személy-értesítés), a szerveroldali jogosultság, az értesítés mezői, a **kétkörös bontás** (1. kör: személy-hivatkozás teljes élménnyel; 2. kör: tartalom-ágak adminnak), a **hat nyitott döntés**, a kockázatok (névütközés, törölt célpont, spam, régi appok) és a becslés. **A következő kör ott kezdje, ne itt.**
- **✅ A TULAJDONOS DÖNTÉSEI (2026-09-24, kérdező ablak):** (1) **mind a hat típus egyszerre** (nem két körben); (2) **minden megemlített személy** kap értesítést, de **legfeljebb 5 személy/üzenet**; (3) a nem admin/moderátor tartalom-hivatkozásait a szerver **kihagyja** (a szöveg marad) + az app jelzi; (4) egyelőre **csak a Chat** (a cikk-kommentek nem). → **Az implementáció e szerint indult: AAB 358** (a 357-et a tulajdonos közben feltöltötte az `alpha` sávra, ezért kellett új verziókód).
- **⚠️ ŐSZINTE KORLÁT:** ez **terv**, nem implementáció — a fenti állapot a **kódból olvasott** tény (nem futó mérés), és a javasolt korlátok (5/10 hivatkozás, napi plafon) **döntést** kérnek. A `@név` szintaxis velejárója, hogy két azonos nevű felhasználónál a **szöveg** kétértelmű lehet (a hivatkozás a **uid**-hoz kötött, ezért a kattintás helyes marad).

### KÉRDÉS ELŐRE: angol nyelv az appban — a MENNYISÉG mérve, lehetőségek (2026-09-25; **kód NEM változott**)

- **A TULAJDONOS KÉRÉSE:** *„valahogy megkéne oldani az angol nyelvet az appban, de a sima AI fordító nagyon lassú volt, lehetőségeket kérek, ráadásul asszem a google translate api szinte ingyen van … ha ilyesmit veszünk igénybe ne 100 évig fordítson. Egyszer megpróbáltuk open ai apival, de nem működött megfelelően"* → **ez a szakasz a mérés és a lehetőségek, nem implementáció.**
- **A MÉRT KIINDULÁS (a mai állapot, nem emlékezet):**
  - Az app **HU-ra van rögzítve**: `lib/main.dart:212` `locale: const Locale('hu','HU')`, `supportedLocales: [Locale('hu','HU')]`, és a `flutter_localizations` csak a keretrendszer-widgetekhez van bekötve. **ARB-fájl, `l10n.yaml`, `AppLocalizations` NINCS** (mérve: 0 találat) — vagyis az i18n **nulláról** indul.
  - **A felület szövege (mérve, új eszköz `tools/check-translation-volume.mjs`, önteszt 8/8):** 180 Dart-fájl, 6 349 string literál; **1 800 magyar ékezetes** (64 409 karakter) + 295 mondatszerű (5 309) = **69 718 karakter**, ebből **egyedi 62 437 karakter (1 615 szöveg)**. A maradék 4 254 literál technikai (kulcs/azonosító).
  - **A szerveroldali szövegek** (`functions/*.js`, 16 fájl, ugyanezzel az eszközzel `--dir=functions`): **11 438 karakter**, egyedi **9 212 (283 szöveg)** — az értesítések és a szöveges válaszok.
  - **A WordPress-tartalom (mérve, `tools/check-content-volume.mjs`, önteszt 4/4, csak olvas):** hírek **10 rekord / 56 607 karakter**, események **3 / 25 496**, szervezők **4 / 933**, DJ-k 17 és kiadványok 21 (a **lista**-végpont csak címeket ad: 145 + 697 karakter — a **bemutatkozók** a részlet-végponton vannak, egy DJ-nál mérve `biography=583` + `excerpt=215`, tehát a valódi összeg ennél ~10-15 ezer karakterrel több). **Együtt ma ≈ 100 ezer karakter**, és **minden új cikk ~5-6 ezer karakter** (a hírlevél/chat nem számolva).
- **A KÖLTSÉG (Google Cloud Translation, 2026-os díjszabás: NMT **20 USD / 1 millió karakter**, az első **500 ezer karakter/hó ingyen** — [díjszabás](https://cloud.google.com/translate/pricing), [összefoglaló](https://chatscontrol.com/blog/google-cloud-translation-api-pricing-limits-2026)):** a felület **1,39 USD** (egyedi szövegre 1,25), a szerveroldali szövegek **0,23**, a teljes mai tartalom **1,68** → **a teljes app + tartalom együtt ~3,3 USD**, utána **~0,11 USD / új cikk**. Vagyis a tulajdonos sejtése („szinte ingyen") **mérés szerint igaz**. (Az Azure ~10 USD/1M, a DeepL ~25 USD/1M; a Translation LLM input+output ~20 USD/1M, de ~3× lassabb.)
- **MIÉRT VOLT LASSÚ A KORÁBBI PRÓBA (a mérésekből következő magyarázat):** a lassúság szinte biztosan a **futásidejű** fordításból jött (minden megjelenített szövegre hálózat + modell-válasz). Az NMT **milliszekundumos** késleltetésű, az LLM-alapú viszont ~3× lassabb; a helyes minta ezért a **egyszer lefordítani és eltárolni**, nem minden kérésnél újra.
- **A LEHETŐSÉGEK (a döntés a tulajdonosé):**
  1. **A) Fordítás építéskor + tartalom-fordítás publikáláskor (ez a javaslat).** A felület szövegei **ARB-fájlokba** kerülnek (`flutter_localizations` + `intl`), egyszer lefordítva (NMT, „glossary"-val a hardstyle kifejezésekre), és a fordítás **bekerül a repóba** → futásidőben **nulla költség, nulla várakozás, offline is jó**. A szerveroldali 283 szöveg ugyanígy egy `messages-en.js`-be. A WordPress-tartalom pedig **publikáláskor** kap egy angol mezőt (plugin vagy Cloud Function hívja az API-t, az eredmény a WP-metában marad) → az app a kért nyelven kéri le. **Ára:** a **refactor** a valódi munka (1 800 literál 180 fájlban, képernyőnként haladva, a HU marad fallback), a fordítás maga **~3 USD** és **percek**.
  2. **B) Futásidejű fordítás gyorsítótárral** (a szöveg → Cloud Function → NMT → Firestore-cache → vissza). **Ára:** nincs refactor, és a CMS-tartalmat is lefedi; **de** minden szöveg **első** megjelenése vár (hálózat + API), az élő chat fordítása minőségi/ bizalmi kérdés, és a felület utólag rajzolódik újra (villanás). **Ez az, ami a tulajdonos panaszát reprodukálja**, ha nincs agresszív cache.
  3. **C) Csak a WordPress-oldal fordít** (a WP fordítja a tartalmát, az app nyelve marad HU) → **fél megoldás**: az angol felhasználó továbbra is magyar menüt lát.
  4. **D) LLM-fordítás glosszáriummal, szintén építéskor** (jobb a hangvétel a marketing-szövegeknél, pl. „Kiemelt esemény"), ~3× lassabb és drágább, de **építéskor ez nem számít** → együtt is csak pár dollár. A korábbi OpenAI-próba valószínűleg a **futásidejű** hívás és a formátum miatt nem működött; építéskori, szigorú JSON be/ki mellett ez más.
- **AMIT EZ MÉG IGÉNYEL (rejtett munka, hogy ne legyen meglepetés):** (1) a **nyelvválasztó** és a választás mentése (a `locale` most **rögzített**); (2) az **értesítések** nyelve: a profilban tárolt nyelv kell, különben az angol felhasználó magyar értesítést kap; (3) a **chat/felhasználói szöveg** szándékosan **kimarad** az első körből; (4) **glosszárium** a márkanevekre és a szakszavakra (pl. „kiadvány" → release, „szervező" → organizer, „nyereményjáték" → giveaway); (5) a fordítás **forrása** (a HU szöveg) ne csússzon el: ha a HU-t később módosítjuk, a fordítást is frissíteni kell (ezt egy teszt tudja jelezni).
- **⚠️ ŐSZINTE KORLÁT:** a fenti számok **statikus mérések** (string-literálok a kódból + a WP nyilvános végpontjai), nem próba-fordítás: a valódi fordítási mennyiség a napló-/hibaszövegek miatt **kevesebb**, a tartalomnál viszont a **részlet-végpontok** miatt **több** lehet. A Google **ingyenes kerete** (500 ezer karakter/hó) a v2/v3 között **közös**, és a **HTML-tagek is karakternek számítanak** (ezért a tartalmat tag nélkül érdemes fordítani). A minőséget **nem mértem** — gépi fordításnál a magyar hardstyle szleng és a márkanevek a gyenge pont, erre való a glosszárium vagy az LLM-ág.

### `@mindenki` a Chatben + a mélyen lévő üzenethez is odaugrik (2026-09-25, **AAB 360** + szerveroldali deploy)

- **A TULAJDONOS KÉT JELZÉSE EGY ÜZENETBEN:** (1) *„fentvan és egy régebbi chat like, update előtti, rányomtam és nem dobott a chat üzire … ja közben teszteltem, régebbi chat üzivel nem megy, újabba igen, ez hibás működés"*; (2) *„kéne egy @mindenki tag is, amit ha beütök, kap mindenki notifyt és csak moderátor/admin használhassa"*.
- **1) A RÉGEBBI ÜZENET — A MÉRT GYÖKÉR (nem a lapozás, és nem az adat!):**
  - **Amit előbb kizártam méréssel:** az új `tools/check-chat-focus-reach.mjs` (önteszt **9/9**, csak olvas) szerint a chatben **117 üzenet** van, és **mind a 13 chat-értesítés a legfrissebb 60 közé mutat** (rang 1–22) — vagyis a lapozásnak **nem is kellett volna** szerepet kapnia, a célpontok mind elérhetők. (`live: 13, paged: 0, beyond: 0, missing: 0`.)
  - **A valódi gyökér a görgetés volt:** a `LiveFeedScreen` `_scrollToFocusedPost`-ja azt kérte a `Scrollable.ensureVisible`-tól, hogy görgessen a kártyához — de a `ListView` **csak a látható elemeket építi fel**, ezért a mélyen lévő (régebbi) üzenet kártyájának **nincs kontextusa**. A régi kód ilyenkor a lista **VÉGÉRE** ugrott (`animateTo(maxScrollExtent)`) — az a **legrégebbi** üzeneteket mutatja, nem a megjelöltet. A **legfrissebb** üzenetnél ez azért működött, mert az már fel volt épülve (a lista tetején van) → innen a „régebbi nem megy, újabb igen" minta. (Ráadás: a lista tetején lévő célnál is elromlott, ha a kártya épp **még nem épült fel** — ez volt a korábbi „néha igen, néha nem".)
  - **A REPRODUKÁCIÓ WIDGET-TESZTTEL (ez a lényeg):** új `test/widgets/chat_focus_scroll_test.dart` — 40 üzenetes lista, a megjelölt üzenet a **24.**, illetve a **39.** helyen; a teszt azt kéri, hogy a kártya **látszódjon is** (a `tester.getRect` a képernyőn belül legyen), ne csak létezzen. **A javítás előtt 3/4 bukott** (a „legrégebbi" véletlenül átment, mert a régi kód pont a lista aljára ugrott — ez mutatja, hogy a régi viselkedés csak véletlenül volt jó).
  - **A JAVÍTÁS:** a tiszta tervben új `chatScrollEstimateForIndex({index, itemCount, maxScrollExtent})` — a lista **átlagos sormagasságából** becsüli a cél pozícióját (a `maxScrollExtent` maga is a felépített gyerekekből számolt becslés), a képernyő pedig **legfeljebb 4 körben** ugrik a becsült helyre, és onnan pontosít a `ensureVisible`; közben a már felépült kártyánál **azonnal** megáll. A `maxScrollExtent` **már nem önálló ugrási cél** (ezt forrás-lint is őrzi: pontosan **egyszer** szerepel, a becslés bemeneteként).
- **2) `@mindenki` — KLIENS + SZERVER:**
  - **Kliens (`chat_mention_plan.dart`, `chat_message_text.dart`, `content_target.dart`, `community_screen.dart`):** új konstansok (`mentionTypeEveryone` / `mentionEveryoneId` / `mentionEveryoneLabel`), a javaslatlista **első** helyén a „Mindenki — Mindenki értesítést kap" sor, **csak privileged** felhasználónak, és **csak ha már gépelt valamit** (üres `@`-nál szándékosan nem: egy véletlen koppintás mindenkinek küldene értesítést). A szövegben a `@mindenki` **ki van emelve, de nem kattintható** (nincs mögötte adatlap → nincs felesleges „nem elérhető" üzenet), a közös `openContentTarget` pedig `everyone` típusra **némán `false`-t** ad.
  - **Szerver (ÉLŐ — `npx firebase deploy --only functions` → „Deploy complete!", a `publishChatPost` a `functions:list`-ben a helyén):** a `sanitizeMentions` az `everyone` típust **csak privilegednek** engedi (különben `dropped`), **egyszer** (a második kiesik), a 10-es plafon rá is érvényes, és a **kanonikus** `{type:'everyone', id:'everyone', label:'mindenki'}` alakot írja (a kliens id/label-jét nem hiszi el). A `chatEveryoneNotifications` a `community_profiles`-ból épít címzett-listát (a **dokumentum-azonosító** a uid), a **szerzőt kihagyja**, **50-es kötegekben** ír `createNotificationBestEffort`-tal (best-effort: a hiba nem viheti el az üzenetet), a plafon **500** (`MAX_EVERYONE_RECIPIENTS`), az értesítés `type: 'chat_mention'`, `title: 'Megemlítettek a Chatben'`, `body: '{név} mindenkit megemlített a Chatben: „{≤80}”'`, `targetType: 'chat'`, `targetId: postId`, `dedupeKey: 'chat-everyone:{postId}:{uid}'`. A callable válasza `everyoneNotified`-dal bővült. **A `firestore.rules` NEM változott** — az `everyone` elem átmegy a meglévő alak-ellenőrzésen (mérve: nincs típus-felsorolás a szabályban), a mélységi jogosultságot a callable adja.
  - **A CÍMZETTSZÁM MÉRVE:** `node tools/check-everyone-reach.mjs` (önteszt **5/5**) → `community_profiles` **41**, `public_profiles` **41**, unió **41** címzett — ezért 1 `@mindenki` üzenet ma **≤ 41 írás**, nem kell kötegelt nagyüzem.
- **BIZONYÍTÉK (mind mérve):** `flutter analyze lib test` → **No issues found!**; `flutter test` → **978/978** (a kör előtt 953; **+25**: `chat_focus_scroll_test` **4**, a pozíció-becslés **6**, a `@mindenki` terv-tesztjei **5**, a szöveg-widget **2**, a `@mindenki` forrás-lintjei **4**, és a `mentionSpans` új esetei **4**); `node --test functions/chat-mention-plan.test.cjs` → **25/25**; `node --test functions/chat-notification-plan.test.cjs functions/actor-name-plan.test.cjs` → **21/21**; `node --check functions/index.js` + `functions/chat-mention-plan.js` → **exit 0**; `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN** (Play-blokkok **206 / 458 / 453 / 171** karakter).
- **MUTÁCIÓS BIZONYÍTÉK (ezúttal a saját teszteimet is megmértem):**
  - **A görgetés:** a becslés kicserélése a régi `maxScrollExtent`-ugrásra → a widget-tesztek **3/4 buknak**, bájtpontos visszaállítás (SHA-256 `513F4C8C…` egyezik) után **4/4 zöld**.
  - **A szerveroldali `@mindenki`:** a szerver-ágens **8 mutációt** futtatott (privileged-kapu kivétele, kanonikus alak elrontása, `public_profiles` használata, a szerző kihagyásának elhagyása, a kötegméret és a try/catch eltávolítása, a visszatérési alak megváltoztatása, a `createNotificationBestEffort` cseréje) — **mindegyik megbuktatott legalább egy tesztet**, és minden visszaállítás **bájtpontos** volt (SHA-256 egyezés), utána újra zöld.
- **Csomag:** **`build/HUHS-v1.0.0+360-release.aab`** (versionCode **360** a merge-elt manifestből, **81,52 MB** / 85 474 799 bájt, SHA-256 `0C000AB2BF79EBF0FB27B2B8F356A09518636978F91B875F215A35DC314B26DF`); `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN** (Play-blokkok **206 / 458 / 453 / 171** karakter); `check-signing-identity.mjs` → **1 aláírási identitás** (AAB 60). **A kód a csomagban (mérve a `libapp.so`-ban):** `ChatFocusStatus` és `ChatMessageText` (UTF-8, mindhárom ABI), `chatScrollEstimateForIndex` (UTF-8, `armeabi-v7a`; a másik kettőben a fordító **beinline-olja**, ezért ott nincs külön név), a `@mindenki` szöveg (UTF-16), a `Mindenki értesítést kap` felirat (Latin-1) és a 360 changelog-sora (UTF-16). ⚠️ **MÉRÉSI TANULSÁG — HÁROM KÓDOLÁS:** a Flutter AOT a `libapp.so`-ban az **azonosítókat UTF-8-ként**, a **csak Latin-1 karaktereket tartalmazó szövegeket Latin-1-ként** (`é` = `0xE9`), a **nem Latin-1 karaktert (pl. `—`, `”`, `ő`) tartalmazó szövegeket pedig UTF-16LE-ként** tárolja. Aki szöveget keres a csomagban, **mindhárom kódolással** próbálkozzon, különben hamis „nincs benne" eredményt kap (ez a saját hibám volt a 360-as körben: a `Mindenki értesítést kap` első keresése ezért nem talált semmit).
- **⚠️ A SZONDA-SZKRIPTEK ÉS A KÓDLAP (ismételt tanulság):** a `tmp/*.ps1` szondákat **ASCII-only** forrásból kell írni, és az ékezetes keresőszavakat **kódból** (`[char]0x00E9`) építeni — a Windows PowerShell 5.1 a `.ps1` fájlt az **ANSI kódlapjával** olvassa, ezért a fájlba írt `é`/`ő` a szkriptben is félrekódolódik (a keresés így csendben „nincs találat"-ot ad).
- **⚠️ ŐSZINTE KORLÁT:** a `@mindenki` fan-outja **éles Firestore ellen nincs megmérve** (a szerver-ágens nem deployolt, én csak a deploy sikerességét és a függvény jelenlétét mértem) — az első éles `@mindenki` üzenet adja meg a valós `everyoneNotified` számot. A `@mindenki` **küszöb nélkül** szól mindenkinek (a szerver plafonja 500); napi külön plafon **nincs** (a `chat_post` rate limit 20 üzenet/perc/fiók maradt). A **görgetés javítása** widget-teszttel bizonyított, de **eszközön** a tulajdonos látja majd (a becslés a lista átlagos sormagasságára épül, ezért a `ensureVisible` pontosít rá).

### Play-javaslatok: a két „teljes képernyős" kártya — MÉRVE, egyik sem a mi kódunk (2026-09-25; **kód NEM változott**)

- **A TULAJDONOS JELZÉSE:** képernyőkép a Play Console-ból a **358**-as kiadásról: *„amúgy ezeket még mindig írja"* — (1) *„Előfordulhat, hogy a teljes képernyős mód nem jelenik meg minden felhasználónál"*, (2) *„Az alkalmazásod elavult API-kat vagy paramétereket használ a teljes képernyős megjelenítéshez"*.
- **A MÉRÉS (a 360-as AAB `base/dex/classes.dex` + `classes2.dex`, `dexdump` + híváshely-keresés; új szkriptek: `tmp/check-edge-to-edge-dex.ps1`, `tmp/probe-decor-fits.ps1`, `tmp/probe-edge-invoke.ps1`):**
  - Az **`EdgeToEdge` szöveg 0-szor** szerepelt a csomagban (a `MainActivity.onCreate` **hívja** ugyan az `enableEdgeToEdge()`-t 356 óta, de az **R8 beinline-olja**, így a híváshely eltűnik).
  - A **`setDecorFitsSystemWindows` 1 híváshely** — a **Flutter-motor** `PlatformPlugin.enableEdgeToEdge()`-je (pontosan a `setSystemUiVisibility(0)` + `setDecorFitsSystemWindows(window,false)` pár, ugyanaz, amit a Flutter-hiba idéz).
  - A **`layoutInDisplayCutoutMode` 4 hivatkozás** — az `androidx.core` `WindowCompat`-ja (R8-összevonva).
  - A **Play szkennere `EdgeToEdge.enable()` híváshelyet keres**, és a `WindowCompat.setDecorFitsSystemWindows(...)` **nem** elég neki — ezt a nyitott Flutter-hiba is így írja: [flutter/flutter#192921](https://github.com/flutter/flutter/issues/192921) (a motor **soha** nem hívja az `EdgeToEdge.enable()`-t, ezért **minden** Flutter-appot jelez a Play).
- **AMIT MEGPRÓBÁLTAM (és miért nem lett belőle kód):** az `android/app/proguard-rules.pro`-ba tett **`-neverinline class androidx.activity.EdgeToEdge { *; }`** szabályra a build **elhasalt**: *„R8: Unknown option -neverinline"* (ez az R8-verzió nem támogatja). A **csak `-keep`** viszont **nem** hat a beágyazásra: mérve `EdgeToEdge hivatkozás=24, HÍVÁS=0` — az osztály bennmarad, de híváshely továbbra sincs, vagyis a szkennert nem győzi meg, csak hizlalja a csomagot. A `-dontoptimize` lenne az egyetlen működő kapcsoló, az viszont a **teljes** R8-optimalizálást kikapcsolja (nagyobb, lassabb csomag) — **egy kozmetikai javaslatért nem érdemes**, ezért a proguard-fájl **bájtpontosan visszaállt** (SHA-256 `0AE47CC9EDF89280B40ADBB8CE93A42347CFAD8142EC83303EAD67C47643AE8B`), és a szállítandó 360-as csomag **változatlan** (`0C000AB2…`, a dokumentumban szereplő hash egyezik).
- **A TANULSÁG:** a Play „művelet javasolt" kártyái **statikus, külső kódot is néző** javaslatok — nem blokkolók (a 358 a zárt teszten **100%-ban** kigördült). A két „teljes képernyős" kártya **Flutter-motor + Google-könyvtár** kódjából jön, a harmadik (bittérkép) az **AdMob SDK**-ból; app-oldali javítás jelenleg **nincs** rájuk. A dokumentum: `docs/PLAY-KIADASI-JEGYZET.md` **§0c**. **Újramérés** egy paranccsal: `tmp\check-edge-to-edge-dex.ps1 -Aab build\HUHS-v1.0.0+<build>-release.aab`.

### Chat-odaugrás javítása: hideg indításnál is a megjelölt üzenetre ugrik (2026-09-25, **AAB 359**)

- **A TULAJDONOS JELZÉSE:** *„chat üzenet like értesítés néha a megfelelő helyre dob, ha rányomok, néha nem"* — vagyis a 357-ben bevezetett odaugrás **nem mindig** működött.
- **A MÉRT GYÖKÉR (kódból, nem tipp):**
  1. A **Chat fül lustán épül fel** (`MainNavigation._tabs[index] ??=` → a nem látott fül `SizedBox.shrink()`), ezért ha a Chatet abban a munkamenetben **még nem nyitottad meg**, a `communityPostsProvider` **hideg**, és az élő ablak **üresen indul**.
  2. A `LiveFeedScreen` az első képkocka után `_resolveFocus(valueOrNull ?? [])`-t hívott: üres listánál a tiszta terv **`keepLoading`**-ot adott, a képernyő pedig **lapozásnak számolta** azt a kört is, amelyben nem volt mit lapozni (`_focusPagesLoaded++` a betöltés ELŐTT történt).
  3. A betöltés alatt a képernyő **pörgőt** rajzol (`CircularProgressIndicator`), az pedig **folyamatos képkockákat** ad — a post-frame lánc másodpercenként sokszor lefutott, így a **10 lapos keret ~0,2 s alatt elfogyott**, jóval azelőtt, hogy a Firestore-adat megérkezett volna. Utána a `_focusFinished = true` miatt a később megérkező adat **már nem indított keresést**.
  4. Ez pontosan a „néha igen, néha nem" mintázat: **meleg** providerrel (a Chat már nyitva volt a munkamenetben) azonnal megtalálta az üzenetet, **hidegen** viszont feladta.
  - ⚠️ **Amit kizártam méréssel:** nem az értesítések hiányos célpontja az ok — az **éles `notifications` gyűjteményben mind a 11 `chat_reaction` és mind a 2 `chat_reply` bejegyzésben benne van** a `targetType: 'chat'` ÉS a `targetId` (új, csak olvasó eszköz: **`tools/check-chat-notification-targets.mjs`**, önteszt **6/6**; a `chat_mention` típusból még **0** van, mert a `@`-os értesítés a mérés idejéig nem keletkezett).
- **A JAVÍTÁS (kliens, két helyen + tesztek):**
  - **Tiszta terv (`lib/services/chat_focus_plan.dart`):** új állapot **`ChatFocusStatus.waiting`** — ha az élő ablak **és** a lapozott lista is üres, akkor **nem lapozunk és nem fogyasztjuk a lap-keretet** (a lap-korlát ellenőrzése **után**, hogy a keret akkor is zárjon, ha az adat sosem jön meg). Az adat megérkezésekor a `build`-beli `ref.listen` úgyis újraszámolja a tervet.
  - **Bekötés (`community_screen.dart`):** a `_loadOlderPosts()` mostantól **`Future<bool>`** — megmondja, történt-e **valódi** lapozás (indult kérés és megjött a válasz); a `waiting` ág **nem indít lapozást**; a lap-keret (`_focusPagesLoaded++`) **csak sikeres lapozás után** nő; sikertelen lapozásnál (nincs mit lapozni / hálózati hiba) az odaugrás **feladja**, nem pörög; a post-frame callback a lapozás előtt **és** után is ellenőrzi a `_focusFinished`-et, ezért nem indít fölösleges lapozást, ha közben megérkezett az adat.
- **BIZONYÍTÉK (mind mérve):** `flutter analyze lib test` → **No issues found!**; `flutter test` → **953/953** (a kör előtt 946; **+7**: a `chat_focus_plan_test` új „amíg az élő ablak nem érkezett meg" csoportja **5** + a `chat_focus_wiring_test` **2** új forrás-lintje); **mutációs bizonyíték kétszer:** (a) a `waiting` ág kikapcsolása (`if (false && …)`) → a terv-tesztek **2 kárán elbuknak** (10/12), bájtpontos visszaállítás után **12/12**; (b) a **számláló a lapozás ELÉ** helyezése → a forrás-lint elbukik, visszaállítás után zöld. ⚠️ **A forrás-lint első változata GYENGE volt** (csak a szövegbeli sorrendet nézte, ezért a (b) mutációt **átengedte**) — a végleges változat a **darabszámot** (`_focusPagesLoaded++` pontosan **egyszer** szerepel) és a sorrendet is kéri. **A tanulság ugyanaz, mint a hírlevél-körben: a forrás-lintet mutációval kell igazolni.**
- **Csomag:** **`build/HUHS-v1.0.0+359-release.aab`** (versionCode **359** a merge-elt manifestből, **81,51 MB** / 85 467 799 bájt, SHA-256 `AB01387AB3E125577DD4AE8156CC96D1B776B32CD353CE363B5657E5672F84FD`); `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN** (Play-blokkok **164**, **463**, **453**, **171** karakter; 37 build-bejegyzés). **A kód a csomagban:** mindhárom ABI `libapp.so`-jában megvan a `chatFocusPlan`, `ChatFocusPlan`, `waiting` és `_loadOlderPosts` szimbólum, valamint a 359 changelog-szövege.
- **⚠️ MÉRÉSI TANULSÁG A CSOMAG-ELLENŐRZÉSHEZ:** a `libapp.so`-ban a **szimbólumok/ASCII azonosítók UTF-8-ként**, az **ékezetes szövegek viszont UTF-16LE-ként** vannak tárolva (a `MINDIG a megjel…` minta csak UTF-16-tal található meg, UTF-8-cal nem) — aki szöveget keres a csomagban, **mindkét kódolással próbálkozzon**.
- **⚠️ SAJÁT HIBA ÉS A JAVÍTÁSA (kódolás):** a dokumentum hash-cseréjét `Get-Content -Raw` + `Set-Content -Encoding UTF8` párossal csináltam, és a PowerShell 5.1 a **rendszer ANSI kódlapjával (CP1250)** olvasta a fájlt, majd UTF-8-ként írta vissza — ettől **minden ékezet két karakterré vált** („é" → „Ă©"), és a Play-blokkok hossza is megugrott. **A javítás** (`tmp/fix-encoding.ps1`): a szöveget UTF-8-ként beolvasni, **CP1250-nel bájtokká** alakítani, majd **UTF-8-ként dekódolni** — a visszafordítás **veszteségmentes** volt (0 db U+FFFD, a blokkok visszaálltak: 164/463/453/171). **A szabály továbbra is áll: szövegfájlt a `read`/`edit`/`write` eszközzel írj, ne `Get-Content -Raw | Set-Content`-tal.** (A `pubspec.yaml` 60–61. sora és az `AGENTS.md` egyik angol sora **régebbi, már bent lévő** mojibake-et tartalmaz — azokat szándékosan nem nyúltam hozzá, mert nem ehhez a körhöz tartoznak.)
- **📌 A TULAJDONOS LÉPÉSE:** a **359 feltöltése a zárt teszt sávra** a **`docs/PLAY-KIADASI-JEGYZET.md` 1. blokkjával**; a nyilvános kiadáshoz az **1b. blokk** (356–359 összesítő). **Miért 359 (mérve):** a zárt teszten a **358** van (`completed`, 100%), a production sávon a **355**.
- **✅ iOS-RE IS FELMENT A 359 — MÉRVE (2026-09-25):** a tulajdonos kérésére (*„sideloadolt iphone-ra"*) a meglévő CI-út futott, kódváltozás nélkül: a `931f7334` push → **GitHub Actions `iOS fordítás-ellenőrzés` (run `36125143080`) zöld** → artifact letöltve: **`build/ios-ipa-359/Runner-unsigned.ipa`, 26 308 567 bájt**, MD5 **`9A3AEB445DAFCA5BB7BC6B34B7E9A93A`** → `node tools/verify-ios-ipa.mjs build/ipa-359/Payload/Runner.app --test-ads --contains=chatFocusPlan --contains=ChatFocusPlan --contains=_loadOlderPosts --contains=ChatMentionTarget --contains="MINDIG a megjel"` → **MINDEN ELLENŐRZÉS RENDBEN** (teszt-AdMob-egységek + a 359 javításának szimbólumai és a 359 changelog-szövege). **A telepítés bizonyítéka** (Sideloadly `installations.db`): `stored_files.id=16` (`stored_name = 9a3aeb445dafca5bb7bc6b34b7e9a93a.ipa` = **a letöltött IPA MD5-je**, `size = 26308567` = **pontosan a letöltött méret**), `installations.id=17`, `12:49:56 → 12:50:20` (24 s), eszköz *Imre iPhone-ja* (`00008030-001475413ED2402E`, USB), Apple ID `djdeeroy@gmail.com`, `final_bundle_id = hu.hungarianhardstyle.app.JQPJ793V65` (változatlan → **valódi frissítés**), `last_error` üres, `failures_count = 0`, `known_ttl = 7`. A „Start" gombot **UIAutomation-nel, név szerint** nyomtam meg. **A telepített verzió a KÉSZÜLÉKRŐL visszaolvasva:** `pymobiledevice3 apps list` → **`CFBundleVersion = 359`**, `CFBundleShortVersionString = 1.0.0`. ⚠️ **Amit NEM tudtam megmérni:** az app **indítása** a gépről ismét **elhasalt** (`NotPairedError` / „Failed to launch"), a `developer dvt screenshot` pedig **fekete** — a telefon **le volt zárva**; az „elindul" igazolást a tulajdonos adja (egy koppintás). A **359 javításának hatása** (hideg indításnál is odaugrik) a készüléken mérhető: friss app-indítás után, a Chat **megnyitása nélkül**, egy chat-lájk értesítésre koppintva kell az üzenetre ugrania.

### Chat-`@`hivatkozás — **MEGÉPÜLVE** (2026-09-25, **AAB 358** + szerveroldali deploy + iOS sideload)

- **MI ÉPÜLT (a fenti terv és a tulajdonosi döntések szerint, egy körben):**
  - **Szerveroldal (ÉLŐ — `npx firebase deploy --only functions,firestore:rules` → „Deploy complete", a szabályok újraellenőrizve: „compiled successfully" + „released rules … to cloud.firestore"):** új tiszta modul **`functions/chat-mention-plan.js`** (`MENTION_TYPES`, `sanitizeMentions(raw,{privileged})` → `{mentions, dropped}`, `mentionExcerpt`, `chatMentionNotifications`; korlátok: max **10** hivatkozás, max **5** személy-értesítés, id ≤ **64**, címke ≤ **80**, részlet ≤ **80** karakter). A **`publishChatPost`** ezt hívja (`privileged = accessRole === 'admin' || 'moderator'`), a `mentions` mezőt csak akkor írja, ha maradt értelmes elem, a **személy-értesítéseket** a válasz-értesítés blokk után küldi (`type: 'chat_mention'`, `title: 'Megemlítettek a Chatben'`, `body: '{név} megemlített a Chatben: „{80 karakteres részlet}”'`, `targetType: 'chat'`, `targetId: postId`, `dedupeKey: 'chat-mention:{postId}:{uid}'`, a szerzőt kihagyja), és visszaadja a **`droppedMentions`** számot. A **`firestore.rules`** `live_feed_posts` create-whitelistje megkapta a `mentions` mezőt, alak-ellenőrzéssel (lista, ≤ 10 elem, az első elem csak `type/id/label`, hosszkorlátok) — ⚠️ **a szabály nem tud listán végigiterálni**, ezért a mélységi jogosultságot a callable kényszeríti (a szabály kommentje ezt ki is mondja).
  - **Kliens (AAB 358):** új tiszta modul **`lib/services/chat_mention_plan.dart`** (`mentionType*`, `activeMentionQuery` — az e-mail-környezetet és a `@@`-ot elutasítja, `insertMention`, `mentionSuggestions` — személyek elöl, prefix veri a „contains”-t, `mentionSpans` — leghosszabb címke elöl); **`lib/services/chat_mention_source.dart`** (személyek élő streamből, tartalom a mentett WordPress-listákból; `mentionPrivileged()`); **`lib/widgets/chat_mention_overlay.dart`** (típus szerint csoportosított lista, `Key('chat-mention-overlay')`, üresen nem rajzol semmit); **`lib/widgets/chat_message_text.dart`** (`Text.rich` a `mentionSpans`-ból, kattintható, a recognizerek el vannak dobva, hivatkozás nélkül sima `Text`); **`lib/core/navigation/content_target.dart`** — **közös** `openContentTarget(navigator, {targetType, targetId})` (profile/user, news/article, event, release, artist, organizer), amit az **értesítés-központ is** használ (5 másolt ág és 6 import törölve → nem tud széthúzni). A `community_screen.dart` kapja a javaslatlistát (egyszeri, lusta adatbetöltés), a `_send` csak a **ténylegesen bennmaradt** hivatkozásokat küldi, és `droppedMentions > 0` esetén jelzi a felhasználónak, hogy egy tartalom-hivatkozás nem ment át.
- **BIZONYÍTÉK (mind mérve):** `flutter analyze lib test` → **No issues found!**; `flutter test` → **946/946** (a kör előtt 907; **+39**: `chat_mention_plan_test` **28**, `chat_mention_wiring_test` **23**, `chat_message_text_test` **6**, `chat_mention_overlay_test` **6**, `chat_mention_source_test` **4**, és a `notification_targets_test` 2 állítása a közös feloldóra állítva); `node --test functions/chat-mention-plan.test.cjs functions/article-comments.test.cjs` → **27/27**; `npx firebase emulators:exec --only firestore --project demo-huhs "node functions/rules.test.cjs"` → **21/21** (a stderrben látszó `PERMISSION_DENIED` sorok a **szándékolt** elutasítások); `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN** (1. blokk **244/500**, 1b. blokk **405/500**, a dokumentumban szereplő hash valós, 36 build-bejegyzés); `node tools/check-signing-identity.mjs` → **1 aláírási identitás** (60 csomag).
- **Csomag:** **`build/HUHS-v1.0.0+358-release.aab`** (versionCode **358** a merge-elt manifestből, **81,51 MB**, SHA-256 `64702D11E6917075B0BD996C78C5F725F473727375D1CA2F299001414240264E`); a kód a csomagban **mindhárom ABI** `libapp.so`-jában megvan (`ChatMentionTarget`, `activeMentionQuery`, `insertMention`, `mentionSuggestions`, `mentionSpans`, `ChatMentionOverlay`, `ChatMessageText`, `openContentTarget`, `chat_mention`, `chatFocusPlan`) — a `mentionTypeUser`/`mentionTypeRelease` azért nem külön név, mert **konstans string**, amit az AOT a helyére inline-ol. Commit: **`b223fa26`**. **⚠️ A `functions/article-comments.test.cjs` pirosan állt** (a 357-es körben bevezetett `actorNameOrGeneric` hiányzott a vm-fixtúrából) — a valódi `actor-name-plan.js` betöltésével **11/11**.
- **📌 A TULAJDONOS LÉPÉSE:** a **358 feltöltése a zárt teszt sávra** a **`docs/PLAY-KIADASI-JEGYZET.md` 1. blokkjával** (a nyilvános kiadáshoz az **1b.**, 356–358 összesítő). **Miért 358 (frissen mérve):** `alpha = 357 completed`, `production = 355`, `beta = 354`, és a feltöltött AAB-ek listája **357-nél végződik**.
- **✅ iOS-RE IS FELMENT A 358 — MÉRVE (2026-09-25):** a tulajdonos kérésére (*„ios sideload mehet"*) a meglévő CI-út futott, **kódváltozás nélkül**: `b223fa26` push → **GitHub Actions `iOS fordítás-ellenőrzés` (run `36115185851`) zöld, 11m18s** → artifact letöltve: **`build/ios-ipa-358/Runner-unsigned.ipa`, 26 308 424 bájt**, MD5 **`F51A5422F80C44CB20BA83617DB90EF7`** → `node tools/verify-ios-ipa.mjs build/ipa-358/Payload/Runner.app --test-ads --contains=ChatMentionTarget --contains=activeMentionQuery --contains=mentionSuggestions --contains=ChatMentionOverlay --contains=ChatMessageText --contains=openContentTarget --contains=chatFocusPlan` → **MINDEN ELLENŐRZÉS RENDBEN** (a teszt-AdMob-egységekkel együtt). **A telepítés bizonyítéka** (Sideloadly `installations.db`, másolatból): `stored_files.id=15` (`stored_name = f51a5422f80c44cb20ba83617db90ef7.ipa` = **a letöltött IPA MD5-je**, `size = 26308424` = **pontosan a letöltött méret**), `installations.id=16`, `11:04:11 → 11:04:28` (17 s), eszköz *Imre iPhone-ja* (`00008030-001475413ED2402E`, USB), Apple ID `djdeeroy@gmail.com`, **`final_bundle_id = hu.hungarianhardstyle.app.JQPJ793V65`** (változatlan → **valódi frissítés**, az app-adatok és az App Check debug token megmaradnak), `last_error` üres, `failures_count = 0`, `known_ttl = 7` nap. **A telepített verzió a KÉSZÜLÉKRŐL visszaolvasva:** `pymobiledevice3 apps list` → `CFBundleVersion = **358**`, `CFBundleShortVersionString = 1.0.0`, `ApplicationType = User` (ez a 356-os körben még hiányzó igazolás). A "Start" gombot **UIAutomation-nel, név szerint** nyomtam meg (`InvokePattern`), nem koppintás-koordinátával — a képernyőkép a `tmp/sideloadly-358-b.png`-n mutatja a **„Done." / 100%** állapotot. ⚠️ **Amit NEM tudtam megmérni:** az app **indítása** a gépről **elhasalt** (`DTXNsError: … Failed to launch process with bundle identifier`), és a `developer dvt screenshot` **fekete képet** adott — a telefon **le volt zárva**, amit innen nem tudok feloldani; ezért az „elindul és tölt" igazolás ehhez a körhöz nem tartozik (a tulajdonos egy koppintással megadja). Ez nem a telepítés hibája: a verzió a készülék nyilvántartásából **358**.
- **✅ A TULAJDONOS FELTÖLTTE A 358-AT A ZÁRT TESZTRE — MÉRVE (2026-09-25):** `node tools/check-play-track.mjs` → **`alpha` sáv: `358 completed 100%`**, a kiadási szöveg **szó szerint a §1. blokk** („ÚJ a Chatben: @-tal hivatkozhatsz személyre, cikkre, DJ-re, szervezőre, eseményre és kiadványra — gépelés közben feldobja a találatokat." + „A hivatkozás kattintható (a megfelelő adatlap nyílik meg), és akit megemlítesz, az értesítést kap róla."); a feltöltött AAB-ek listája **358-nál** végződik, és a **`--hashes`** szerint a feltöltött 358 SHA-256-ja **`64702d11e6917075b0bd996c78c5f725f473727375d1ca2f299001414240264e`** = **bájtazonos** a helyben épített csomaggal. A `docs/PLAY-KIADASI-JEGYZET.md` §0 és a fejléc ennek megfelelően frissült. **Ami még hátra van (a tulajdonos döntése):** a **nyilvános** kiadás a **§1b. blokkal** (356–358 összesítő) — a `lastPublishedBuild` továbbra is **355**.
- **📌 A KÖVETKEZŐ KÖR VERZIÓKÓDJA:** mivel a **358 a zárt teszt csúcsa**, a következő app-változás **359** lesz (a 358-at nem lehet újra felhasználni).
- **⚠️ ŐSZINTE KORLÁT:** a **mention-élményt eszközön nem próbáltam** — a javaslatlista, a beszúrás és a navigáció fedezete a tesztek + forrás-lintek; a `ChatMentionSource` **hálózati ágai** (személyek/tartalom lekérése) szintén nem futottak eszközön; a **fekvő + billentyűzet** melletti overlay-magasság (132 px) szintén nem mért. A **tartalom-hivatkozás jogosultsága** a szerveren dől el (a kliens csak UX), és a **régi appok** a `mentions` mezőt nem ismerik — nekik a szöveg marad `@Név` alakban, ami változatlanul olvasható. **✅ A tulajdonos iOS-en igazolta: az app indul, ÉS a Chat `@`-hivatkozás is működik** (a sideload után: *„működik az ájfónon"*, kérdező ablakban megerősítve: *„Az app indul, és a @-ot is kipróbáltam — jó"*). ⚠️ Ez **tulajdonosi eszközpróba**, nem az én mérésem — a fenti pontok közül az **Android-oldali** viselkedés (Play-ről telepített 358) továbbra is a tulajdonos készülékén mérhető.

### Értesítések: KI tette + a Chat-értesítés a megjelölt üzenetre ugrik (2026-09-24, **AAB 357** + szerveroldali javítás)

- **A TULAJDONOS JELZÉSE:** *„izé, jön notify hogy kedvelték egy chat üzentem, meg arról is hogy valaki írt egy hírhez kommentet, de odaírhatná, hogy KI likeolta, a chatnél meg odaugorhatna arra az ünezetre amit lájkoltak, ha a notifyre nyomok"*.
- **A MÉRT GYÖKÉR (éles `notifications` gyűjtemény, csak olvasva — `tools/check-notifications.mjs` + egy célzott szonda):**
  - A **chat-lájk** értesítés **5 példányából 2-ben nem volt név**: „Egy HUHS tag kedvelte a Chat-üzenetedet." — viszont a küldő profiljában **mindkét esetben volt** `displayName` (`community_profiles` **és** `public_profiles` is). A kód **egyetlen forrásból** olvasott (`community_profiles/{uid}.displayName`), és ha az abban a pillanatban üres volt, a szöveg a semmitmondó általános alakra esett.
  - A **cikk-komment** értesítés (adminoknak) szövege **eleve névtelen** volt: „Új hozzászólás érkezett egy cikkhez. Ellenőrizd a közösségi tartalmak között." — és a `targetType: article` miatt a koppintás a cikkre visz (nem a hozzászólásokra).
  - A **Chat-ág** a koppintásnál **dobta a `targetId`-t**: `LiveFeedScreen()`-t nyitott minden esetben, vagyis nem lehetett tudni, melyik üzenetről szólt.
- **A JAVÍTÁS — két oldal:**
  - **Szerveroldal (functions, azonnal élő, ehhez nem kell új app):** új tiszta modul **`functions/actor-name-plan.js`** (`pickActorName`, `actorNameOrGeneric`, 80 karakteres korlát) + **`resolveActorName(uid)`** az `index.js`-ben: **közösségi profil → nyilvános profil → Auth `displayName`**, minden olvasás best-effort (egy hiba nem bukhatja el a lájkot/kommentet). A `toggleChatReaction` ezt használja; a **cikk-komment** értesítés szövege mostantól: `„{név} hozzászólt egy cikkhez: »{rövid részlet}«”` — a név a hozzászóló **már betöltött** profiljából jön (nincs plusz olvasás). **Telepítve:** `npx firebase deploy --only functions` → **minden függvény „Successful update"**, a `toggleChatReaction` és az `articleComments` is a helyén (nodejs22, us-central1).
  - **App (AAB 357):** új tiszta modul **`lib/services/chat_focus_plan.dart`** (`chatFocusPlan`: `found` / `keepLoading` / `giveUp`, lap-korlát **10 lap = 300 üzenet**), a `LiveFeedScreen` új **`focusPostId`** paramétert kapott, a `NotificationCenterScreen` pedig a `targetType == 'chat'` ágban **átadja a `targetId`-t**. A képernyő a megtalált üzenethez **görget** (`Scrollable.ensureVisible`, ha kell a lista végére ugrálva) és **5 másodpercig kiemeli** (keret a közös `_withFocusHighlight` wrapperből — ugyanaz mindkét listaszakaszra).
- **BIZONYÍTÉK:** `flutter analyze lib test` **tiszta**; `flutter test` **879/879** (a kör előtt 863; **+16**: `chat_focus_plan_test.dart` **8** + `chat_focus_wiring_test.dart` **8** forrás-lint); `node --test functions/actor-name-plan.test.cjs functions/chat-notification-plan.test.cjs` **21/21**; `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN** (currentBuild **357**, 35 build-bejegyzés, a hash valós); a versionCode a merge-elt manifestből **357**; **a kód a csomagban:** a `libapp.so`-ban (arm64-v8a) megvan a `chatFocusPlan`, a `chat_focus_plan` és a `_withFocusHighlight` szimbólum.
- **⚠️ MIÉRT 357 (mérve):** a Play-mérés szerint a **356-ot a tulajdonos már feltöltötte a zárt tesztre** (`alpha = 356 completed 100%`), a **production sávon a 355** van, a `beta` sávon pedig a **354** — ezért **ugyanaz a verziókód nem használható újra**. A `docs/PLAY-KIADASI-JEGYZET.md` ennek megfelelően frissült: meta `currentBuild 357` / `lastPublishedBuild **355**`, **1b. blokk = 356–357 összesítő** (mert a nyilvános felhasználók a 355-tel a 352–356 szöveget kapták), új **357** bejegyzés a 3. pontban. **Csomag:** `build/HUHS-v1.0.0+357-release.aab` (81,38 MB, SHA-256 `019B54CCDB0B2EE52F13B493FEEB1DAE14E090844D0C6410B1ADCEF69F244B81`); a `docs/PLAY-PRODUCTION-BEADAS.md` §0 „következő csomag" sora is 357.
- **⚠️ ŐSZINTE KORLÁT:** (1) a **már beírt** értesítések szövege nem javul visszamenőleg (a `notifications` gyűjtemény bejegyzései megmaradnak) — a név az **új** lájkoknál/kommenteknél lesz ott; (2) az **odaugrás** csak akkor találja meg az üzenetet, ha az a **legfeljebb 300 legfrissebb** között van (ennél régebbinél a Chat a szokásos módon nyílik, nem görgetünk találomra); (3) a **cikk-komment** értesítés koppintása **továbbra is a cikket** nyitja (nem a hozzászólás-szakaszt) — ez most szándékosan nem változott; (4) a **címzett** oldali viselkedés (a név megjelenése és az odaugrás) **eszközön** még nincs igazolva: a tulajdonos készülékén mérhető, miután a 357-et feltöltötte a zárt tesztre (az iOS-oldali 357-es sideload külön kérésre megy).

### Hírlevél: ugyanarra a címre nem megy ki újra a megerősítő e-mail (2026-09-24, plugin **2.10.0** + AAB **356**)

- **A TULAJDONOS JELZÉSE:** *„hírlevél feliratkozásnál ugyanazt az email címet bármennyiszer be tudják küldeni és kimegy az ellenőrző mail is, kéne valami ellenörzés erre, ha már feliratkozott"* → kiegészítés: *„mailchimpre megy a feliratkozás"*, és *„az aab maradhat 356, ha kell új"*.
- **A MÉRT GYÖKÉR (a kódból):** a `POST /huhs/v1/newsletter/subscribe` (`.tmp-api-260/huhs-mobile-api/includes/newsletter.php`) a Mailchimp `lists/{audience}/members/{md5(email)}` állapotát **kiolvasta**, de **csak a `subscribed`** esetben állt meg. Ha a cím **`pending`** volt (kiment a megerősítő levél, de nem kattintottak rá), akkor **minden beküldés** egy új `PUT status=pending` hívást indított a Mailchimpre — az pedig **újra kiküldi** a megerősítő levelet. Így egy felületen **korlátlanul** lehetett levelet generálni ugyanarra a címre. **Ráadás:** ugyanezt a végpontot hívja a **szavazási ág** is (hozzájárulásnál, `voting_service.dart`), sőt a hívás **try/catch nélkül** volt, ezért egy hírlevél-hiba (pl. a régi IP-korlát `429`-e) **elvihette a szavazatot**.
- **A JAVÍTÁS — két oldal, mert a védelem a szerveren van, a szöveg a felületen:**
  - **Plugin 2.10.0** (`includes/newsletter.php`): **e-mailenkénti várakozás** (`huhs_newsletter_requested_{md5(email)}` transient, alapból **15 perc**, a `huhs_newsletter_resend_cooldown` szűrővel állítható). A várakozáson belül a **Mailchimp-hívást meg sem indítjuk**, ezért **nem megy ki levél**; a válasz viszont **sikeres** (`already_requested`, `state=confirmation_pending`, `retry_after`), hogy a régi app se lásson hibát. ⚠️ A várakozás **csak sikeres Mailchimp-válasz után** indul, tehát egy hibás kísérlet nem zárja ki a címest a valódi újrapróbálkozásból. A `subscribed` cím továbbra is **korán, Mailchimp-hívás nélkül** tér vissza (`already_subscribed`), az IP-korlát (`huhs_ip_rate_limited`, 5/60 s) **megmarad**, és a Mailchimp-kulcs továbbra is **csak a szerveren** van. A verzió **2.10.0** (fejléc + `HUHS_API_VERSION`).
  - **App (AAB 356):** új **tiszta modul** `lib/services/newsletter_plan.dart` — `newsletterResultFromResponse()` a három **sikeres** kimenetelt ismeri (`confirmationSent` / `alreadySubscribed` / `confirmationPending`), a `newsletterMessage()` pedig a hozzá tartozó magyar szöveget adja (a várakozásnál **felfelé kerekített percekkel**). A `wordpress_service.subscribeNewsletter` már **nem `void`**, hanem `NewsletterResult`; a `newsletter_screen.dart` ezt rajzolja ki, és a címet **csak akkor törli**, ha tényleg kiment a levél. A `voting_service.dart`-ban a hívás **try/catch-be** került: **a hírlevél hibája nem viheti el a szavazatot**.
- **BIZONYÍTÉK:** `flutter analyze lib test` **tiszta**; `flutter test` **863/863** (a kör előtt 849; **+14**: `test/services/newsletter_plan_test.dart` **10** + `test/services/newsletter_wiring_test.dart` **4** forrás-lint); `node --test functions/newsletter-cooldown.test.cjs` **7/7**; `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN** (currentBuild **356**, a dokumentumban szereplő SHA-256 valós); **1 aláírási identitás**. **A kód a csomagban:** a `libapp.so`-ban **mindhárom ABI-nál** megvan a `newsletterResultFromResponse`, a `newsletterMessage`, a `confirmation_pending` és az `already_requested` szöveg.
- **⚠️ MUTÁCIÓS BIZONYÍTÉK (a saját tesztem első változata GYENGE volt):** az első változat csak a **forrás-pozíciókat** hasonlította (a korai visszatérés előbb van, mint a Mailchimp-hívás), és **átengedte** azt a mutációt, amely a kaput kikapcsolja (`if (false && $cooldown > 0 …)`) — vagyis a teszt zölden hazudott volna. Ezért a teszt **a feltétel pontos alakját** is elvárja (`if ($cooldown > 0 && $last_requested > 0) {` + a transient kiolvasása). **A bizonyíték:** mutált kódon **6/7** (a várakozás-teszt bukik), a visszaállítás **bájtpontos** (`Copy-Item` a mentésből, SHA-256 `097252BC…` egyezik), utána újra **7/7**.
- **⚠️ KÉT ELAVULT, VERZIÓHOZ KÖTÖTT TESZTET IS JAVÍTANOM KELLETT (a 2.8.0 kör óta pirosban voltak):** a `functions/prize-active-payload.test.cjs` a **2.8.0**-t, a `functions/artist-profile-plan.test.cjs` a **2.7.0**-t követelte meg **pontosan** — ezért minden plugin-emelésnél elhasaltak. Mindkettő (és az `artist-private-email.test.cjs` is) mostantól **„legalább X.Y.0"**-t ellenőriz, és azt, hogy a **fejléc verziója és a `HUHS_API_VERSION` konstans egyezik**. **Tanulság: plugin-verziót ne pinneljünk pontosra egy forrás-lintben.**
- **Csomagok:** **`build/HUHS-v1.0.0+356-release.aab`** (versionCode **356**, 81,36 MB, SHA-256 `AE0563A7A35B7D3E0869DB810CBF5ACF9DD480F011E97054992EFCED6D618430`) **és `build/huhs-mobile-api-2.10.0.zip`** (45 fájl, 150,7 KB, SHA-256 `6D95DE5AA65FA9CEB94A5410B612F41AA62F51F78C9AEF0D560E030A2998442F`). A plugin különbsége átnézhető: **`docs/plugin-2.10.0-newsletter-cooldown.patch`** (2 fájl, +30/−4 sor: `includes/newsletter.php`, `huhs-mobile-api.php`).
- **⚠️ A 356-OT KÉTSZER ÚJRAÉPÍTETTEM (ugyanaz a verziókód, MERT A PLAY MÉRÉSE SZERINT MÉG NEM KERÜLT FEL):** a `node tools/check-play-track.mjs --hashes` szerint a feltöltött AAB-ek listája **355-nél végződik** (a zárt teszt sávján a **355** fut `completed` 100%-kal, a kiadási szöveggel) — vagyis a korábban kihirdetett 356 (`4C30E378…`, majd `5C8A5487…`) **nem került fel**, ezért a hírlevél-javítás **belekerült a 356-ba**, és **a 356-ot kell feltenni** (a 355-öt nem). ⚠️ **A második újraépítés a saját hibám miatt kellett:** az app-changelogba először **megismételtem a 355/354 sorait**, amitől a `test/widgets/about_changelog_test.dart` elbukott (ugyanaz a szöveg kétszer szerepelt a listában) — a changelog mostantól **csak a 356 saját újdonságát** (hírlevél) tartalmazza. **Ha a Play mégis elutasítja a 356-ot „már használt verziókód" miatt, akkor 357-et kell építeni** — az a 356 tartalma, csak új számmal.
- **📌 A TULAJDONOS LÉPÉSE:** (1) a **`build/huhs-mobile-api-2.10.0.zip`** feltöltése a WordPressre (**a 2.9.0-t cseréli**; ebben a 2.8.0 és a 2.9.0 javítása is benne van) — **enélkül a hírlevél továbbra is korlátlanul küldi a megerősítő levelet**; (2) a **`build/HUHS-v1.0.0+356-release.aab`** feltöltése a zárt teszt sávra; (3) eszközpróba a **Hírlevél menüpontban** (Több → Hírlevél): kétszer egymás után ugyanazzal a címmel feliratkozni → a második válasz azt írja, hogy már kiment. **✅ A tulajdonos ezt meg is tette: a plugin 2.10.0 fent van, a 356 pedig feltöltés alatt a zárt teszt sávra.**
- **✅ iOS-RE IS FELMENT A 356 — MÉRVE (2026-09-24 este):** a tulajdonos kérésére (*„mehet fel ez a 356 iphone sideloaddal is"*) a **meglévő CI-út** futott, kódváltozás nélkül: a `1638a871` push → **GitHub Actions `iOS fordítás-ellenőrzés` (run `36062243273`) zöld** (9m3s) → az artifact (`HUHS-ios-unsigned-ipa`) letöltve: **`build/ios-ipa-356/Runner-unsigned.ipa`, 26 282 400 bájt** → `node tools/verify-ios-ipa.mjs build/ipa-356/x/Payload/Runner.app --test-ads --contains=newsletterResultFromResponse --contains=newsletterMessage --contains=confirmation_pending --contains=already_requested` → **MINDEN ELLENŐRZÉS RENDBEN** (a teszt-AdMob-egységek is). **A telepítés bizonyítéka a Sideloadly adatbázisából** (`%LOCALAPPDATA%\Sideloadly\installations.db`, másolatból olvasva, mert a program zárolja): `installations.id=15`, `23:53:27 → 23:53:44` (17 s), eszköz *Imre iPhone-ja* (`00008030-001475413ED2402E`), Apple ID `djdeeroy@gmail.com`, **`final_bundle_id = hu.hungarianhardstyle.app.JQPJ793V65`** (változatlan → **valódi frissítés**, az app-adatok és az App Check debug token megmaradnak), **`last_error` üres**, `failures_count = 0`; a feltöltött fájl **MD5-je `b7b74a82182709dc33787155d44fb34e` = pontosan a letöltött IPA**. ⚠️ **Amit NEM tudtam megmérni:** a telefon a telepítés után **lecsatlakozott USB-ről** (`pymobiledevice3 usbmux list` → `[]`), ezért a telepített app **`CFBundleVersion`-jét (356)** és az indítás+képernyőkép igazolást nem tudtam megcsinálni — ez a kábel visszadugása után két perc. Az ingyenes aláírás **7 napig** érvényes (a DB szerint `known_ttl=7`). **⚠️ A Sideloadly GUI-only:** parancssori kapcsolója nincs az IPA-hoz/Apple ID-hoz (a binárisban **nincs** `--ipa`/`--appleid` szöveg), ezért a folyamat úgy ment, hogy az `sideloadly.exe "<ipa>"` elindult (a fájl betöltve, az eszköz és a mentett Apple-ID session felismerve), majd **a Start gombot `SendKeys`-szel nyomtam meg**, és a képernyőképen látszott a **100% / „Done."** — utána az adatbázis igazolta.
- **⚠️ ŐSZINTE KORLÁT:** a **Mailchimp viselkedését nem tudom innen mérni** (a Mailchimp-kulcs csak a szerveren van, és a levélküldést nem tudom megfigyelni): a gyökér a **kódból** olvasott útvonal, a tulajdonos pedig a **jelenséget** mérte (minden beküldésre kiment a levél). A javítás a Mailchimp-hívást **el sem indítja** a várakozáson belül — ezért akkor is megszünteti a dupla levelet, ha a Mailchimp oldalán bármi más a pontos viselkedés. **Amit a plugin feltöltése UTÁN lehet mérni:** kétszer egymás után ugyanaz a `POST /newsletter/subscribe` → a második `already_requested` + `retry_after` (innen, a tulajdonos címével — a kód és a dokumentáció készen áll rá). A PHP-szintaktika **nem futott** (`php -l` nincs a gépen) — a zárójel-egyensúly ellenőrzött, a fájlok **BOM nélküli UTF-8**-ak, és a csomag a **2.10.0** verziót tartalmazza.

### A Play „3 művelet javasolt" a 355-höz — kettő GOOGLE-KÖNYVTÁR, egy a miénk (2026-09-24, mérés; **kód NEM változott**)

- **A TULAJDONOS JELZÉSE:** képernyőképek a Console-ból a **355**-ös kiadásról: *„3 művelet javasolt"*, és a kérdés: *„ezt meg lehet oldani úgy, hogy ne törjön el az app?"* A három kártya: (1) **teljes képernyős mód** (edge-to-edge, Android 15), (2) **elavult API a teljes képernyős megjelenítéshez**, (3) **bittérkép-optimalizálás** (memóriahasználat).
- **A MÉRT ALAPÁLLAPOT (a 355-ös csomagból):** `android:targetSdkVersion="36"` (a merge-elt manifestből), `androidx.activity` **1.12.4**, `androidx.core` **1.18.0**, és a **manifestben NINCS** `windowOptOutEdgeToEdgeEnforcement`; a saját kódban **egyetlen** `SystemChrome` / `enableEdgeToEdge` / `setDecorFitsSystemWindows` hívás sincs. A felület **26 helyen használ `SafeArea`-t** (a fő shell is: `main_navigation.dart`), és külön `ScrollBottomInset`-ünk van az alsó sávra.
- **1) AZ EDGE-TO-EDGE KÁRTYA — EZ AZ EGYETLEN, AMIBEN TUDUNK LÉPNI:** az **Android 15+ a targetSdk 35 felett kötelezően teljes képernyős**, nálunk targetSdk **36**, tehát **a készüléken ma is így fut** — és **nem tört el**, mert a felület kezeli a rendszersávokat. A Play a **`enableEdgeToEdge()`** meghívását kéri a **visszamenőleges kompatibilitásért** (hogy Android 14 és alatta is ugyanígy induljon). Ez **egy sor** a `MainActivity`-ben, és **Android 15/16-ön semmit nem változtat** — ott ma is ez a viselkedés; a kockázat a **régebbi** verziókon van, amit a 26 `SafeArea` és a fő shell véd, de **egy régebbi készüléken érdemes ránézni**. ⚠️ A „temporary opt-out" (`android:windowOptOutEdgeToEdgeEnforcement`) **nem megoldás**: targetSdk 36 mellett a rendszer figyelmen kívül hagyja.
- **2) AZ „ELAVULT API" KÁRTYA — GOOGLE-KÖNYVTÁR, NEM JAVÍTHATÓ (mérve):** a Console pontosan megnevezte: **`LAYOUT_IN_DISPLAY_CUTOUT_MODE_SHORT_EDGES`**, „ahol kezdődik: **`qp2.j`**".
  - A `qp2` az R8-mappingből: **`androidx.core.text.PrecomputedTextCompat$Params$$ExternalSyntheticApiModelOutline4`** — vagyis egy **R8 szintetikus API-model outline osztály** (az R8 több ilyet is **összevonhat** egy osztályba, ezért a név **félrevezető**; ugyanaz a tanulság, mint a bittérkép-kártyánál).
  - **A valódi hivatkozás mérve:** az `androidx.core` **1.18.0**-ban **egyetlen** osztály írja/hivatkozza a `layoutInDisplayCutoutMode` mezőt: **`androidx.core.view.WindowCompat`**. A **legfrissebb 1.19.1**-ben (Google Maven, letöltve és összevetve) **ugyanaz az osztály, ugyanaz a hivatkozás** → **library-emeléssel NEM tűnik el**.
  - **A `javap`-os bizonyíték (1.19.1, `WindowCompat.class`):** `getfield android/view/WindowManager$LayoutParams.layoutInDisplayCutoutMode:I` majd `putfield` ugyanerre + `Window.setAttributes(...)` — tehát a `WindowCompat` **ténylegesen beállítja** ezt a paramétert. (A saját kódunk nem hívja: nincs se `WindowCompat`, se `SystemChrome` hívásunk — az AndroidX-stack hozza, amit az activity/fragment/ads/media3 egyaránt behúz.)
  - **Amit ez jelent:** a paraméter az **edge-to-edge-hez épp a kívánt** viselkedést írja le (a cutout-területre is rajzolhat az app), az Android 15 viszont **deprecaltnak** jelöli, ezért a Play statikus elemzése kifogásolja. **Nem hiba, nem blokkoló** — és **amíg a Google nem módosítja a `WindowCompat`-ot, a kártya ott marad**. Ez pontosan ugyanaz az esetosztály, mint a **bittérkép-kártya** (AdMob SDK) és a korábbi **SafetyNet**-javaslat: külső könyvtár kódja.
- **3) A BITTÉRKÉP-KÁRTYA:** már kivizsgálva (lásd a lentebbi szakaszt) — **Google Mobile Ads SDK**, a legfrissebb 25.5.0-ban is benne van, nálunk nem javítható.
- **ÖSSZEGZÉS A TULAJDONOS KÉRDÉSÉRE („meg lehet oldani úgy, hogy ne törjön el?"): IGEN, de csak az első kártyára érdemes lépni** — a `enableEdgeToEdge()` **egy sor**, Android 15/16-ön **nem változtat semmit**, és pontosan azt teszi, amit a Play kér. A másik kettő **Google-könyvtárak** kódja (androidx.core, AdMob), ott a helyes válasz a **mérés rögzítése** és az **újramérés a következő függőség-emelésnél** — kódváltozás nélkül.
- **⚠️ ŐSZINTE KORLÁT:** a Play **statikus** elemzése a **becsomagolt** kódot nézi, nem a futásidejű hívásokat — ezért jelezhet olyan útvonalat is, amit a mi appunk sosem hív. Azt, hogy a `WindowCompat` cutout-útja **futásidőben** lefut-e nálunk, innen **nem tudom megmérni** (eszközös mérés kellene). Amit igazoltam: a hivatkozás **pontos helye** (`WindowCompat`), hogy a **legfrissebb** androidx.core-ban is megvan, és hogy **a saját kódunkban nincs** ilyen hívás.

### Az edge-to-edge kártya **MÉRVE**, majd **BEÉPÍTVE** — AAB **356** (2026-09-24; Android 14-en a `enableEdgeToEdge()` nem változtat a felületen)

- **A TULAJDONOS DÖNTÉSE (kérdező ablak, két lépésben):** először *„Mérd meg előbb API 34 emulátoron, aztán döntünk"*, majd a mérés után *„Tegyük be most, és a feltöltésre szánt csomag a 356 legyen"*. Ez a szakasz a mérést **és** a beépítést tartalmazza.
- **A MÓDSZER (két csomag, ugyanaz a forrás, ugyanaz az emulátor):** letöltöttem a **`system-images;android-34;google_apis_playstore;x86_64`** képet (a gépen eddig **csak API 35** volt), és létrehoztam a **`HUHS_API34`** AVD-t (Pixel 8 profil, 1080×2400, 420 dpi). Ezután **két release APK** készült ugyanabból a forrásból: **alap = a mostani 355-ös kód**, **mérőváltozat = + `onCreate` felülírás `enableEdgeToEdge()`-vel** (import `androidx.activity.enableEdgeToEdge`, a hívás a `super.onCreate` ELŐTT — ez az Android-dokumentáció mintája). Mindkettőt ugyanarra az emulátorra telepítettem, és **ugyanazt a rögzített koppintás-sort** futtattam (`adb shell input tap`): Kezdőlap → Hírek → Több → DJ-k.
- **⚠️ A MÉRÉS KÖZBEN TANULT DOLGOK (hogy a következő kör ne szenvedje ezt újra):** (1) az **induló értesítés** (a WP-ből jövő *KIEMELT ESEMÉNY* kártya, 2 órás cooldown-nal) **elsötétíti a felületet**, és tönkreteszi az összehasonlítást — ezért **NINCS `pm clear`** (a cooldown csendben marad), és van egy **elsötétítés-kapus X-kereső** (`find-close.ps1`), ami csak akkor koppint, ha a képernyő tényleg sötétített; (2) a `pm clear` **felhozza a rendszer értesítés-engedély dialógusát** (ez is sötétít) → az engedélyt `pm grant`-tal előre megadjuk; (3) az emulátort **`-memory 4096 -cores 4`**-gyal kell indítani, és **kétszer magától leállt** a mérés alatt (ezért rövid, 4 képernyős futás lett a hosszú helyett); (4) a képernyőképet **`screencap` + `pull`** adja helyesen.
- **MÉRT EREDMÉNY 1 — a hívás TÉNYLEG lefut (futásidejű ablak-attribútum):** a `dumpsys window` szerint az app ablaka:
  - **alap:** `mAttrs={(0,0)(fillxfill) sim={adjust=resize forwardNavigation} ty=BASE_APPLICATION fmt=TRANSLUCENT …}`
  - **mérőváltozat:** `mAttrs={… layoutInDisplayCutoutMode=always …}` ← **megjelent a cutout-mód**, tehát az `EdgeToEdge.enable()` tényleg végrehajtódott (a `wanim` értéke is más: `0x1030001` → `0x10302fe`).
- **MÉRT EREDMÉNY 2 — a FELÜLET NEM VÁLTOZIK (ez a lényeg a „ne törjön el" kérdésre):**
  - **Kezdőlap és Hírek:** a képernyő **sorfolyama pixelre ugyanaz** mindkét buildben — a felső sáv `y=0…131` `rgb(10,10,12)`, az app háttere `y=132…2134` `rgb(13,14,16)`, a rádiósáv `y=2138…2336` `rgb(18,24,32)`, a rendszer-navigációs sáv `y=2337…2399` `rgb(0,0,0)`, a gesture-pill `y=2364…2373`.
  - **Több képernyő:** a **tartalom sorai bitre ugyanott vannak** (`y=360, 419, 494, 534, 772, 899, 933, 1245, 1262, …, 2138, 2337, 2364, 2374`), **egyetlen** eltérés van: az alap buildben a **statuszsáv sávja** (0…131) az app saját hátterét mutatta, a mérőváltozatban ugyanaz **~11%-kal sötétebb** (`13,14,16` → `10,10,12`) — ez a rendszer státuszsáv-árnyalata, **nem elmozdulás**.
  - **Amit ebből tudunk:** az app a **statuszsáv ALÁ már most is rajzol** (az alap buildben is, `enableEdgeToEdge()` nélkül) — a Flutter/AndroidX-stack már edge-to-edge-ben fekteti ki a nézetet; a hívás ehhez képest **csak a cutout-módot** állítja át, és a **régi Androidokon sem mozdít el semmit**. Android 15/16-ön a rendszer **amúgy is kötelezően** ezt csinálja (targetSdk 36), tehát ott sem változik semmi — ezt a tulajdonos készüléke a 353/355-tel **már bizonyította**.
- **A KÓD BEÉPÍTVE — A TULAJDONOS DÖNTÉSE (2026-09-24, kérdező ablak):** *„Tegyük be most, és a feltöltésre szánt csomag a 356 legyen"*. Ezért:
  - `android/app/src/main/kotlin/hu/hungarianhardstyle/app/MainActivity.kt` — **`import androidx.activity.enableEdgeToEdge` + `import android.os.Bundle`**, és az `onCreate` felülírás a `super.onCreate` ELŐTT hívja az `enableEdgeToEdge()`-t (a hívás mellett a mérés számaival kommentelve, hogy a következő olvasó lássa, miért biztonságos). A bázis (`AudioServiceFragmentActivity`) **változatlan**.
  - **`pubspec.yaml`:** `1.0.0+355` → **`1.0.0+356`**; a `lib/data/app_changelog.dart` legfelső bejegyzése **356**-ra állítva (a felhasználó számára ugyanaz a tartalom — a hívás nem látható —, de a changelog a telepített verzióhoz illeszkedik).
  - **⚠️ A BUILD ELSŐRE ELHASALT, ÉS EZ TANULSÁG:** a `Bundle` import kimaradt, ezért a Kotlin-fordító *„'onCreate' overrides nothing / Unresolved reference 'Bundle'"* hibát adott — a `flutter build appbundle` **elhasal**, nem néma. A javítás után a build zöld.
  - **A 356-OS CSOMAG (ez az ELSŐ 356-os build volt — a végleges, hírlevél-javítást is tartalmazó csomag hash-e a legfelső szakaszban van):** `build/HUHS-v1.0.0+356-release.aab` (versionCode **356** a merge-elt manifestből, 81,36 MB, SHA-256 `4C30E3789AECDDC4F596FBC54338BAB91BD7272FC3FD056213AB626B485D49EE`). **A kód bizonyítottan benne van:** az R8-mappingben `hu.hungarianhardstyle.app.MainActivity` alatt megjelent a **`void onCreate(android.os.Bundle):44:44`** metódus, és az `androidx.activity.EdgeToEdge` osztály **bennmaradt** a csomagban (nem dobta ki a csökkentés).
  - **Kapuk:** `flutter analyze lib test` **tiszta**; `flutter test` **849/849**; `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN** (currentBuild **356**, az AAB létezik, a dokumentumban szereplő SHA-256 valós, **34 build-bejegyzés**, nincs helykitöltő); **1 aláírási identitás**.
  - **A dokumentumok frissítve:** `docs/PLAY-KIADASI-JEGYZET.md` (meta: currentBuild **356** + új AAB + hash; a „miért a 356-ot" szakasz; **1b. blokk = 352–356 összesítő**; új **356** bejegyzés a 3. pontban, a **355** bejegyzése megmaradt, mert a Play-ellenőrzés minden kihagyott buildhez tételes bejegyzést kér) és `docs/PLAY-PRODUCTION-BEADAS.md` §0 („következő csomag" = **356** + plugin **2.9.0**). **A Play-szöveg NEM változott** (a hívás a felhasználónak nem látható).
  - **📌 A TULAJDONOS LÉPÉSE EBBŐL:** a **356** feltöltése a zárt teszt sávra (**a 355-öt nem kell feltenni** — a 356 tartalmazza), és a **plugin 2.9.0** feltöltése a WordPressre.
- **⚠️ ŐSZINTE KORLÁT:** a mérés **emulátoron** történt (x86_64, API **34**, Pixel 8 profil) — **valódi régi készüléken nem**, és **API 33 vagy alatta nem** (ahhoz külön rendszerkép kellene). Azt, hogy a Play **leveszi-e a kártyát** a 356-tól, csak a következő **feltöltés után** lehet látni. **A bizonyíték a repóban:** `tmp/edge34/base/*.png` (alap), `tmp/edge34/e2e/*.png` (mérőváltozat), `tmp/edge34/dump-base.txt` + `tmp/edge34/e2e/dumpsys.txt` (ablak-attribútumok), valamint a futtatható szkriptek (`flow-short.ps1`, `find-close.ps1`, `colscan.ps1`, `edge-compare.ps1`).

### A DJ **privát e-mail címe** látható és javítható (2026-09-24, plugin **2.9.0** — **AAB NEM kell hozzá**)

- **A TULAJDONOS JELZÉSE:** *„kérésre nem tudom átírni egy artist/dj privát mail címét se a WP Apiban se a natív huhs vezérlőben. Meg látni se látom sehol, pedig igény lenne mindkettőre, főleg ha elbaszta a delikvens."*
- **A MÉRT GYÖKÉR (kód szinten, három helyen hiányzott ugyanaz a mező):** a `contact_email` post meta a **privát (kapcsolattartó)** cím a `huhs_artist` bejegyzésen — ez az, ami a **beküldésből átkerül**, és **az adatlap átvételét is igazolja** (a claim e-mail-egyeztetése ezt is elfogadja). Ez a mező:
  1. **a natív admin mezőlistájából hiányzott** (`huhs_admin_resource_fields('huhs_artist')` az `api-admin.php`-ban — pedig a `booking_email` **benne volt**, tehát a séma készen állt),
  2. **a WordPress admin DJ-meta-boxából szintén** (`huhs_artist_meta_box_callback`, `artists.php` — a `booking_email` ott is látszott, a privát nem),
  3. **a WP admin mentése sem ismerte** (`artist-save.php` mezőlistája).
- **A JAVÍTÁS (4 fájl, összesen 6 sor):** `includes/api-admin.php` — a mező bekerült a DJ admin-mezőlistába `type: 'email'`-lel (a mentés így `sanitize_email`-en megy át, `huhs_admin_encode_meta_value`); `includes/artists.php` — külön mező a WP meta-boxban, **jelöléssel, hogy NEM publikus**; `includes/artist-save.php` — a mentett mezők listájában + a `sanitize_email` ágban; `huhs-mobile-api.php` — verzió **2.9.0**.
- **⚠️ AMIÉRT EZ NEM KÉR ÚJ APPOT:** a **natív admin űrlapja szerver-vezérelt** — a `wordpress_admin_screen.dart` a mezőket az `action=resource` válaszából rajzolja (`_adminField`), és a `type: 'email'` már támogatott. Ezért a mező **a következő megnyitáskor megjelenik**, app-frissítés nélkül. **Ez a kör tehát tisztán plugin-oldali: nincs AAB, nincs Play-feltöltés.**
- **⚠️ AMIT SZÁNDÉKOSAN NEM VÁLTOZTATTUNK:** a **nyilvános** adatlap válasza (`huhs_build_artist_response`) továbbra sem tartalmazza a privát címet — az csak a **védett `claim-emails`** végponton jön ki. Ezt **forrás-lint őrzi** (`functions/artist-private-email.test.cjs`, **6/6**): a natív admin listában benne van, a WP meta-boxban benne van, a mentés sanitize_email-lel ír, a nyilvános payloadban **nincs benne**, a claim-emails végponton **igen**, és a verzió 2.9.0.
- **Csomag:** **`build/huhs-mobile-api-2.9.0.zip`** (45 fájl, 150,2 KB, SHA-256 `B6B1C1F96D6E7869800B502F24F2C7F964FA1D91E08EAE32166C6580116B4CA9`); a 2.8.0-hoz képest **pontosan 4 fájl** változott (teljes fa-összevetéssel ellenőrizve), a különbség átnézhető: **`docs/plugin-2.9.0-private-email.patch`**. A `docs/PLAY-KIADASI-JEGYZET.md` §4 kiadásjegyzéke 2.9.0-ra frissült.
- **📌 A TULAJDONOS LÉPÉSE:** a **plugin 2.9.0 feltöltése** a WordPressre (Bővítmények → Új hozzáadása → feltöltés → „Csere a feltöltött verzióra"). Utána a **natív vezérlőben**: DJ-k → a DJ kiválasztása → a **„Privát e-mail (kapcsolattartó…)"** mező kitöltése/javítása → mentés. Ugyanez a mező a **WP admin** DJ-adatlapján is megjelenik („Fellépés kérése" szakasz).
- **⚠️ ŐSZINTE KORLÁT:** a PHP-szintaktika **nem futott** (`php -l` nincs a gépen) — a zárójel-egyensúly mind a négy érintett fájlban ellenőrzött, és a fájlok **BOM nélküli UTF-8**-ak; a viselkedés a **plugin feltöltése után** mérhető (a mező megjelenik, a mentés átírja a címet, és a DJ ezzel a címmel **át tudja venni** az adatlapját). **Eszköz a mérésre:** `node tools/check-artist-claims.mjs --scan-emails` — kilistázza, melyik DJ-nél van claimhez használható cím (maszkolva), és a javítás után az érintett DJ-nek meg kell jelennie a listában.

### A nyereményjáték: a leírás a játék alatt + azonnali nyitás (2026-09-24, AAB **355** + plugin **2.8.0**)

- **A TULAJDONOS KÉT JELZÉSE (ugyanarra a képernyőre):** *„a nyereményjátékjba nem kerül bele a játék leírása"* és *„100 év mire betölt"* — kérdező ablakban megerősítve, hogy a lassú képernyő **a most futó nyereményjáték**.
- **1) A LEÍRÁS — a gyökér KETTŐS volt (mindkettő mérve):**
  - **A WordPress** a `huhs_prize_api_active()` **nyitott** ágában **szándékosan üres stringet** küldött: `'prize_type' => ''`, `'prize_description' => ''` (`includes/prize.php`), és csak a **sorsolás után** adta ki a metaértékeket. **Élőben igazolva:** a most futó játék (`id=12852`, *„Mi Noisecontrollers igazi neve?"*) válaszában `prize_type=''`, a leírás **0 karakter**. ⚠️ A mező saját súgója viszont azt ígéri: *„Részletek, amit a nyertes és a játékosok látnak"* — vagyis a két viselkedés **ellentmondott egymásnak**, és a tulajdonos elvárása a súgóval egyezik.
  - **Az app** a `_buildOpen` nézetben **egyáltalán nem** rajzolta ki ezeket — csak a `_buildDrawn` (nyertes) nézet tette. Tehát **hiába küldte volna a szerver**.
  - **A javítás:** plugin **2.8.0** — a nyitott ág is kiküldi a `_huhs_prize_type` / `_huhs_prize_description` metát; az appban pedig új **`lib/widgets/prize_reward_details.dart`** (`PrizeRewardDetails`) jeleníti meg, **ugyanaz** a widget a nyitott játékban és a nyertes mellett (nem másolódik). A **helyes válasz** továbbra sem megy ki — ezt a **`functions/prize-active-payload.test.cjs` (6/6)** őrzi.
- **2) A LASSÚ BETÖLTÉS — a mért ok a képernyő szabálya volt:** a válaszlehetőségek **csak akkor** jelenhettek meg, ha a szerver kifejezetten azt mondta, hogy ez a fiók **még nem játszott** — ezért **minden megnyitás megvárta a teljes szerver-körutat** (app → Cloud Function → WordPress). **Ami mérve gyors, az nem itt volt:** a nyilvános végpontok hidegen 1,0–1,2 s / melegen 0,38–0,45 s, a `/prize/active` 412–710 ms, a `prizeVote` callable melegen 162–347 ms — a **hideg** callable-viselkedés viszont korábban **4035 ms** volt mérve, és ez a „100 év" élmény.
  - **A javítás — ugyanaz a minta, mint a cache-körben:** a `VoteMemory` mostantól a **„még nem játszottál" szerver-választ is megjegyzi** (`markPrizeChecked`), a `prizePlayProvider` pedig **azonnal** a mentett válaszból rajzol, és a háttérben ellenőriz. **⚠️ Ez soha nem tipp:** kizárólag a `prizeVote` válasza kerülhet a mentésbe, ezért a válaszlehetőségek továbbra is valódi szerver-válaszon alapulnak (legfeljebb régebbin); eltérésnél a jelzés frissül (a `_revalidatePlay` **mindkét irányt** kezeli).
  - **⚠️ KÖZBEN A SAJÁT KORÁBBI HIBÁNK IS ELŐKERÜLT:** a `markPrizePlayed`/`markPrizeChecked` a memóriabeli tükröt a **lemez után** írta, ezért egy mikrotasknyi ablakban a **régi** értéket lehetett visszaolvasni (ezt egy provider-teszt buktatta el). A sorrend mostantól **tükör előbb, lemez utána** — ugyanaz a szabály, mint a törléseknél.
- **BIZONYÍTÉK:** `flutter analyze lib test` **tiszta**; `flutter test` **849/849** (a kör előtt 837; **+12**: `test/widgets/prize_reward_details_test.dart` **6** + a `prizePlayProvider`/`VoteMemory` új esetei); `node --test functions/prize-active-payload.test.cjs` **6/6**; `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN** (1. blokk **171/500**, 1b. blokk **406/500**); a versionCode a merge-elt manifestből **355**; **a kód bizonyítottan a csomagban van:** a `libapp.so`-ban mindhárom ABI-nál megtalálható a `PrizeRewardDetails`, a `markPrizeChecked` és a `prize_description` szimbólum.
- **Csomagok:** **`build/HUHS-v1.0.0+355-release.aab`** (versionCode **355**, 81,33 MB, SHA-256 `F8D2132D83CC69D2378887181CE23FA0BED2AE218D8E9645E806A8921860AF82`) **és `build/huhs-mobile-api-2.8.0.zip`** (45 fájl, 149,5 KB, SHA-256 `595775ACE6E90EA7251CBAE1C19A74BE369F853500BFB4AA012A16CF4AF31476`). A plugin különbsége átnézhető: **`docs/plugin-2.8.0-prize-description.patch`** — a teljes fa összevetése a 2.7.0-ás ZIP-pel **pontosan 2 módosított fájlt** mutat (`huhs-mobile-api.php` verzió, `includes/prize.php` javítás). **A függvények nem változtak** (nincs `firebase deploy`).
- **📌 A TULAJDONOS LÉPÉSE:** (1) a **plugin 2.8.0 feltöltése** a WordPressre — **enélkül a leírás továbbra sem megy ki**; (2) a **355 feltöltése** a zárt teszt sávra (`Könyvtárból`/feltöltés) — **a 354-et nem érdemes külön feltenni, a 355 tartalmazza**; (3) eszközpróba: nyereményjáték megnyitása (azonnali nyitás + látszik-e a leírás), majd egy DJ-adatlap (Goze) az „Megjelenései" szakasszal.
- **⚠️ ŐSZINTE KORLÁT:** eszközön mért igazolás nincs (a gyorsulás és a leírás megjelenése a tulajdonos készülékén mérhető); a leírás **csak akkor látszik, ha a WordPress adminban ki van töltve** a „Nyeremény leírása" mező, és a plugin 2.8.0 fent van. A plugin PHP-szintaktikája **nem futott** (`php -l` nincs a gépen) — a zárójel-egyensúly ellenőrzött (78/78), és a fájl **BOM nélküli UTF-8** (a PowerShell-írás BOM-ot tett rá, azt visszavontam).

### KÉRDÉS ELŐRE: a kiadvány-feltöltés és az iOS-bolt — MI MŰKÖDIK MA, és mi kellene (2026-09-24, kutatás; **kód NEM változott**)

- **A TULAJDONOS KÉRDÉSE:** *„Ugye most úgy működik, ha feltöltök egy kiadványt, akkor beküldi a google playbe is — meg lehet oldani, hogy az iphone-os boltba is beküldje majd, ha kész a developer acc?"* és *„munka még nem kell hozzá"* → **ez a szakasz a válasz, nem implementáció.**
- **1) AMI MA TÖRTÉNIK (mérve a kódból, nem emlékezetből):** a WordPress a **hangfeldolgozás sikeressége után azonnal** beír egy kérést a Firestore-ba (`label_product_sync_requests/{requestId}`), azt a **`syncQueuedWordPressLabelProducts`** (`onDocumentCreated`) veszi fel, és lefuttatja a **`runWordPressLabelSync(releaseId)`**-t; mellette **biztonsági háló** az 5 percenkénti **`syncWordPressLabelProducts`** (`onSchedule`, 300 s keret, `sync_locks/label_product_sync` zár — a 60 s-os keret **mérten kevés** volt, 24 órában ~475 időtúllépés). A lényeg a **`upsertPlayProduct(...)`**: a **Play Developer API**-n létrehozza/PATCH-eli a terméket **árral, a 9 ország régióival és a vásárlási opciókkal**. Vagyis **a kiadvány feltöltése magától létrehozza a Play-termékeket** — a Play Console-ban **nem** kell kézzel terméket gyártani. **Kézzel marad:** az **app** (AAB) feltöltése és a kiadási szöveg. ⚠️ Az **„upcoming"** kiadvány termékei szándékosan **DRAFT**-ban maradnak (mérve: a 12699-nél 4 db), ezért ott a megjelenésig **nem vásárolható** — ezt a megjelenés napján a szinkron aktiválja.
- **2) A VÁLASZ AZ iOS-RE: IGEN, megoldható ugyanezzel a mintával** — az **App Store Connect API** ma már a **teljes életciklust** tudja programból (Apple dokumentáció, 2026):
  - **létrehozás:** `POST /v2/inAppPurchases` (név, `productId`, típus, `reviewNote` + az app kapcsolata) → `MISSING_METADATA` állapot;
  - **listázás / módosítás:** `GET /v1/apps/{id}/inAppPurchasesV2`, `PATCH /v2/inAppPurchases/{id}` (a `name`, `reviewNote`, `familySharable` szerkeszthető);
  - **megjelenítendő szövegek:** az új, verzió-alapú út: `POST /v1/inAppPurchaseVersions` → `POST /v2/inAppPurchaseLocalizations` (locale + név + leírás);
  - **ár:** `GET /v2/inAppPurchases/{id}/pricePoints?filter[territory]=…` → **`POST /v1/inAppPurchasePriceSchedules`** (árpont-azonosítóval); ⚠️ egy **áremelést** az Apple nem enged visszavonni;
  - **elérhető országok:** külön „In-App Purchase availability" erőforrás;
  - **beküldés:** `POST /v1/reviewSubmissions` (platform `IOS`) → `POST /v1/reviewSubmissionItems` (az IAP-verzió hozzákötése) → `PATCH /v1/reviewSubmissions/{id}` `submitted: true`; az állapot `PREPARE_FOR_SUBMISSION` → `READY_FOR_REVIEW` → `WAITING_FOR_REVIEW` → `IN_REVIEW` → `APPROVED`/`REJECTED`;
  - **kötelező melléklet a beküldéshez:** egy **review-képernyőkép** (`POST /v1/inAppPurchaseAppStoreReviewScreenshots` → `PUT` feltöltés → `PATCH` véglegesítés);
  - **jogosultság:** `ACCOUNT_HOLDER` / `ADMIN` / `APP_MANAGER` szerepű **API-kulcs** kell hozzá.
- **⚠️ A NÉGY DOLOG, AMI MIATT AZ iOS NEM TÜKÖR-MÁSOLAT (ez a valódi munka):**
  1. **Az ELŐ terméket az Apple csak app-bináris beküldéssel együtt fogadja el** — a fenti review-submission út **kizárólag a másodiktól** működik. Vagyis az első kör **félig kézi**, utána automatizálható.
  2. **Ár:** az Apple **fix árpontokat** használ (nem szabad ár) — a mostani létra (550 Ft → 1,49 EUR / 39 CZK / 169 RSD / 59 UAH) **hozzárendelést** kér az adott ország árpontjaihoz. Ez **döntés**, nem képlet, és országonként ellenőrizni kell.
  3. **Az „upcoming" logika:** a Play-en a termék `DRAFT` állapota adja a kaput; az Apple-nél ilyen „megjelenésig előkészítés" nincs — ott a **kereskedelmi kaput a kliens/szerver** adja majd (a meglévő `isUpcoming` szabályunk erre alkalmas, de ez tudatos döntést kér).
  4. **A kliens- és szerveroldal:** iOS-en a vásárlást **StoreKit** adja (`in_app_purchase` ezt már támogatja), a visszaigazolás viszont **App Store Server API + App Store Server Notifications V2** — a **`verifyLabelPurchase` iOS-ága ma nem létezik**, és ez a legkockázatosabb rész (pénz mozog). Mellette: **fizetős Apple Developer Program ($99/év) + Paid Applications Agreement + bank/adó**, és a jutalék (15% a Small Business Programban, egyébként 30%).
- **AMI VISZONT INGYEN JÖN (újrahasznosítható):** a **WP → Firestore → függvény** trigger és a **zár/retry** minta, a **tiszta plan-modul** séma (`play-product-plan.js` mintája), a termék-azonosító séma (a két bolt **külön** nyilvántartás, ugyanaz a `radio_mp3_320` szöveg jó), a leírások/review-note szövegek és az árlétra forrása.
- **📌 A JAVASOLT SORREND, HA EGYSZER NEKIÁLLUNK (mindegyik külön döntés):** (1) tagság + szerződés + bank/adó; (2) **API-kulcs** a Secret Managerbe, és egy **csak olvasó** ellenőrző eszköz (`tools/check-appstore-products.mjs`, a `check-play-products.mjs` mintájára) — írás csak `--confirm`-mal, ahogy a projektben szokás; (3) tiszta **`functions/appstore-product-plan.js`** + tesztek (árpont-hozzárendelés, országok, elnevezések); (4) szinkron: **létrehozás + ár + elérhetőség** (még beküldés nélkül); (5) beküldés (a **második** terméktől); (6) **kliens StoreKit + szerveroldali visszaigazolás** — ez az utolsó, és ez kér zárt tesztet a készüléken.
- **📌 A WP-OLDAL NEM IGÉNYEL ÚJ API-T (mérve, 2026-09-24):** a szinkron **nem** a WP-tól kap kész terméklistát — a `syncWordPressLabelProducts()` maga kéri le a **meglévő** `${WORDPRESS_BASE_URL}/releases` végpontot, és abból építi a termékeket (az `releaseId` csak szűrő). Ezért a **trigger és az adat ma is megvan** az Apple-ághoz: kiadvány-azonosító, cím, előadók, borító, műfaj, megjelenés dátuma, `is_upcoming`, `is_free`, valamint a `products` tömb (termék-azonosítók, címkék, árak). **Amit az Apple IAP létrehozása pluszban kér, az nem WP-adat:** ár**pont**-azonosító (API-lookup + hozzávetőlegesítés a mi létránkról), lokalizációs szöveg (a címből + terméktípusból **generálható**, de az Apple rövid nevet/leírást kér — a hosszkorlátot a beküldés előtt kell ellenőrizni), és egy **review-képernyőkép** (a kiadvány borítója adott, de hogy az Apple elfogadja-e review-képernyőként, az **mérés** kérdése a beküldésnél). **Opcionális WP-bővítés (NEM kötelező):** kézzel írható IAP-név/leírás és review-kép meta mezőnként (az plugin 2.8.0-t jelentene), illetve a WP-admin szinkron-panel kiterjesztése Apple-státusszal (csak kijelzés). **Ha ezekre nincs igény, a WordPress és a plugin VÁLTOZATLAN marad (2.7.0), és a WP nem is tud az Apple-ágról.**
- **📄 A RÉSZLETES TERV KÜLÖN DOKUMENTUMBAN: `docs/APPLE-IAP-TERV.md`** — ott van a mért „ami ma működik" lánc, az Apple-végpontok táblázata, a négy különbség, az újrahasznosítható rész, a **hat lépéses sorrend**, a **nyitott döntések** (árpont-hozzárendelés, review-kép forrása, IAP-szöveg, upcoming-kapu, visszatérítés) és a korlátok. **A következő kör ott kezdje, ne itt.**
- **⚠️ ŐSZINTE KORLÁT:** mindez **Apple-oldali**, és **nem próbáltuk élesben** — a fenti végpontok az Apple **2026-os dokumentációjából** származnak, nem saját mérésből. Amint megvan a tagság és az API-kulcs, az **első olvasó hívás** (terméklista) fogja megmondani, hogy a mi konkrét fiókunkon mi elérhető — addig ez **terv**, nem ígéret.

### A DJ „megjelenései" a DJ-adatlapon (2026-09-24, AAB **354** — kliens-oldali újdonság)

- **A TULAJDONOS KÉRÉSE:** *„kéne olyan az appba, hogy a dj adatlapon legyen ott a megjelenése is"*. **Kérdező ablakban tisztázva** (mert a szó kétfélét jelenthet): a válasza *„Csak a kiadványok, a következő fellépése már szerepel"* → **azok a kiadványok kellenek, amelyekben a DJ szerepel**, a **közelgő fellépések szakaszhoz NEM nyúlunk**.
- **AMIT ELŐBB MEGMÉRTEM (mielőtt bármit írtam volna):**
  - A `/releases?artist=<id>` szűrő **már létezik és él** (Goze **3**, Adam Bass **5**, Outragers **1** kiadvány) — a `getReleases` pedig a **mentett** HEAD-válaszból dolgozik (`_getHeadCached('/releases', {summary, artist})`, a cache-kulcs a **teljes URI**), tehát **nem kellett új cache-réteg**: a szakasz a meglévő, tartós gyorsítótárból azonnal jön.
  - **⚠️ Amit NEM érintettünk, de rögzítem (mert a mérés mást mutat, mint amit a tulajdonos feltételezett):** az élő artist-payload `upcoming_events` mezője **mind a 16 DJ-nél üres** — **Adam Bass** esetében is, pedig ő rajta van az **október 17-i Hard Base Classic** fellépőlistáján (esemény #12505). Vagyis a „Közelgő események" szakasz a 353-ban **nem jelenik meg**. A tulajdonos azt mondta, a következő fellépés már szerepel, ezért **nem nyúltam hozzá** — de a mérés itt van, ha egyszer előkerül: a `/events` lista a `visible` mezőt **igaz/hamis** alapon szűri, a DJ-adatlaphoz tartozó `huhs_get_artist_upcoming_events()` viszont `meta_query`-vel **szó szerint `visible = '1'`-et** kér és csak a **kezdő** dátumot hasonlítja — ez a kettősség a gyanús pont (szerveroldali javítás, AAB nélkül).
- **MI ÉPÜLT (három új fájl + egy bekötés):**
  - **`lib/services/artist_releases_plan.dart` (ÚJ, tiszta):** `artistReleasesFor` (csak a **saját** kiadványok, dátum szerint **csökkenő**), `artistReleasesPreview` (legfeljebb `artistReleasesPreviewCount` = **4**), `hasMoreArtistReleases`, `artistReleasesLabel` (1-nél **„Megjelenése"**, többnél „Megjelenései"). **`artistId <= 0` → üres lista**, hogy egy hibás azonosító **soha** ne hozza be a teljes katalógust a DJ-adatlapra.
  - **`lib/widgets/artist_releases_section.dart` (ÚJ, tesztelhető widget):** a szakasz a `releasesProvider((search: '', artistId: artistId))`-ból dolgozik, **`valueOrNull`-lal**: amíg nincs érték, **`SizedBox.shrink()`** (nincs pörgő, nincs villogás), hibánál **nem tesz hibadobozt** a DJ-adatlap közepére, üresen **nem hagy helyet**. Négy kiadvány után **„Összes megjelenése (N)"** gomb → a **DJ-re szűrt** `ReleasesScreen`.
  - **`lib/widgets/release_card.dart` (ÚJ, KÖZÖS):** a kiadvány-kártya **kiemelve** a `releases_screen.dart`-ből (bitre ugyanaz a megjelenés: 150 px borító, előadók, dátum, műfaj, `DetailPrefetch` + koppintásra kiadvány-adatlap). **Így a két felület nem tud széthúzni** — ez a 351-es tanulság alkalmazása (a szabály lehet helyes, a bekötés hibás).
  - **`artist_detail_screen.dart`:** a szakasz a **bemutatkozás után**, a közelgő események **előtt**; a képernyő csak ennyit tud: `ArtistReleasesSection(artistId: artist.id, artistName: artist.title)`.
- **BIZONYÍTÉK:** `flutter analyze lib test` **tiszta**; `flutter test` **837/837** (a kör előtt 811; **+26**: `test/services/artist_releases_plan_test.dart` **14**, `test/widgets/artist_releases_section_test.dart` **6** — köztük a *„amíg tölt, semmit nem rajzol"*, a *„más DJ kiadványa nem kerül a lapra"* és a *„hibánál nincs hibadoboz"* —, valamint `test/services/artist_releases_wiring_test.dart` **6** forrás-lint: a bekötés, a szerver-szűrő, a **nincs villogás** szabály és a **közös kártya**); `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN** (a Play-blokkok **229/500** és **406/500** karakter); **1 aláírási identitás** (`check-signing-identity.mjs`, 56 csomag); a versionCode a merge-elt manifestből **354**.
- **Csomag:** `build/HUHS-v1.0.0+354-release.aab` (versionCode **354**, **81,33 MB**, SHA-256 `CED7CBFDD6515C662839633EB11794711D47F8CEDD4BC0ECBEDF2550613B48B8`). A `docs/PLAY-KIADASI-JEGYZET.md` frissítve: meta **354**, §0 (alpha = **353**), AAB-tábla, **1. blokk = a 354** (229 karakter), **1b. blokk = 352–354 összesítő** a nyilvános kiadáshoz (406 karakter), **§3 = új 354 bejegyzés**; a `docs/PLAY-PRODUCTION-BEADAS.md` §0 „következő csomag" sora is 354-re állítva. Az app-changelog (`lib/data/app_changelog.dart`) új 354 bejegyzést kapott.
- **A WordPress-plugin és a szerveroldali függvények NEM változtak** — a szűrő már létezett, ezért **nincs plugin-feltöltés és nincs `firebase deploy`**.
- **📌 A TULAJDONOS LÉPÉSE:** a **354 feltöltése a zárt teszt sávra** (`Könyvtárból`/feltöltés) → eszközpróba (egy DJ-adatlap, pl. **Goze**: „Megjelenései" + „Összes megjelenése") → utána a **nyilvános kiadás** a **352–354 összesítő** szöveggel (a 353-at nem kell külön előléptetni, a 354 tartalmazza).
- **⚠️ ŐSZINTE KORLÁT:** a szakasz **annyit mutat, amennyit a szerver ad** — ha egy kiadvány a WordPress-en nincs a DJ-hez rendelve, itt sem fog megjelenni. A **kártyák helyét** a készüléken a tulajdonos látja (a DJ-oldali görgetést én nem tudom vezérelni).
- **✅ iOS-EN IS TELEPÍTVE ÉS FUT — MÉRVE (2026-09-24, ugyanez a nap):** a tulajdonos kérésére *„sideloadold fel nekem a 354-et iOS-re"* a **meglévő CI-úton** ment végig, **kódváltozás nélkül**: `a57c3581` push → **GitHub Actions `ios-unsigned-check.yml` (run `36015542142`) zöld** → `HUHS-ios-unsigned-ipa` letöltve (`build/ios-ipa/Runner-unsigned.ipa`, 25,06 MB) → **Sideloadly** (a telefon: *Imre iPhone-ja*, iPhone SE 2, iOS 26.7) **frissítésként** felrakta, ugyanazzal a bundle ID-val (`hu.hungarianhardstyle.app.JQPJ793V65`, tehát az app-adatok és a regisztrált App Check debug token **megmaradtak**). **A bizonyíték három szinten:** (1) a Sideloadly adatbázisa: `2026-09-24 17:01:47 → 17:02:06`, `last_error` üres; (2) a **csomagban benne van a 354 kódja** — `node tools/verify-ios-ipa.mjs build/ipa-354/Payload/Runner.app --test-ads --contains=ArtistReleasesSection --contains=artistReleasesFor '--contains=Összes megjelenése'` → **minden ellenőrzés zöld** (a `--contains=` kapcsoló ehhez került az eszközbe, önteszt **6/6**); (3) az app **elindult és tölt** a készüléken (`pymobiledevice3 developer dvt launch` + `screenshot`, a kép: `build/ios-354-home.png`). ⚠️ **Amit ez NEM bizonyít:** az új szakaszt **koppintással** nem tudtam megnézni (a `pymobiledevice3` nem tud koppintani) — azt a tulajdonos látja. Az ingyenes Apple ID-s aláírás **~7 napig** érvényes.

### A Play „bittérkép alul-mintavételezés" javaslata kivizsgálva — GOOGLE SAJÁT hirdetési SDK-ja, nem a mi kódunk (2026-09-24, AAB **353**; kód NEM változott)

- **A TULAJDONOS JELZÉSE (képernyőkép a Play Console 353-as kiadásából):** *„Az alkalmazás teljesítményének javítása bittérkép alul-mintavételezésével — Alkalmazásod a következő helyeken használja a(z) BitmapFactory paramétert alul-mintavételezés nélkül: **1b6.X**, **k17.X**"*, és a kérdés: *„ezzel mit lehet, hogy ne törjön el az app és a képek minősége se romoljon?"*
- **⚠️ ELSŐ CSAPDA — a név félreolvasható:** a Play a `<osztály>.<metódus>` alakot írja, és a képernyőn `1b6.X` áll, de az valójában **`lb6.X`** (kis „L", nem egyes). Emiatt az első DEX-keresés (`1b6/X`) **nulla találat** volt — a helyes névvel viszont mindkettő megvan. **Aki ezt keresi, ne az `1b6`-t keresse.**
- **A MÉRT GYÖKÉR (a 353-as csomagból, nem következtetés):** a `dexdump` + az R8-`mapping.txt` összevetésével a két megjelölt osztály:
  - `lb6` → **`com.google.android.gms.internal.ads.zzelp`**
  - `k17` → **`com.google.android.gms.internal.ads.zzgtp`**
  Mindkettő a **Google Mobile Ads (AdMob) SDK** belső osztálya, és a DEX-ben **kizárólag** a `BitmapFactory.decodeStream(InputStream)` **paraméter nélküli** alakját hívja (tehát tényleg nincs `Options`/`inSampleSize`). **A Play jelzése pontos.**
- **NEM JAVÍTHATÓ LIBRARY-EMELÉSSEL (mérve):** a csomagban `com.google.android.gms:play-services-ads:25.4.0` van; a legfrissebb elérhető a **25.5.0** (Google Maven, 2026-09-17). A két AAR-t letöltve és összevetve **ugyanaz a minta mindkettőben benne van** (25.4.0: `zzaow`, `zzdrm`, `zzelp`; 25.5.0: `zzape`, `zzdrz`, `zzemc` — egyik sem használ `inSampleSize`-t). **Ezért a 354-es csomag ezen nem változtatna — nem indítunk emiatt kört.**
- **A SAJÁT KÓDUNK NEM ÉRINTETT — és ez a lényeg a tulajdonos kérdésére:** a `android/` alatti **négy** saját forrásfájlban (`MainActivity.kt`, `RadioPlaybackService.kt`, `BaselineProfileGenerator.kt`, `GeneratedPluginRegistrant.java`) **egyetlen `BitmapFactory`-találat sincs**. Az app képei a **Flutter-en (Skia)** mennek át, nem ezen a Java-úton, ezért **itt nem is lehetne úgy „javítani", hogy a mi képeinkhez hozzányúljunk** — és mivel nem nyúlunk hozzá, a **képminőség garantáltan nem változik**. **Jó hír:** a lejátszó/értesítés borítóképéért felelős `com.ryanheise.audioservice.AudioService` **használ `inSampleSize`-t** (mérve).
- **AKI MÉG ALUL-MINTAVÉTELEZÉS NÉLKÜL DEKÓDOL a csomagban (mind NEM a mi kódunk, mérve a 353-ból):** 3 AdMob-osztály, `com.google.firebase.messaging.ImageDownload` (FCM-értesítés képe), `androidx.localbroadcastmanager.content.LocalBroadcastManager` és egy Tink-osztály. **⚠️ Az utóbbi kettő R8-összevonás következménye lehet** (a kódot az R8 beolvaszthatta egy másik osztályba), ezért az SDK-hovatartozás **tájékoztató**; a biztos pont a Playen megjelenő fedett név.
- **ÚJ ESZKÖZ (csak olvasás): `tools/check-bitmap-subsampling.mjs`** — `--self-test` **15/15**. Három mód: (1) `--aab <csomag>` a **ideküldött** csomagot vizsgálja `dexdump`-dal, és a `build/app/outputs/mapping/release/mapping.txt`-ből **visszafejti a fedett neveket** (ez válaszolja meg egy az egyben a Play jelzését: kié az `lb6`/`k17`); (2) `--aar <fájl>` SDK-verziók összevetésére („javít-e egy emelés?"); (3) `--json`. A DEX-módhoz `dexdump` kell (Android SDK build-tools) — az eszköz magától megkeresi. **A jelentés mindig kiírja a korlátjait:** ha az `Options`-t más osztály tölti fel, azt nem látja, és az R8-összevonás miatt a kiírt osztálynév a kódot *befogadó* osztály lehet.
- **A HELYES VÁLASZ EZÉRT: nincs kódváltozás, nincs új AAB.** Ez a Play **javaslata** (ugyanaz a fajta tájékoztató kártya, mint a korábbi „4 művelet javasolt"), a Google saját hirdetési SDK-jának memória-viselkedéséről szól, nem hiba és nem blokkoló — **és amíg AdMobot használunk, ott is marad**, mert a kód nem a miénk. A helyes út a **mérés rögzítése**, és **újramérés a következő SDK-emelésnél** egy paranccsal: `node tools/check-bitmap-subsampling.mjs --aab build/HUHS-v1.0.0+<build>-release.aab`.
- **⚠️ ŐSZINTE KORLÁT:** azt **nem** tudom mérni innen, hogy ezek a hirdetés-kódutak futásidőben mennyi memóriát használnak, és a Play jelzése **statikus elemzés** (nem futásidejű hiba). Amit igazoltam: a két megjelölt osztály pontos kiléte, hogy a legfrissebb SDK-ban is így van, és hogy a **saját** kódunkban ilyen hely nincs. Az app a 353-mal a készüléken **működik** (tulajdonosi eszközpróba).
...
- **✅ 1. RÉSZ KÉSZ — a kvíz „már játszottál" állapota AZONNAL látszik (2026-09-24):** a tulajdonos kiegészítése (*„a kviz is írhatná, hogy már játszottál … kell pár másodperc mire beáll"*) megoldva. **A mért gyökér:** az emlékezet olvasása **aszinkron** volt (`SharedPreferences.getInstance()`), a `GameScreen` az **auth-állapot első jelzésére** is várt, a **kvíz kártyája pedig egyáltalán nem** kérdezte az emlékezetet (csak a képernyő). **A javítás:** (1) a `VoteMemory` **memóriabeli tükröt** kapott — a `main()` a `runApp` előtt egyszer betölti (`preload()`), utána a `isGamePlayedSync` / `isPollVotedSync` / `prizePlaySync` **szinkron**, lemez- és hálózatmentes; (2) a `GameScreen` a jelzést **még az első képkocka előtt** beállítja (initState, await nélkül), és az auth-jelzésre csak akkor vár, ha nincs bejelentkezett fiók; (3) a **kvíz kártyája** (`home_screen.dart`) a szinkron emlékezetből dönt, és a `VoteMemory.revision` `ValueListenableBuilder`-rel **magától** frissül beküldés után („Már játszottál — eredmény megtekintése"); (4) a **szerver marad a hiteles forrás** (ha az emlékezet téved, a jelzés törlődik). ⚠️ **A törlés sorrendje számít:** a memóriabeli tükör **előbb** törlődik, mint a lemez — fordítva egy mikrotasknyi ablakban a téves „már szavaztál" látszana (ezt egy meglévő provider-teszt buktatta el, és így javítottuk). **Bizonyíték:** `flutter analyze lib test` **tiszta**; `flutter test` **785/785** (a kör előtt 776); új tesztek a `test/services/vote_memory_game_test.dart`-ban (szinkron tükör: preload előtt/után, írás/törlés azonnal, értesítés-szám, forrás-lint a kártyára), és a `VoteMemory.resetForTests()` bekerült a `poll`/`prize` provider- és widget-tesztek `setUp`-jába (a statikus tükör nem szivároghat át).
- **✅ 2. RÉSZ KÉSZ — a DJ-adatlap claim-állapota és a profil DJ-adatlap kártyái cache-first (2026-09-24):** új **közös cache-réteg**: `lib/services/async_cache_plan.dart` (tiszta: boríték + elavulás + változás-összehasonlítás) és `lib/services/async_cache_store.dart` (`AsyncCacheStore` + `readThroughCache`: mentett adat **azonnal**, háttér-egyeztetés, **hiba esetén a mentett adat marad**, és **csak változáskor** rajzol újra). A `artistClaimStatusProvider` és az új `claimedArtistsOfUserProvider` ezt használja; a mentési kulcs **tartalmazza a UID-t** (más fiók nem örökölheti); átvétel/visszavonás után a **tudott új állapot** kerül a mentésbe (`rememberArtistClaimStatus`), ezért nincs régi-állapot villanás. A profil „DJ-adatlap" szekciója `FutureBuilder` helyett providerből dolgozik, a **saját** profil (`community_screen.dart`) pedig a **mentett** listát rajzolja azonnal — a `getMyClaimedArtists` és a `getClaimedArtistsForUser` a szerver forrásában **ugyanaz a lekérdezés** (`artist_claims where uid == …`), ezért ez nem viselkedésváltozás.
- **✅ 3. RÉSZ KÉSZ — előtöltés bejelentkezés után:** új `AccountPrefetch` widget (a `main.dart`-ban a kezdőképernyő körül): bejelentkezés után **egyszer**, a first frame UTÁN, a háttérben melegíti a saját claim-listát, a hozzájuk tartozó claim-állapotokat (legfeljebb 5) és a saját profilt. **Csak olvas**, a hibákat elnyeli, és fiókonként egyszer fut.
- **✅ 4–5. RÉSZ — a listák és a kiadványok MÁR cache-first-ek (kód+mérés):** a `newsProvider`/`stickyNewsProvider` a WordPress-réteg **mentett JSON**-jából dolgozik (`WordpressService._getHeadCached`, 5 perces egyeztetési ablak, a megjelenítés mindig a mentett értéket adja), és a `PublicContentWarmer` már induláskor melegíti a hírt + az eseményeket; a `releasesProvider` `keepAlive` + WordPress-cache; a `labelLibraryProvider` mentett lista + háttéregyeztetés + 30 s. **Ezért itt szándékosan nem változtattunk** — a hálózati mérés (`tools/check-content-latency.mjs`) mutatja, mennyit spórol a mentés (~0,55 s hívás helyett ~0 ms).
- **📦 A 353 ÚJRAÉPÍTVE (a 353-at a Play még NEM kapta meg — a feltöltött lista 352-ig tart, ezt `check-play-track.mjs` igazolja):** ugyanaz a verziókód, de **tartalmazza az összes fenti javítást**; `build/HUHS-v1.0.0+353-release.aab` (81,32 MB, **SHA-256 `CE669CD629709E7525EC756AA0206CE2EBE789870DABAEC1BE160F0578F3FFAB`**, 1 aláírási identitás). A changelog (app + Play 1./1b. blokk + a 3. pont bejegyzése) frissült, `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN**.
- **Bizonyíték:** `flutter analyze lib test` **tiszta**; `flutter test` **811/811**; új tesztek: `test/services/async_cache_plan_test.dart` (**12**), `test/providers/artist_claim_cache_test.dart` (**8**), `test/widgets/account_prefetch_test.dart` (**6**). **⚠️ ŐSZINTE KORLÁT:** eszközön mért igazolás nincs — a nyereség (kvíz azonnali állapot, claim-állapot azonnal) a tulajdonos készülékén mérhető, a zárt tesztben.
...
- **MI VÁLTOZOTT (`pubspec.yaml`) — a Firebase-család EGYÜTT lép:** `firebase_core` 3.15.2 → **4.15.0**, `firebase_auth` 5.7.0 → **6.7.0**, `cloud_firestore` 5.6.12 → **6.10.0**, `cloud_functions` 5.6.2 → **6.5.0**, `firebase_messaging` 15.2.10 → **16.7.0**, `firebase_app_check` 0.3.2+10 → **0.4.8**. **Szándékosan NEM** emeltünk mást (`google_sign_in` **6.3.0** marad, riverpod **2.x** marad, `share_plus`/`app_links` rögzítve): a `flutter pub upgrade --major-versions` **95 csomagot** mozgatott volna (riverpod 3.x, google_sign_in 7.x, local_auth 3.x, package_info_plus 10.x) — az **külön kör**, nem keverjük bele.
- **KÓDVÁLTOZÁS (összesen egy API-átnevezés + egy teszt):** a `firebase_app_check` 0.4.x-ben az `androidProvider`/`appleProvider` paraméter **elavult**, helyette provider-**osztályok** mennek. A `lib/main.dart` mostantól: `providerAndroid: const AndroidDebugProvider()` / `const AndroidPlayIntegrityProvider()`, valamint `providerApple: _appleAppCheckProvider`, ahol a getter típusa `AppleAppCheckProvider`, értéke `AppleDebugProvider()` / `AppleAppAttestWithDeviceCheckFallbackProvider()`. **A viselkedés bitre ugyanaz** (debug → debug, release → Play Integrity; iOS-en App Attest DeviceCheck-visszaeséssel). A `test/services/app_check_platform_test.dart` az új nevekre **átírva**, és **ÚJ őr** került bele: az elavult `androidProvider:`/`appleProvider:` **nem** kerülhet vissza.
- **MÉRT NYERESÉG (ugyanaz az eszköz, ugyanaz a riport, csak az új függőségekkel — `node tools/analyze-r8-config.mjs`):**

| Mérce | **352** (előtte) | **353** (utána) |
|---|---|---|
| R8 optimalizálás | 54,33% | **92,09%** |
| R8 obfuszkiálás | 54,52% | **92,28%** |
| R8 csökkentés | 54,46% | **92,22%** |
| blokkolt elemek | 92 912 / 203 434 | **15 756 / 199 136** |
| a legnagyobb visszatartó szabály | `com.google.android.gms.internal.**` (firebase-auth 23.2.1) — **39,66%** | `com.google.android.recaptcha.**` (recaptcha 18.7.1) — **2,89%** |
| DEX a csomagban | 12,03 MB / **3 fájl** | **9,86 MB / 2 fájl** |
| SafetyNet a mappingben | 394 osztály | **0** |

- **A VALÓDI NYERESÉG a DEX:** **12,03 → 9,86 MB (−18%)**, és **3 helyett 2 DEX-fájl**. **⚠️ Az AAB fájlmérete viszont 80,67 → 81,25 MB** (+0,58 MB), mert a `BUNDLE-METADATA/com.android.tools.build.obfuscation/proguard.map` **+1,09 MB**-tal nagyobb (sokkal több osztályt obfuszkiálunk) — **az a bejegyzés NEM megy le a felhasználóhoz** (Play-oldali metaadat a hibák visszafejtéséhez). **A fájlméret önmagában tehát félrevezető** — a DEX és a letöltött APK számít.
- **BÓNUSZ — a Play-panel 1. javaslata magától megszűnt:** az új `firebase_app_check` **nem hozza többé a SafetyNetet**. Mérve: a merge-elt manifestben a `FirebaseAppCheckSafetyNetRegistrar` **eltűnt** (helyette `FirebaseAppCheckPlayIntegrityRegistrar` és `…RecaptchaRegistrar` vannak), a `play-services-safetynet.properties` **nincs** a csomagban, a mappingben **0** SafetyNet-találat. **A saját keep-szabályainkhoz NEM nyúltunk** (appcheck 0,42%, audioservice 0,37%, WorkManager 0,23%) — a javulás tisztán a library-verziókból jött.
- **BIZONYÍTÉK:** `flutter analyze lib test` **tiszta**; `flutter test` **776/776** (a kör előtt 775; a +1 az új App Check-őr); a **release build lefutott**; a versionCode a merge-elt manifestből **353**; **nincs `missing_rules.txt`** (az R8 nem jelzett hiányzó hivatkozást); a mappingben **a helyükön vannak** a reflection-nel elért kritikus osztályok: `com.ryanheise.audioservice.AudioService` (eredeti néven), `androidx.work.impl.WorkDatabase`, `hu.hungarianhardstyle.app.MainActivity` és `RadioPlaybackService`, valamint a `io.flutter.plugins.firebase.appcheck.*` osztályok.
- **Csomag:** `build/HUHS-v1.0.0+353-release.aab` (versionCode **353**) — ⚠️ **ezt a csomagot a cache-kör ÚJRAÉPÍTETTE** (ugyanaz a verziókód, mert a 353-at a Play még nem kapta meg): a **mostani** fájl 81,32 MB, SHA-256 `CE669CD629709E7525EC756AA0206CE2EBE789870DABAEC1BE160F0578F3FFAB` — lásd a legfelső szakaszt. A `docs/PLAY-KIADASI-JEGYZET.md` **frissítve** (meta: currentBuild 353 / lastPublishedBuild 352; AAB-tábla; 1. rövid blokk = a 353; **1b. blokk = 352–353 összesítő a nyilvános kiadáshoz**, mert a nyilvános felhasználók a 329–351 szöveget látták; 3. pont = 353 bejegyzés), és a `lib/data/app_changelog.dart` is; `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN**.
- **📌 A TULAJDONOS LÉPÉSE:** a **353 feltöltése a zárt teszt sávra** (`Könyvtárból`/feltöltés), majd **eszközpróba** a tesztelőkön és a saját készülékén: Google- és e-mailes bejelentkezés, értesítések (FCM), chat/komment írás, kiadvány-vásárlás + „Megvásárolt zenéim", háttérzene/zárképernyő-vezérlés, hír- és tartalom-betöltés (App Check-kel védett hívások). **Csak ezután** érdemes a production sávra léptetni.
- **⚠️ ŐSZINTE KORLÁT:** **futásidejű, eszközön mért** igazolás nincs (az obfuszkiálás 54% → 92% közötti ugrás **pont azt a hibaosztályt** érinti, amit a 345-ös/347-es körben már kétszer **csak release-ben** találtunk meg: reflection-nel hívott osztályok, eltávolított konstruktorok). Ezért a **zárt teszt kötelező lépés, nem formalitás.** Amit itt igazolni tudtam: a build lefut, a tesztek zöldek, az R8 nem jelzett hiányzó hivatkozást, és a kritikus osztályok a mappingben megvannak. A Play-oldali R8-mutató **várhatóan** nagyot javul, de a Play a **maga nevezőjével** számol — a pontos számot a Console mutatja majd a 353 feldolgozása után.

### Az „R8-optimalizálás" javaslat kivizsgálva — a blokkolt kód 40%-át EGY Google-library szabálya tartja vissza (2026-09-24, mérés + kutatás; az app NEM változott)

- **A TULAJDONOS JELZÉSE:** a Console (352) *„4 művelet javasolt"* 4. pontja: *„Az alkalmazás memóriájának és teljesítményének javítása R8-optimalizálással"* — alacsony **optimalizálási (36%)**, **obfuszkálási (37%)** és **csökkentési (37%)** arány. A kérdés: *„ezzel tudunk kezdeni valamit? bár eddig se nagyon sikerült"* → **a tulajdonos döntése: először CSAK KUTATÁS, kódváltozás nélkül.**
- **ÚJ ESZKÖZ:** **`tools/analyze-r8-config.mjs`** (csak olvas). A Gradle `:app:analyzeReleaseR8Config` riportjából (`build/app/reports/r8/r8-config-analyzer-release.html` / `.pb`) dolgozik, és megmutatja, hogy a visszatartott kód **melyik konfigurációs fájlból** jön, illetve hogy a **saját** szabályaink mekkora **exkluzív** hatásúak. `--self-test` **6/6**, `--json`, `--top N`. A riport előállítása (a `android` mappából, a kötelező `-P` AdMob-azonosítókkal, különben a Gradle szándékosan elhasal): `.\gradlew.bat :app:analyzeReleaseR8Config -P "HUHS_ADMOB_APP_ID=…" -P "HUHS_ADMOB_BANNER_ID=…" -P "HUHS_ADMOB_REWARDED_ID=…"`. ⚠️ **Ezen a gépen a `python.exe` egy hibás architektúrájú stub**, ezért a skill Python szkriptjei nem futnak — az eszköz a repóban már meglévő **`functions/node_modules/protobufjs`**-szal dekódolja a protobufot.
- **MÉRT EREDMÉNY (180 keep-szabály, 203 434 élő elem: 23 750 osztály, 58 712 mező, 120 972 metódus):** **nincs** globális tiltó szabály (`-dontoptimize`/`-dontobfuscate`/`-dontshrink` sehol). A visszatartott kód **39,78%-át EGYETLEN szabály** adja: **`-keep class com.google.android.gms.internal.** { *; }`** a **`firebase-auth-23.2.1`** AAR **beépített `proguard.txt`**-jéből (package-wide; egymaga 80 927 elemet érint). Utána: `recaptcha-18.6.1` **3,05%**, **a MI szabályaink 1,24%**, `play-services-ads` 0,88%, az AGP default `proguard-android-optimize.txt` 0,72%. A saját 6 szabályunk **exkluzív** hatása **1,19%** (appcheck 0,51%, audioservice 0,36%, work.impl.model 0,22%, a többi 0,1%).
- **⚠️ A KÉT SZÁM NEM UGYANAZ A MUTATÓ:** a Play 36/37/37-e a maga nevezőjével számol, a riport 54,3/54,5/54,5-e az R8 „élő elemek" nevezőjével — a kettőt **nem szabad egymás mellé tenni**. A riport a **blokkolás összetételét** mutatja (kitől jön), nem a Play pontszámát. A `-repackageclasses` / `-allowaccessmodification` **már benne van** (`android/app/proguard-rules.pro`), R8 full mode **aktív** (`gradle.properties`-ben nincs `android.enableR8.fullMode=false`) → a konfigurációs oldalon a gyakorlati maximumon vagyunk.
- **A KUTATÁS EREDMÉNYE (a tulajdonos által kért irány: „nézd meg a library-verziókat") — a valódi AAR-fájlokból mérve:**
  - **`firebase-auth` 23.2.1** (amit használunk, a Flutter `firebase_core` 3.15.2 → **Firebase BoM 33.16.0** miatt) `proguard.txt`-jében **benne van** a `-keep class com.google.android.gms.internal.** { *; }`.
  - **`firebase-auth` 24.2.0** (legfrissebb, elérhető) `proguard.txt`-jében **MÁR NINCS BENNE** — csak a szűk `-keepclassmembers class * extends com.google.android.gms.internal.firebase-auth-api.zza… { <fields>; }` maradt. Vagyis **a 39,7%-os fal egy library-frissítéssel eltűnik**. (A letöltött AAR-okból ellenőrizve, nem feltételezés.)
  - **`recaptcha` a `firebase-auth` KÖZVETLEN függősége** (`com.google.android.recaptcha:recaptcha:18.6.1`, a Gradle-fa szerint), és **az auth 24.2.0 is 18.6.1-et kér** (a POM-ból mérve) → a recaptcha 3,05%-a **nem tűnik el** az auth-frissítéstől. A **recaptcha 18.9.3** viszont már szűk szabályokat hoz (a `com.google.android.recaptcha.**`, `play.core.integrity.**`, `gms.tasks.**` és okhttp package-wide keep-ek helyett `keepclassmembers`-eket) — ezt külön kell emelni.
  - **A frissítés mérete (mért, `flutter pub outdated`):** a Firebase-család **major** verziókat lépne: `firebase_core` 3.15.2 → **4.15.0**, `firebase_auth` 5.7.0 → **6.7.0**, `cloud_firestore` 5.6.12 → **6.10.0**, `firebase_messaging` 15.2.10 → **16.7.0**, `firebase_app_check` 0.3.2+10 → **0.4.8**. A verziók a `pubspec.yaml`-ban **rögzítve** vannak (ezért nem ajánlja fel őket automatikusan). Ez **nem konfigurációs tweak**, hanem az auth/adat/push/App Check réteg **együttes major emelése** (Dart API-törések lehetségesek) → új AAB + zárt teszt + eszközös ellenőrzés kell hozzá.
- **⚠️ AMIT EZ NEM JAVÍT:** semmi nem tört el — a 352 éles és működik. A mutató **tájékoztató**; a nyereség kisebb DEX/APK és kicsit kevesebb memória, nem funkcionális javítás. Ezért **a bírálat alatt és a mostani kiadáshoz semmihez nem nyúlunk.**
- **✅ A TULAJDONOS DÖNTÉSE (2026-09-24):** *„nézd meg a library-verziókat (csak kutatás)"* → **megtörtént, ez a szakasz az eredmény**. A kód és a kiadás **változatlan**; a további lépésről (parkolás / Firebase major kör / csak Android-oldali BoM-emelés) **külön döntés** születik. → **A döntés meg is született: „Indítsuk el a Firebase major kört", és le is futott — lásd a legfelső szakaszt (AAB 353).**

### Nyilvános (production) kiadás előkészítése — terv, és a tesztelői mérés TANULSÁGA (2026-09-22)

- **A TULAJDONOS JELZÉSE:** *„van már éles kiadás engedélyem"* → a **production access megvan**, ezért a **12 tesztelő / 14 folyamatos nap** követelmény **nem** blokkol. A kör célja a **nyilvános kiadás előkészítése**.
- **A TERV: `docs/PLAY-PRODUCTION-BEADAS.md`** — mért állapot, az App content nyilatkozatok (a mi válaszainkkal), a kiadás lépéssorrendje, a kiadás utáni ellenőrzés eszközei, és a nyitott döntések. **Ez a dokumentum a nyilvános kiadás egyetlen forrása.**
- **⚠️ A LEGFONTOSABB SZABÁLY (benne van a tervben is):** a **production sáv ország-listája pontosan a 9 ország** legyen, ahol a **termékek** elérhetők (HU, AT, HR, SI, SK, NL, CZ, RS, UA). Ha a production **több** országot tartalmaz, azokban a vevők **telepíteni tudnak, de vásárolni nem** — pontosan az a hiba, amit ma javítottunk.
- **MÉRT ÁLLAPOT (2026-09-22):** `alpha` = **352** (completed, a kiadási szöveggel); `beta` üres, production-nel szinkronizál; **`production` = 0 ország**; `internal` = 278 + egy üres piszkozat (eldobni); a nyilvános bolt-lap **404** (zárt tesztben várt); termékek **9 országban**, 56 `ACTIVE` + 4 `DRAFT` (a 12699 megjelenéséig); az **adatvédelmi nyilatkozat és az ÁSZF él (HTTP 200)**.
- **⚠️ A fióktörlés KÉT külön dolog (a tulajdonos jogosan szólt bele):** az **appban VAN fióktörlés** (Beállítások → fiók törlése, szerveroldalon végigfutva — `verify-live-account-deletion.mjs` méri), ezért **az nem hiányzik**. A Play Data safety viszont **pluszban** kér egy **webes URL-t** („Delete account URL"), ahol az app **nélkül** is kérhető a törlés — **az ma 404** (`…/fiok-torles/`, `…/adattorles/`). **A tervben ez így is van megfogalmazva**, hogy senki ne higgye azt, hogy a funkció hiányzik.
- **⚠️ TANULSÁG — a tesztelői létszám az API-ból NEM mérhető megbízhatóan:** az új `tools/check-play-testers.mjs` a `edits.testers.get` végponttal **0 tesztelőt** mért az `alpha`/`beta`/`internal` sávon, miközben a tulajdonos szerint a Console-ban **30+ tesztelő** van. **Ez az én téves következtetésem volt** (arra jutottam, hogy nincs tesztelő), és a tulajdonos igazított helyre. **A Console az irányadó**, nem ez a végpont. Az eszköz ezt a figyelmeztetést **ki is írja** a 0-s eredmény mellé, hogy ez a hiba ne fordulhasson elő újra. **ÁLTALÁNOSAN:** a Play API-ból **olvasott** érték nem bizonyíték a Console állapotára — a `testers` végpont különösen nem.
- **ÚJ ESZKÖZ:** `tools/check-play-testers.mjs` (csak olvasás; a `testers` végpont edit-alapú, ezért a saját ideiglenes piszkozatát törli; a címeket **maszkolva** írja ki).
- **✅ A TULAJDONOS DÖNTÉSEI (2026-09-22):** **korhatár/célközönség 16+**; **kigördítés azonnal 100%**; a **fióktörlési oldalt én hozzam létre**.
- **✅ A FIÓKTÖRLÉSI OLDAL ELKÉSZÜLT ÉS ÉL:** `https://hungarianhardstyle.hu/fiok-torles/` (WordPress-oldal **#12843**, HTTP **200**, helyes ékezetekkel) — ezt az URL-t kell a Play **Data safety** „Delete account URL" mezőjébe írni. **A szöveg NEM találgatás:** a `functions/index.js` `deleteUserReferences()` (4491+) és `deleteCommunityUser()` (4753+) **tényleges** viselkedését írja le (mi törlődik: profil, képek, chat, kommentek, pontok, szavazatok, DJ-átvételek, **vásárlási jogosultsági rekordok**; mi **marad**: a **Google Play-vásárlás** a Google-fióknál, ami új fiókkal **újra ellenőrizhető**; idő: fiók **azonnal**, képek **≤48 óra** — `expiresAt: +48 óra`, 4913. sor —, e-mailes kérés **≤30 nap**). **ÚJ ESZKÖZ:** `tools/create-deletion-page.mjs` — **`--confirm` nélkül nem ír**, `--confirm --update` frissíti; a végén **élőben ellenőrzi** a nyilvános oldalt (HTTP 200, ékezetek, kapcsolat). **⚠️ Ha az éles törlés viselkedése változik, a szöveget is frissíteni kell**, különben az oldal nem lenne igaz.
- **⚠️ AMI MÉG NYITOTT (Console-only):** a **hír-deklaráció** (az app ad hírtartalmat → „news app" = igen), és az **adatbiztonsági nyilatkozat** (Data safety) kitöltése a fióktörlési URL-lel.
- **A PRODUCTION KIADÁS BEKÜLDVE (2026-09-23):** a **352** a **production sávon** van (`completed`, a nyilvános **329–351 összesítő** kiadási szöveggel), az **országok pontosan a 9** (`check-play-products.mjs --tracks` → *„✔ ez pontosan a termékek elvárt ország-listája"*), a **zárt teszt (alpha) sáv változatlanul fut**. A **nyilvános bolt-lap akkor még 404** volt (a bírálat futott) — **azóta lefutott, lásd a következő bejegyzést**. ⚠️ **A zárt teszt sávjából hiányzik Hollandia** (alpha = 8 ország) — ez csak a tesztelőket érinti, a nyilvános kiadásban NL benne van.
- **✅ A NYILVÁNOS KIADÁS ÉL (2026-09-23, mérve):** a **bírálat lefutott** — a **nyilvános bolt-lap HTTP 200** (már nem 404), a **production sávon a 352** (`completed`, kiadási szöveggel), az **`alpha` szintén 352**, az **`internal` 278** (+ egy üres piszkozat), a feltöltött AAB-ek listája **352-ig** tart; `node tools/check-play-notes.mjs` → **MINDEN ELLENŐRZÉS RENDBEN**. **A nyilvános kiadás tehát kint van, a zárt teszt (alpha) pedig továbbra is fut.**
- **✅ AMELYIK CSOMAG TÉNYLEG FENT VAN — `check-play-track.mjs --hashes` (2026-09-24, ÚJ):** a Play API `edits.bundles.list` válasza **megadja a feltöltött AAB `sha256`-át** is, ezért a **verziókód önmagában nem elég** (ugyanazt a verziókódot újra lehet építeni!). A mérés (2026-09-24): a Playen a **353** lenyomata `ce669cd6…` = **pontosan az újraépített 353** (a cache-javításokkal), a 352/351/350 lenyomata is egyezik a helyi fájlokkal. **Eszköz:** `node tools/check-play-track.mjs --hashes` (a többi kapcsoló: `[csomagnév]`), a helyi oldallal összevetve `Get-FileHash build\HUHS-*.aab`.
- **📌 A `beta` SÁV = NYÍLT TESZTELÉS — ÉS EZ MAGYARÁZTA A „FELTETTEM, DE NEM KAPOM MEG" ESETET (2026-09-24, igazolt):** a Google API-dokumentuma szerint ([APKs and Tracks](https://developers.google.com/android-publisher/tracks)) a **fix sávnevek**: `production`, **`beta` = nyílt tesztelés (open testing)**, `qa` = belső tesztelés, a **zárt teszt sávja pedig saját név** (nálunk `alpha`). **A mért eset:** a tulajdonos a **nyílt tesztelésben** van, a `353`-at viszont a **zárt tesztre (`alpha`)** tette fel — az `alpha` 353 `completed` 100%-kal, és **a többi tesztelő meg is kapta**, ő nem, mert a Play őt a **production** sávból szolgálja ki (352). Ez **nem** kiadáshiba. **Amit kipróbált és nem működött:** a 353-at a **nyílt teszt sávra** tenni — a Play **nem engedte** (verziókód-megkötés; egy verziókód egy kiadásban használható, sávok között csak a `Könyvtárból`/előléptetés úton, és ez itt elutasításra került). **A `beta` sávon eközben egy ÜRES PISZKOZAT maradt** (mérve: `check-play-track.mjs` → `beta: draft`), ezt a Console-ban **el kell dobni**. **✅ A TULAJDONOS DÖNTÉSE (2026-09-24):** *„csak Play-frissítés / várakozás a zárt teszttel"* — **egyelőre nem építünk 354-et**; előbb a zárt teszt Play-oldali érvényesülését próbálja (Play Áruház adatainak törlése → a meghívó link megnyitása a helyes fiókkal → csatlakozás → frissítés), mert a Google a tesztelői változások érvényesülését **akár órákig** késleltetheti. **📌 HA EZ NEM ELÉG:** a biztos út a **nyílt teszthez** egy **új verziókód** (354, ugyanaz a tartalom) felhelyezése a `beta` sávra.
  - **⚠️ A TULAJDONOS HELYESBÍTÉSA (2026-09-24) — NE ISMÉTELD MEG:** a tulajdonos szavai: *„ne haluzz, ugyanaz a google fiók, ne találj ki problémákat amik nincsenek ott"*. A **fiók-elméletet elvetettük** (ugyanaz a Google-fiók), a címe **rajta van a zárt tesztelők listáján**, és a **nyílt bétából is kilépett** — a mért állapot (alpha 353 completed 100%, jó csomag) viszont **tiszta**, és a **többiek megkapták**. Ezért a hátralévő rész **kizárólag Play-oldali kiosztás** (a Google sáv-dokumentuma szerint *„it can take several hours for the changes to take effect"*). **✅ A TULAJDONOS DÖNTÉSE (2026-09-24): kivárja, nem építünk új csomagot** — a további teendő a Play-oldali frissítés (adatok törlése / újraindítás / holnapi újranézés), nem kód.
  - **✅ MEGOLDVA — A 353 MEGÉRKEZETT (2026-09-24, a tulajdonos: „na sikerült"):** a **kivárás bevált**, **kód- és csomagváltozás nélkül**. Ez pontosan az, amit a Google sáv-dokumentuma is mond: a **tesztelői/sáv-oldali változás érvényesülése órákig késhet**, és **nem** kiadáshiba. Az újramért állapot a megérkezés után **változatlan és tiszta**: `production` 352 completed 100%, `alpha` **353 completed 100%**, `beta` üres piszkozat, a feltöltött AAB-ek 353-ig, a 353 lenyomata `ce669cd6…`. **⚠️ NE vezessük le ebből azt, hogy „a Play mindig lassú, tehát várni kell"** — a lépés, ami elhozta: a **Play Áruház adatainak törlése + a meghívó link újra megnyitása** (a tulajdonos végezte). **A zárt teszt a helyes csatorna, csak a Play-oldali átfutás idejét hozzá kell számolni.** **📌 A KÖVETKEZŐ LÉPÉS innen:** eszközpróba a 353-mal (a zárt teszt kötelező a Firebase major + R8 92% miatt), majd a **353 előléptetése a production sávra** a `Könyvtárból` úton, az **1b. blokk** (352–353 összesítő) kiadási szövegével.
  - **✅ AZ ESZKÖZPRÓBA ZÖLD — A FIREBASE MAJOR + R8 92% ÁTMENT A VALÓS KÉSZÜLÉKEN (2026-09-24, a tulajdonos visszajelzése: „megy minden faszán"):** ezzel **bezárult az a korlát, amit eddig minden ide vonatkozó szakasz nyitva hagyott** — a major emelés és az obfuszkiálás 54% → **92%** ugrása **nem tört el semmit élesben** (eddig csak az volt igazolva, hogy a build lefut, a tesztek zöldek, az R8 nem jelzett hiányzó hivatkozást, és a kritikus osztályok a mappingben megvannak). **⚠️ ŐSZINTE MEGJEGYZÉS:** ez **tulajdonosi visszajelzés**, nem az én mérésem — a készüléken futó appot innen nem látom. **Amit ezért továbbra is a Console fog megmondani:** a Play saját R8-mutatója és a zárt teszt hibamentességi adatai. **✅ A KÖVETKEZŐ LÉPÉS EZZEL MEGNYÍLT:** a **353 előléptetése a production sávra** (`Könyvtárból`, az **1b. blokk** szövegével) — a döntés a tulajdonosé, a lépést ő végzi a Console-ban, az utána következő mérést én.
- **📌 „SOKAKNAK BÉTÁT ÍR A PLAY" — NEM a mi kiadásunk neve (a tulajdonos jelzése, 2026-09-23):** a **`beta` sávon nincs kiadás** (mérve: `check-play-track.mjs` — 2026-09-24-én egy **üres piszkozat** van rajta, lásd a fenti bejegyzést), ezért a felirat **kizárólag a tesztelői jogosultságból** ered: a Play a **saját tesztelői felületén** jelöli meg azt, aki **benne van a teszt listáján** — ez **Play-oldali címke**, nem a mi kiadásunk elnevezése. **Nem hiba és nem baj:** ugyanazt a **352**-t kapják, mint a nyilvános felhasználók (`alpha` = `production` = 352), a nyilvános látogatónál **nincs** ilyen felirat. **A tesztelői lista az API-ból nem mérhető megbízhatóan** (lásd a `testers` végpont tanulságát) — **a Console az irányadó**. **A felirat csak úgy tűnik el, ha az illető kikerül a tesztelői listáról, ÉS a Playben is kilép a tesztből** (addig a Play teszt-verzióként kezeli). **✅ A TULAJDONOS DÖNTÉSE (2026-09-23): „semmit most, de később térjünk vissza rá"** — a **zárt teszt marad** (ez a gyors előteszt-csatornánk a következő kiadás előtt), a `beta`/`alpha` sávhoz és a tesztelői listához **most nem nyúlunk**.
- **⚠️ VERZIÓKÓD-ÜTKÖZÉS (a tulajdonos jelzése, 2026-09-23):** a production kiadásnál a Console ezt írta: *„A verziókódot (352) már felhasználták. Próbálkozz másikkal."* — mert a **352 már fent van a zárt tesztből**, és a Play egy verziókódot **csak egyszer** fogad el. **Nem kell új AAB:** a kiadás-létrehozásnál a **`Könyvtárból`** (*Add from library*) opcióval a **meglévő** csomag választható (a tulajdonos ezt meg is találta), vagy a zárt teszt kiadásából a **`Kiadás előléptetése`** (*Promote release*) használható. **Ugyanaz a csomag több sávon is lehet** — pont ezért tesztelünk zárt körben, majd léptetünk elő.
- **⚠️ A KIADÁS BEKÜLDÉSE NEM KÉRDEZI a fióktörlési URL-t és a bírálói fiókot** — azok az **„Alkalmazás tartalma" nyilatkozatok** között vannak, **nem** a kiadás-varázslóban. **Attól még kitöltendők**, ha hiányoznak (a bírálat ellenőrzi őket). **A tulajdonos Console-jában nincs „Szabályzati program → Alkalmazás tartalma" menüpont** (mérve, képernyőképről), és **kereső sincs** — ezért a nyilatkozatok helyét **a Google bírálati levele** fogja megmutatni (linkkel), ha kellenek.
- **📌 A KIADÁS IRÁNYÍTÓPULTJÁNAK 4 JAVASLATA (nem blokkoló):** a Console *„4 művelet javasolt"*-ot ír a 352-höz; ezek **javaslatok**, a kiadást **nem tartják vissza**. **A mögöttes szabály:** a Play Console a **harmadik féltől származó SDK-k** fejlesztőinek saját megjegyzéseit jeleníti meg (*„Using third-party SDKs in your app"*, <https://support.google.com/googleplay/android-developer/answer/10358880>) — ez **tájékoztató**, nem szabálysértés és nem blokkoló. **Ami mérve mögötte van:** (1) *„a play-services-safetynet kritikus fontosságú megjegyzésével ellátott SDK-verziót használ"* — a Console pontosan meg is nevezi: **`com.google.android.gms:play-services-safetynet:18.0.0`**, a Google megjegyzése szerint *„The SafetyNet Attestation API is deprecated and has been replaced by the Play Integrity API"*. Ezt a **Firebase App Check** viszi be (a merge-elt manifestben `FirebaseAppCheckSafetyNetRegistrar`, az R8-mappingben **394 SafetyNet-osztály**), **de a mi appunk `AndroidProvider.playIntegrity`-t használ** (`main.dart` 176–179), tehát ez **használaton kívüli, becsomagolt függvény** — figyelmeztetés + kis méret, **nem funkcionális hiba** (ha a SafetyNet szolgáltatás kikapcsol, minket nem érint, mert nem hívjuk). Javítás (jövőbeli kör): a `firebase-appcheck-safetynet` komponens **kizárása** a Gradle-ből — **csak óvatosan**, mert a függőségek nyúlása az App Check-et elronthatja (a 345-es R8-tanulság mintájára: minden ilyen változás újraépítést és eszközös ellenőrzést kér). (2–3) teljes képernyős mód / elavult fullscreen API-k (Android 15 edge-to-edge) — **kozmetikai**, (4) R8-optimalizálás — **agresszívabb R8-ot nem javaslok vakon**, mert reflection-alapú SDK-kat törhet el (lásd a 345-ös keep-szabályt) — **ezt 2026-09-24-én megmértük, lásd a legfelső szakaszt: a blokkolt kód 40%-át egy Google-library beépített szabálya adja, nem a mi konfigurációnk**. **Egyik sem sürgős, és a bírálat ideje alatt semmihez nem nyúlunk.**
- **✅ A BÍRÁLÓI TESZT-FIÓK KÉSZ (2026-09-22):** a `review@hungarianhardstyle.hu` fiók létrejött az **Identity Toolkit Admin API**-val (`emailVerified: true`), és **élőben igazoltam**: az e-mail + jelszavas bejelentkezés **működik** (Firebase Auth REST, status 200), és a **szerver is elfogadta** a fiókot (`getMyLabelLibrary` → 200). **ÚJ ESZKÖZ:** `tools/create-review-account.mjs` (`--confirm` létrehoz, `--confirm --reset` új jelszót állít; a végén bejelentkezik és ellenőrzi). **⚠️ A projekt szabálya szerint a jelszót a REPÓBAN NEM TÁROLJUK** — ezért sem az AGENTS.md-ben, sem a tervben nincs benne; a tulajdonos a chatben kapta meg, és a szkripttel bármikor újat kérhet. **⚠️ Miért nem az app regisztrációjával hoztuk létre:** az e-mail-igazolást kér (link a levélben), a bírálónak viszont **azonnal** beléphető fiók kell; a profil-adatokat (név, szerepkör) az app a **bejelentkezéskor magától kiegészíti** (`bootstrapGoogleProfile`, ha a profil hiányos), ezért kézzel nem írtunk profilt. **A bírálat után** érdemes új jelszót adni vagy a fiókot törölni.
- **Ki mit tud tenni:** a **Console-only** elemeket (App content nyilatkozatok, bírálói teszt-fiók, országok, kiadás, kigördítés) **a tulajdonos** végzi; az **API-ból mérhető** ellenőrzéseket (bolt-lap, sávok, ország-egyezés, changelog, aláírás) **én** tudom futtatni a kiadás után.
- **⚠️ ŐSZINTE KORLÁT:** az **App content** nyilatkozatok állapotát (mi „Hiányos") **nem lehet az API-ból olvasni** — azokat a Console mutatja, ezért a tervben **ellenőrző listaként** vannak, nem mért tényként. A kiadás **bírálata** és a kigördülés is a Play oldalán történik; innen csak a **kiadás utáni** állapot mérhető.

### AAB **352**: vásárlási diagnosztika — a táblagépes vásárlási hiba kivizsgálásához (2026-09-22)

- **A TULAJDONOS JELZÉSE:** a **351 élesítése után** a **táblagépen** ugyanaz a vásárlási hiba jött — *„ez a tétel nem áll rendelkezésedre az országodban"* —, miközben **ugyanazzal a Google-fiókkal a telefonon már működik** a vásárlás. A Play-gyorsítótár törlése a táblán **nem** segített; az app **351**-es, a Playből (zárt teszt linkjéről) van telepítve, a fiók **rajta van a tesztelői listán**, és **egy másik appban ugyanazzal a fiókkal megy** a vásárlás.
- **AMIT MÉRTEM ÉS KIZÁRTAM (mind mérve, nem feltételezve):**
  1. **A kiadvány, amit a táblán nyitott: 12466 „Goze – TikaTika"** — a termékei **`ACTIVE`-ak**, mind a 9 országban elérhetők, az árak beállítva (`radio_mp3_320 = 550 Ft`). Tehát **nem** a termék-állapot és **nem** az ország-lista a hiba (ezt ma amúgy is javítottuk és élőben igazoltuk).
  2. **Két „termék nélküli" kiadvány NEM hiba:** a **12475 „Dutch Master – Take Some"** és a **12471 „BSW x Tankcsapda – BAZ+"** `is_free = true`, ezért **szándékosan** nincs Play-termékük. (A nyilvános `/releases` végpontból mérve.)
  3. **A 12699 „Goze – Change of Pace" `upcoming = true`** (megjelenés **2026-09-25**), a 4 terméke `DRAFT` — és az app **elrejti** a vásárlási kártyákat, ha `isUpcoming` (a kód: `if (!release.isUpcoming && …)`). Ott tehát **nincs mit venni**, amíg meg nem jelenik; a megjelenés napján a szinkron aktiválja őket.
  4. **A táblán a vásárlás gombon a MI tartalék-feliratunk állt (`550 Ft`)** → a Play **nem adta vissza a terméket** (`product == null`). Ez **elavult Play Áruházra** utalt (a termékeink az **új egyidejű-termék / `purchaseOptions`** modellt használják, amit egy régi Play Áruház nem old fel) — **a Play Áruház frissítése után meg is jelent a Play-ár**, tehát a **katalógus helyreállt**.
  5. **A maradék hiba a fizetési lépésben van, és továbbra sem magyarázott:** a frissítés után a Play-ár megjelenik, a vásárlás viszont **még mindig** az ország-üzenettel utasítódik el. **Amit innen mérni tudok, az tiszta** (termék `ACTIVE`, 9 ország, fiók jó, tesztelői jogosultság megvan) — ezért a következő lépéshez **a készüléken** kell mérni.
- **A MEGOLDÁS — a hiányzó MŰSZER (ez a 352 lényege):** a hibaüzenetet a **Play saját ablaka** írja ki, a kódunk pedig **soha nem látta** sem a nyers hibakódot, sem azt, hogy a Play egyáltalán visszaadta-e a terméket. Ezért **négy körön át következtetni** kellett. Mostantól van műszer:
  - **`lib/services/purchase_diagnostics_plan.dart` (ÚJ, tiszta):** `PurchaseDiagnosticsVerdict` (`storeUnavailable` / `queryFailed` / `noProducts` / `partialCatalog` / `catalogReady`), `purchaseDiagnosticsVerdict`, `purchaseDiagnosticsVerdictText` (magyar, **mit jelent**), `purchaseDiagnosticsLines` és `purchaseDiagnosticsText` (másolható jelentés). A jelentés **soha nem szépíti el** a nyers kódot és a Play saját üzenetét — azok a bizonyítékok.
  - **`LabelPurchaseService`:** új mezők (`lastPurchaseErrorCode/Message/ProductId/At`) — a **nyers hibakód** a purchase-streamből —, és új `diagnose(...)`, ami **megkerüli a `_catalogCache`-et** (a friss, nyers Play-válasz a lényeg), **nem indít vásárlást** és nem ír semmit.
  - **„Az appról" képernyő:** új **Vásárlási diagnosztika** szakasz (gomb → jelentés + **Másolás** a vágólapra). A termék-azonosítók a `releasesProvider`-ből jönnek; ha a katalógus nem töltődik be, **azt mondja meg**, nem hazudik mérést.
- **BIZONYÍTÉK:** `flutter analyze lib test` **tiszta**; `flutter test` **775/775** (a kör előtt 760); `test/services/purchase_diagnostics_plan_test.dart` **11/11** (verdiktek, a nyers kód és a Play üzenetének jelenléte, a **pénznem** kiírása — forint vs euró, mert a Play az **országot** a pénznemben is elárulja —, a „(nincs)" helyőrzők, a másolható szöveg) és `test/services/purchase_diagnostics_wiring_test.dart` **4/4** forrás-lint (a hibakód rögzítése a purchase-streamben; a diagnosztika **friss** választ kér, **nem** hív `buyNonConsumable`-t és **nem** használja a gyorsítótárat; a képernyőn megvan a szakasz, a másolás és a tiszta modul szövege). **Mutációs bizonyíték:** a verdikt „üres válasz" ágának kiiktatása **1 tesztet elbuktatott**, majd a fájl **byte-pontosan** visszaállt (SHA-256 `4203F66B…` egyezik).
- **Csomag:** `build/HUHS-v1.0.0+352-release.aab` (versionCode **352** — a merge-elt release manifestből visszaolvasva `processReleaseMainManifest` **és** `outputReleaseAppLinkSettings` is 352 —, 80,67 MB, SHA-256 `0396D2AC7C3E48C2FAF368ABA0F28220D50B8300E9B91E8465718085E2671974`), **1 aláírási identitás**; `check-play-notes` **MINDEN ELLENŐRZÉS RENDBEN** (currentBuild 352, `lastPublishedBuild` **351** — mert a 351 ma kiment). **A build parancsa kötelező `-P` paraméterekkel** (különben a Gradle szándékosan elhasal): `HUHS_ADMOB_APP_ID`, `HUHS_ADMOB_BANNER_ID`, `HUHS_ADMOB_REWARDED_ID`.
- **📌 A TULAJDONOS LÉPÉSE:** (1) **352 feltöltése** a zárt teszt sávra (a 351-et váltja); (2) a táblán **frissítés** a Playből; (3) **Több → Az appról → Vásárlási diagnosztika → futtatás → Másolás**, és a szöveget küldje el. Abból **egy lépésben** kiderül a hiba oka (a nyers kód és a pénznem mondja meg, hogy a Play melyik országba teszi a fiókot, illetve hogy a fizetési lépés mit utasít el).
- **⚠️ ŐSZINTE KORLÁT:** a 352 **nem javítja** a táblagépes hibát — **mérhetővé** teszi. A vásárlás sikerét továbbra sem tudtam igazolni (pénzt mozgatni csak a tulajdonos tud); amit igazoltam: a katalógus a frissítés után a táblán is rendben van, és a jelentés minden eddiginél pontosabb képet ad.

### 352: a Play-termékek ország-listája — „A tétel nem áll rendelkezésre az adott országban" (2026-09-22, **szerveroldali: AAB NEM kell hozzá**)

- **A TULAJDONOS JELZÉSE:** képernyőkép a Play „Fizetési módok" képernyőről, ahol **minden** kártyánál ez állt: *„A tétel nem áll rendelkezésre az adott országban."*, majd *„ez fura mert HU-ban vagyok"*, végül a döntő adat: **„Másik appban tudnék vásárolni."** Emellett: *„a play consoleban magyarország szerepel"*.
- **A MÉRT GYÖKÉR (élő Play API, 60 termék):** a `regionalPricingAndAvailabilityConfigs` **mind a 60 terméknél `[HU]` volt**, miközben az **alkalmazás 8 országban érhető el** (AT, CZ, HR, HU, RS, SI, SK, UA). Ezért aki nem magyar Play-fiókkal telepítette az appot, az **egyetlen tételt sem tudta megvenni**. ⚠️ **Ez nem csak a tulajdonos tesztjét akadályozta: minden külföldi vevőjelöltet elveszítettünk.**
- **⚠️ AMIT MENET KÖZBEN KI KELLETT ZÁRNI (mind mérve, nem feltételezve):**
  1. **A hibaszöveg nem a miénk.** A kódbázisban a *„Fizetési mód"* szövegre **egyetlen** találat van (a saját diagnosztikai eszközünk dokumentációja) — tehát az a képernyő a **Google Play Áruház saját** beállítása, nem az appunk.
  2. **A fiók/fizetési profil rendben van.** A tulajdonos más appban sikeresen vásárolt, ezért a „vevő országa nem HU" felvetés **nem** magyarázat — a hiba a **termékeink** konfigurációjában volt.
  3. **A Play Console „Magyarország" félrevezető.** Az a **fejlesztői (eladói)** oldal országa; a vásárlást a **vevő** fiókja dönti el (`play.google.com/store/account`), ami a Console-ban **nem is látszik**.
  4. **Az ár nem bizonyít elérhetőséget.** A gomb felirata `product?.price ?? '${configured.price} Ft'`, ezért **árt akkor is mutat, ha a Play nem adta vissza a terméket** — a letiltott gomb viszont nem kattintható (`onPressed: product == null ? null : …`), tehát a sikeres kattintás azt jelenti, hogy a Play **megtalálta** a terméket.
- **A JAVÍTÁS (tiszta szabály + egy bekötés):**
  - **`functions/play-product-plan.js`:** új **`LABEL_PRODUCT_REGIONS`** (a 8 ország), **`regionalPriceFor(baseHuf, currency)`**, **`regionalPricingConfigs(baseHuf)`** és **`mergeRegionalConfigs(current, desired)`**. A helyi árak a magyar alapárból, a **jóváhagyott létráról** (550 Ft → **1,49 EUR / 39 CZK / 169 RSD / 59 UAH**).
  - **`upsertPlayProduct`:** a régi „csak HU-t írjunk" blokk (`regionalPrices.set('HU', …)`) **eltűnt**; a kívánt régiók a modulból jönnek, a PATCH pedig **`mergeRegionalConfigs`**-szal dolgozik, ezért a **kézzel, a Play Console-ban beállított ország nem veszik el** (tulajdonosi kérés: „semmit ne kapcsolj ki").
  - **⚠️ A LEGFONTOSABB RÉSZ — a gyors-út:** a `playProductMatches` mostantól **MINDEN kívánt régiót** megkövetel (nem csak a magyart), különben a „változatlan termék ⇒ nincs írás" gyors-út **némán kihagyta** volna a régiók pótlását, és a termékek **örökre HU-only** maradtak volna. Ez **pontosan ugyanaz a hiba-osztály**, mint a vásárlási opció állapotánál (2026-09-22): a gyors-út korán visszatér, és a javítás soha nem fut le. A `regions` hiánya szándékosan **„nem egyezik"** (hangos), nem csendes kihagyás; a bekötést forrás-lint is megköveteli.
  - **`tools/check-play-products.mjs`:** kiírja az **ország-lefedettséget** (minden termék mind a 8 országban?), és a várt listát a **tiszta modulból** veszi, ezért nem tud széthúzni. **⚠️ A sávok ország-listája mostantól opt-in (`--tracks`)**, mert az edit-API-t hívja, és egy nyitott edit a Play Console piszkozatát érvénytelenítheti — az alapértelmezett futás **100%-ban olvas**.
- **BIZONYÍTÉK (négy szinten):**
  1. **Tiszta tesztek:** `functions/play-product-plan.test.cjs` **32/32** (a kör előtt 22) — a helyi árak **konkrét értékkel** rögzítve, a HU-only termék **eltérésnek** számít, az egyetlen hiányzó ország / rossz ár / nem elérhető régió is eltérés, a kézzel beállított ország megmarad, az érvénytelen alapár **hangos hiba**, és forrás-lint a bekötésre + a 8 országra.
  2. **Mutációs bizonyíték:** a `LABEL_PRODUCT_REGIONS` **HU-only-ra szűkítésével 9 teszt elhasalt**, majd a fájl **byte-pontosan** visszaállt (SHA-256 `196DE42A…` egyezik).
  3. **ÉLŐ, valódi úton mért javítás:** a függvények telepítése (`Deploy complete!`) után **nem** nyúltam kézzel a termékekhez — az **ütemezett szinkron** végezte a pótlást, és ezt **végigmértem**: `0/60 → 4/60 → 27/60 → 48/60 → 60/60`. A záró mérés: `check-play-products.mjs` **EXIT=0**, „Országok, ahol LEGALÁBB egy termék elérhető: **8**" (mind a 8 országnál **60 termék**), és a nyers Play-válasz egy 550 Ft-os termékre: `HU 550 HUF, AT/HR/SI/SK 1,49 EUR, CZ 39, RS 169, UA 59` — mind `AVAILABLE`.
  4. **Ami NEM történt meg:** a **4 `DRAFT` termék** (a 12699-es, `is_upcoming` kiadásé) állapota **változatlan** — a megjelenés előtt szándékosan nem vásárolható.
- **⚠️ AZ APP ÉS A PLUGIN VÁLTOZATLAN:** ez **szerveroldali + Play-adat** javítás, ezért **nem kell új AAB** (a 351 marad érvényben), és a plugin sem változott. A tulajdonosnak **nem kell feltöltenie semmit**.
- **⚠️ ŐSZINTE KORLÁT:** a **vásárlás sikerét** (pénzt mozgatva) nem tudtam igazolni — az csak a tulajdonos készülékén, valódi fiókkal mérhető. Amit igazoltam: a termékek **mind a 8 országban megvásárolhatók** a Play saját adatai szerint, mind a 60-nál, a helyes helyi árakkal. A tulajdonosnak érdemes **egyszer** megismételnie a vásárlást; ha az továbbra is hibázik, akkor a fiókja országa **nincs benne** az app 8 országában (ezt a `play.google.com/store/account` mutatja meg).
- **📌 KÖVETKEZŐ KÖR, HA KELL:** a `newRegionsConfig` **egyik terméknél sem** van beállítva (mérve) — ez csak a **jövőben hozzáadott** Play-régiókra hat, ezért most szándékosan nem nyúltam hozzá. Ha az app forgalmazási országai bővülnek, a `LABEL_PRODUCT_REGIONS` listát **is** bővíteni kell; a `check-play-products.mjs --tracks` pont ezt az eltérést jelzi.
- **✅ A TULAJDONOS ESETÉNEK LEZÁRÁSA (2026-09-22, mérve):** a hibás telefonon a fiók **`djdeeroy@gmail.com`**, az országa **magyar**, és az app **a Playből** van telepítve — mégis **minden** tételre ugyanaz a hiba jött, **a javítás után is**.
  - **A DÖNTŐ PRÓBA (a tulajdonos végezte):** **ugyanezzel a fiókkal egy MÁSIK appban is ugyanaz a hiba jött** → **nem az appunk és nem a termékeink** (ez az egyetlen lépés, ami egyből szétválasztja az app-oldali és a fiók-/készülék-oldali okot).
  - **✅ A VALÓDI MEGOLDÁS (a tulajdonos, 2026-09-22):** a **Play Áruház gyorsítótárának (adata) törlése** után a vásárlás **megjavult**. Vagyis a beragadt állapot a **készülék Play-állapotában** volt — **nem** a fiók országában és **nem** a fizetési profilban. ⚠️ **A saját következtetésemet itt javítom:** a „másik appban is hibázik" tény a **fiók-/készülék-oldali** okot bizonyítja, de **nem** azt, hogy az a fizetési profil — a legolcsóbb és leggyakoribb ok a **Play-gyorsítótár**, ezért azt kell **először** próbálni.
  - **📌 A HELYES DIAGNOSZTIKAI SORREND (ez a kör tanulsága):** (1) **másik app, ugyanaz a fiók** → app-oldali vagy fiók-/készülék-oldali; (2) **Play Áruház adatának/gyorsítótárának törlése** (Beállítások → Alkalmazások → Google Play Áruház → Tárhely → *Adatok törlése*) — ez a legolcsóbb javítás; (3) a fiók országa (`play.google.com/store/account`) és a **fizetési profil** (`payments.google.com`) — a kártyák kibocsátási országa egyezzen a profillal; (4) végül a termékek ország-listája (ez volt a mi oldalunkon a **valódi** hiányosság, és ma javítva).
- **✅ A SÁVOK MÉRÉSE (`node tools/check-play-products.mjs --tracks`, a tulajdonos engedélyével):** az **`alpha`** sáv **pontosan a 8 ország** (AT, CZ, HR, HU, RS, SI, SK, UA), `restOfWorld=false`, `syncWithProduction=false` → **a termékek listája pontosan az app terjesztését fedi, nincs eltérés**. A **`beta`** sáv `syncWithProduction=true` (a production listáját használná, ami **üres**), a **`production`** 0 ország (nincs publikálva), az **`internal`** sáv ország-listát nem is támogat („Track internal does not support country availability"). **Ebből következik:** a `countryavailability` válaszában a **`syncWithProduction`/`restOfWorld` mezőt MINDIG ki kell írni** — e nélkül egy szinkronizáló sáv „0 országa" **hamis riasztást** ad, és a drift-ellenőrzés is értelmezhetetlen (ezért most kihagyja az egyeztetést, ha a sáv production-nel szinkronizál).
- **✅ AMI A TULAJDONOSNÁL VOLT — ÉS MEGOLDÓDOTT (2026-09-22):** a **`djdeeroy@gmail.com`** fiók/telefon kombinációnál a **Play Áruház adatának törlése** után a vásárlás **azonnal működött**. **Nálunk nincs teendő**, és nincs mit javítani az appban vagy a termékekben.
  - **⚠️ HA EZ MÁSKOR ELŐJÖN (a bevált sorrend):** (1) Play Áruház **adat/gyorsítótár törlése**; (2) **másik app, ugyanaz a fiók** — ez választja szét az app- és a fiók-oldali okot; (3) fiók országa (`play.google.com/store/account`); (4) fizetési profil (`payments.google.com`) + a kártyák kibocsátási országa; (5) a termékek ország-listája (ez a mi oldalunk — ma már mind a 8 országban rendben).
  - **📌 AMI VISZONT MEGMARAD:** a vásárlás a `verifyLabelPurchase` után a **HUHS-fiókhoz** kerül (nem a Google-fiókhoz), ezért ha valaki más Google-fiókkal vásárol, a zene attól még **a saját HUHS-fiókján** lesz elérhető — ez a támogatásnál hasznos érv, és egyben a „másik fiókkal vettem, mégsem látom" típusú kérdés magyarázata (ilyenkor az **app-fiók** számít).
- **✅ HOLLANDIA HOZZÁADVA (2026-09-22, a tulajdonos kérésére: „hozzácsaphatnánk a hollandokat is"):** a `LABEL_PRODUCT_REGIONS` **9 országra** bővült (`NL`, EUR) — a lista így: HU, AT, HR, SI, SK, **NL**, CZ, RS, UA. **Új árdöntés nem kellett:** Hollandia EUR-t használ, és a jóváhagyott euró-ár érvényes rá is.
  - **⚠️ A SORREND SZÁMÍT (és ezt be is tartottuk):** előbb a **termékek** kapták meg az országot (szerveroldal, tisztán hozzáadás), és **csak utána** kerül be a Play Console-ban a **sáv ország-listájába**. Fordítva lenne egy ablak, amiben valaki **telepíteni tud, de vásárolni nem** — pontosan az a rés, amit ebben a körben javítottunk.
  - **⚠️ NEM KELL HOZZÁ ÚJ AAB:** a sáv ország-listája **Play Console-beállítás**, nem kliens-kód — a 351 marad érvényben, és a plugin sem változott.
  - **BIZONYÍTÉK (mérve):** `functions/play-product-plan.test.cjs` **32/32** (a helyi árak konkrét értékkel — ebben a `NL: 1,49 EUR` is —, a régió-lista pontosan, és a `NL` literál forrás-lintje: az `index.js`-ben **nem** lehet ország-besorolás); a függvények telepítve (`Deploy complete!`); az **ütemezett szinkron** pótolta mind a 60 terméknél, végigmérve: `0/60 → 2/60 → 30/60 → 57/60 → 60/60`. A záró mérés: **mind a 60 termék elérhető `NL`-ban, és mind a 9 országban**, a holland árak: **1,49 EUR (30 termék)** és **1,99 EUR (30 termék)** — ez pontosan a két magyar alapar (550 és 700 Ft) létra szerinti euró-ára, tehát az átszámítás konzisztens.
  - **📌 A TULAJDONOS LÉPÉSE:** Play Console → **Tesztelés → Zárt tesztelés → az `alpha` sáv → Országok/régiók → Hollandia** hozzáadása (mentés). Utána a `node tools/check-play-products.mjs --tracks` **megmondja, hogy a sáv listája pontosan a termékek 9 országát fedi-e** — ha nem, ott van a figyelmeztetés, és a `LABEL_PRODUCT_REGIONS`-t kell igazítani. **Amíg ez a lépés nincs meg, Hollandiában senki nem tudja telepíteni az appot** (a termékek viszont már készen állnak, tehát a hozzáadás pillanatától **azonnal vásárolható** is).
...
- **A TULAJDONOS DÖNTÉSEI (kérdező ablakokban):** a „Claim" magyar neve **„Adatlap átvétele"** (gomb: „Adatlap átvétele", átvett állapot: „Ez a te DJ-adatlapod." + „Átvétel visszavonása", foglalt: „Ezt a DJ-adatlapot már átvette egy fiók.", kártya: „Átvett DJ-adatlap"); a DJ **szövegeket + közösségi linkeket + KÉPET** szerkeszthet; az értesítés-törlés **kijelölés mód gomb a fejlécben**; a régi changelog-bejegyzéseket **nem** írjuk át (csak az élő felületeket és az új bejegyzést).
- **1. A GÖRGETÉS GYÖKERE (mérve a kódban):** a `CommunityPublicProfileScreen` `ListView`-je `EdgeInsets.all(20)`-szal ért véget, **rendszer-alsó-sáv helye nélkül** — a hír-, esemény-, DJ- és szervező-adatlap viszont már használt ilyet (`MediaQuery.viewPaddingOf(context).bottom + 24/28`). A saját profil a leghosszabb (átvett adatlapok, kedvenc DJ-k, megjelölt események), ezért pont ott látszott: az utolsó kártya a rendszer sávja alatt maradt. **Javítás:** új, tiszta **`lib/core/layout/scroll_bottom_inset.dart`** (`scrollBottomInsetFor`, `scrollBottomInset`, `ScrollBottomInset` widget), és mind az **öt** érintett képernyő ezt használja — a pixelértékek változatlanok (+24/+28), csak egy helyen élnek.
- **2. AZ ÁTVETT DJ-ADATLAP SZERKESZTÉSE — három réteg, egy szerződés:**
  - **Plugin 2.7.0:** új **`POST /huhs/v1/dj-profile/<id>`** végpont (szándékosan **nem** `/artists/...` és **POST**, hogy a nyilvános gyorsítótár ne érinthesse; `permission_callback = manage_options`, mert a HUHS szervere hívja az admin-alkalmazásjelszóval). Ír: `post_title` (≤120), `post_content` (bemutatkozás, ≤6000, `wpautop(esc_html())` — HTML nem mehet be), `real_name/city/country/website/facebook/instagram/tiktok/spotify/soundcloud/youtube`, valamint `logo_url`/`hero_image_url` + az **attachment-azonosító nullázása** (különben a nyilvános válasz a régi képet adná). **Nem** ír: `booking_email` (ez igazolja az átvételt), `contact_email`, `visible`, `featured`, `booking_via_huhs`, `genre`, taxonómiák, slug. A válasz a **nyilvános payload** (`huhs_build_artist_response`), ezért az app egyből frissíthet. Meta-írásnál **kifejezetten** hívjuk a `huhs_invalidate_public_cache()`-t (a meta-írás magától nem ürít).
  - **Szerver:** új **`updateClaimedArtistProfile`** callable — bejelentkezés + `allowCall(uid,'artist_profile_edit',10)`, majd az **átvétel ellenőrzése** (`artist_claims/<id>.uid === context.auth.uid`), a bemenet szűrése a **tiszta `functions/artist-profile-plan.js`**-ben, végül a WordPress-hívás. A booking/privát cím és a ház döntései **a tiszta modul kimenetébe sem kerülhetnek** (a kliens is szűri: `artist_profile_form.dart`).
  - **Kliens:** az adatlap `claim.mine` állapotában „Adatlap szerkesztése" gomb → új `ArtistEditScreen`; a kép a **meglévő** Cloudinary-úton megy (`WordpressService.uploadProfileImage`, ugyanaz a validáció, mint a beküldésnél), és csak a **megváltozott** mezők mennek ki. A képernyő meg is mondja, mi nem szerkeszthető.
- **3. A „CLAIM" MAGYARÍTÁSA:** minden látható szöveg „Adatlap átvétele" alakot használ (artist adatlap + profil-kártya + SnackBar-ok). **A belső nevek szándékosan változatlanok** (`claimArtistProfile`, `ArtistClaimStatus`, `artist_claims`, `claim-emails`) — a callable átnevezése szerver+kliens egyeztetést igényelne haszon nélkül. A 348-as changelog-bejegyzés (és a Play-en kint lévő szöveg) **marad**, mert az már kiment a felhasználókhoz.
- **4. ÉRTESÍTÉS-KIJELÖLÉS:** új tiszta **`lib/services/notification_selection_plan.dart`** (`toggleNotificationSelection`, `deletableNotificationIds`, `pruneNotificationSelection`, `notificationSelectionLabel`, `notificationDeletedLabel`) + `NotificationService.deleteIds()` (450-es kötegekben). A fejlécben pipa ikon indítja a kijelölést; ilyenkor a sorra koppintás **jelöl** (nem nyit meg), és ott van „összes kijelölése" + „kijelöltek törlése". **⚠️ Biztonság:** a törlésre csak a **saját** sorok azonosítója mehet ki (`recipientUid == uid`) — a `AppNotification` modell ezért megkapta a `recipientUid` mezőt, és a Firestore-szabály is ugyanezt kéri (`allow delete: if isRegistered() && resource.data.recipientUid == request.auth.uid`). Fület váltva a kijelölés törlődik.
- **⚠️ BIZTONSÁGI JAVÍTÁS A SAJÁT 2.6.0-UNKBAN (megtalálva, bizonyítva, javítva):** a nyilvános válasz-gyorsítótár engedélylistája (`huhs_public_cache_route()`) **előtagra** illeszkedett, ezért a `/huhs/v1/artists/<id>/claim-emails` — amely a DJ **privát** e-mail címét adja vissza — is cache-elhető volt; a `rest_pre_dispatch` (és a plugin-betöltéskori korai kiszolgálás) pedig a **hitelesítés előtt** fut, így egy korábbi hitelesített hívás válasza **120 másodpercig azonosítatlan kérésre is kijöhetett volna**. A fájl saját feltétele szerint ide csak `permission_callback => '__return_true'` útvonal kerülhet. **Fix:** a `claim-emails` és a `dj-profile` útvonalak **kifejezetten kizárva**. (A javítás a plugin 2.7.0-ban van, ezért **fel kell tölteni**.)
- **BIZONYÍTÉK:** `flutter analyze lib test` **tiszta**; `flutter test` **685/685** (a kör előtt 650); szerver-tesztek **248/248** (a `registration.integration.test.cjs` kivételével, ami App Check-token hiányában bukik — lásd a 349-es szakaszt); `check-play-notes` **MINDEN ELLENŐRZÉS RENDBEN**; 1 aláírási identitás; az AAB `libapp.so`-ban benne az új kód (`ScrollBottomInset`, `ArtistEditScreen`, `notificationSelectionLabel`, „Adatlap"). **Új tesztek:** `test/core/scroll_bottom_inset_test.dart` (6), `test/services/notification_selection_plan_test.dart` (10), `test/widgets/hungarian_claim_wording_test.dart` (3), `test/services/artist_profile_form_test.dart` (15), `functions/artist-profile-plan.test.cjs` (13).
- **⚠️ A PLUGIN KÓDJA NEM FUTOTT (nincs PHP a gépen):** a `php -l` lint **nem elérhető**, ezért a plugin ellenőrzése: **zárójel/kapcsos-zárójel egyensúly** mindhárom módosított fájlon (OK), kézi átnézés, és **forrás-lint** a `functions/artist-profile-plan.test.cjs`-ben (útvonal + POST + `manage_options`, a tiltott mezők hiánya, a Cloudinary-szabály, a cache-kizárás, a verzió 2.7.0). A PHP-szintaktika tehát **nem** futásidejűleg igazolt.
- **Csomagok:** `build/HUHS-v1.0.0+350-release.aab` (versionCode **350**, 80,58 MB, SHA-256 `0CA04B764B825EAD4ED0EB08913377A3F1246B8616F79D3F7D62B40B3F90D215`) **és** `build/huhs-mobile-api-2.7.0.zip` (45 fájl, 149,3 KB, SHA-256 `25900509936BBEF8AA8A8E8FC0730B662D2F41B3BF5838CAA575ED9B26B0047C`). A plugin különbsége átnézhető: `docs/plugin-2.7.0-dj-profile-edit.patch`. **Mindkettőt a tulajdonos tölti fel**; a **függvények már telepítve** (`Deploy complete!`, benne az új `updateClaimedArtistProfile`).
- **⚠️ A PLAY ÁLLAPOTA (mérve):** a zárt teszt sávján a **349 van élesben** (completed), a feltöltött AAB-ek listája 349-ig tart — ezért a `play-notes-meta` `lastPublishedBuild: 349`, és a rövid (1.) blokk **csak a 350 újdonságait** írja le.
- **✅ A PLUGIN 2.7.0 FELKERÜLT ÉS ÉLŐBEN IGAZOLVA (2026-09-22, a tulajdonos „2.7.0 fent" jelzése után):** `apiVersion = 2.7.0` (`verify-submission-payout.mjs --live` → **4/4 OK**), és az **új `--probe` mód** (`check-artist-claims.mjs --probe 12812`) **5/5 OK**, adat írása nélkül: privát `claim-emails` hitelesítés nélkül **401**, új `dj-profile` hitelesítés nélkül **401** (nem 404 → fent van és védett), `claim-emails` hitelesítéssel **200**, `dj-profile` üres kéréssel **400** (nem ír semmit), a nyilvános `/artists/<id>` pedig **200 + `X-HUHS-Cache=fresh`** (a kizárás nem vitte el a nyilvános gyorsítótárat). `verify-wp-admin-endpoints.mjs` → **15/15 OK**. **⚠️ Közben a saját eszközünk hibáját is javítottuk:** a `verify-submission-payout.mjs` a verziót csak az **utolsó** száma alapján hasonlította (`2.7.0` → `0 >= 8` = hamis), ezért a 2.5.8 utáni **minden** kiadást hibásnak jelzett — most **`versionAtLeast()`** van (10 új önteszt-eset, a kapu 15/15).
- **⚠️ AMI MÉG HÁTRA VAN A 350-BŐL:** a **350-es AAB a Play ellenőrzésén van** (a tulajdonos jelzése), ezért az **eszközön mért** igazolás (görgetés, kijelölés, adatlap-szerkesztés, képcsere) az **elfogadás után** esedékes; és a **mentés** élő próbája (egy átvett adatlap szerkesztése az appból) szintén akkor.
- **⚠️ ŐSZINTE KORLÁT:** futásidejű, **eszközön mért** igazolás nincs (a görgetés, a kijelölés, a szerkesztő űrlap és a képcsere a tulajdonos készülékén mérhető) — a szerveroldal viszont **élőben igazolt**.

### Vásárlások HELYREÁLLÍTÁSA — a szerveroldali keret (2026-09-22, **szerveroldali: AAB NEM kell hozzá**)

- **A TULAJDONOS DÖNTÉSEI (kérdező ablakokban, 2026-09-22):** (1) a kivitel **„A + B"** = kézi gomb a Beállításokban **ÉS** automatikus háttéregyeztetés; (2) ha a vásárlás **más app-fiókhoz** van kötve, a felület **magyar magyarázatot** adjon, és az **ne tűnjön hibának**; (3) **„előbb csak a szerveroldali rész"** — a kliens (A + B) a következő körben, új AAB-ként. **A teljes terv a `docs/VASARLAS-HELYREALLITAS-TERV.md`-ben van** (mért tények, szerkezeti gyökér, három kivitel, kiviteli + teszt-terv, a döntések 5b. pontban).
- **⚠️ A MEGOLDANDÓ PROBLÉMA (mérve):** a `verifyLabelPurchase` **10 hívás / 60 másodperc / fiók** kerettel futott (`allowCall(uid, 'label_purchase', 10)`), a helyreállítás viszont **egyszerre több** birtokolt terméket ellenőriz — **10-nél több** megvásárolt kiadványnál a 11. már `resource-exhausted` lett volna, vagyis a funkció a legaktívabb vásárlóknál **el sem indult volna**.
- **A MEGOLDÁS (tiszta döntés + két vödör):** új `purchaseVerificationBudget({ restore })` a `functions/label-library-plan.js`-ben. A `verifyLabelPurchase` mostantól:
  - **normál út:** `label_purchase` **10/perc** — **VÁLTOZATLAN** (a `restore` jelző hiányában pontosan a régi viselkedés),
  - **helyreállítás (`restore: true`):** `label_restore` **20/perc**,
  - **ÉS minden ellenőrzés ezen felül a KÖZÖS `label_verify` 20/perc összkeretből is fogy** — így a `restore` jelző **nem** válik a korlát megkerülésének eszközévé (a két út együtt sem mehet 20/perc fölé).
  - A `restore` szándékosan **csak szigorú `true`** esetén számít (hiányzó/más érték = normál út). A beégetett régi keret **eltűnt** a végpontból, hogy a modul és az éles viselkedés ne tudjon csendben széthúzni.
- **⚠️ MIÉRT NEM KELL HOZZÁ ÚJ VÉGPONT:** a `verifyLabelPurchase` már most a helyes dolgot teszi, ha a **helyes** `releaseId`-t kapja (a szerver kötelezően egyezteti a termék-azonosítóval), a token-egyediség pedig érintetlen (`label_purchase_claims`, *„Ez a vásárlás már másik felhasználóhoz tartozik."*) — **egy vásárlási token csak EGY fiókhoz tartozhat**, tehát a helyreállítás **nem** adható arra, hogy egy Google-fiókkal több app-fiókot lássunk el ingyen zenével. A kliens a `getMyLabelLibrary`-ből tudja, mi van már meg, ezért felesleges hívást nem indít.
- **BIZONYÍTÉK (két szinten, mindkettő valódi hívás/viselkedés):**
  1. **Tiszta tesztek:** `functions/label-library-plan.test.cjs` **16/16** (5 új: a régi keret változatlansága — köztük a `restore: 'true'`/`1` **nem** számít helyreállításnak —, a külön vödör 20/perc, a közös összkeret, a keretek egy helyen élnek, és forrás-lint a bekötésre + a token-egyediség megmaradására).
  2. **ÉLŐ, emulátoros viselkedés-teszt (ÚJ: `functions/label-purchase-limit.test.cjs`, 5/5):** valódi callable-hívásokkal méri, hogy a **11. normál hívás `RESOURCE_EXHAUSTED`** (a régi keret nem lazult), a **11. helyreállító hívás még megy**, a **21. hívás `RESOURCE_EXHAUSTED`** (közös összkeret), és a **kevert út (10 + 10 + 1) sem megy 20 fölé**. A hívások szándékosan hibás tokennel mennek (a Google Play-t nem terheljük), ezért a teszt a „**nem keret-blokk**" állítást méri.
  3. **Mutációs bizonyíték:** a `restore` vödör elvétele **2 tesztet**, a közös összkeret törlése **1 forrás-lintet** buktatott, majd mindkét fájl **byte-pontosan** visszaállt (SHA-256 egyezik).
- **A FÜGGVÉNYEK TELEPÍTVE** (`npx firebase deploy --only functions` → *Deploy complete!*), a `firebase functions:list` szerint a `verifyLabelPurchase` fent van (v1, callable, us-central1, nodejs22).
- **⚠️ AZ APP VÁLTOZATLAN:** a `restore` jelzőt **még senki nem küldi** (a kliens-oldali rész a következő kör), ezért **a 349-es AAB marad érvényben** — ez a kör kizárólag szerveroldali.
- **📌 A KÖVETKEZŐ KÖR (kliens, A + B, új AAB ~350):** `lib/services/label_restore_plan.dart` (tiszta: termék-azonosító → kiadvány, „már megvan" kiszűrés a `getMyLabelLibrary`-ből, 8-as darabolás, magyar összegzés) + fiókszintű reconciler a `LabelPurchaseService`-ben (a `purchaseUpdates` streamből, **nem** a képernyő visszahívásaiból) + „Vásárlások helyreállítása" kártya a Beállításokban + automatikus futás a „Megvásárolt zenéim" megnyitásakor + a hívás **`restore: true`** jelzővel.

### Jutalmazott `free_link` feloldás, tabletes kiadvány-adatlap, villogó listák (2026-09-21, AAB **349**)

- **A TULAJDONOS NÉGY JELZÉSE + EGY ÖTLETKÉRÉS:** (1) *„a jutalmazott külső linkes ingyenes kiadvány feloldása nem működik"*, (2) *„tableten fekvő nézetben ha megnyitok egy kiadványt, rohadt nagy a cover és frán nagy minden"*, (3) *„djk, szervezők listája még mindig villog néha"*, (4) *„a megvásárolt zenék között ne látszódjon a már eltávolított kiadvány, felesleges"*, (5) *„lehet kéne vásárlások visszaállítása rész, de nemtudom ez jó ötlet e (a beállításokba) és ezt hogy lehetne kivitelezni, erre kérek ötleteket"* → **az 5. pont szándékosan CSAK ötletkérés, abból ebben a körben SEMMI nem épült meg.**
- **1. A `free_link` FELOLDÁS — A GYÖKÉR A SZERVEREN VOLT (élesben mérve):**
  - A `LABEL_VARIANTS` lista **nem tartalmazza** a `free_link`-et (az nem fájl, hanem **külső link**, ezért a „lejátszható változatok" listájában nincs helye) — az `activeAdUnlock` viszont **pont ezzel a listával** döntött: `adUnlockedVariants(data, releaseId).includes(variant)`. Ezért a `free_link`-re a kapu **soha, egyetlen esetben sem** mondott igazat.
  - **A tünet innen következik:** a kliens a jutalmazott reklám után `waitForAdUnlock`-kal vár, a szerver viszont **örökre `false`-t** ad → 20 másodperc után *„A reklám lefutott, de a feloldás nem érkezett meg."* Vagyis **nem a kliens és nem a reklám** volt a hibás.
  - **ÉLES ADAT (mérve, `label_ad_unlocks`):** **22** feloldási rekord, ebből **1** tartalmazott `variants.free_link`-et (kiadvány **12405**, uid `07527e2c…`). A **régi** kapu ezt **1/1 elutasította**, az **új** kapu **1/1 elfogadja** — a hiba tehát nem elméleti volt.
  - **A JAVÍTÁS (a döntés tiszta modulba került):** új `adUnlockGrantsVariant(unlock, releaseId, variant)` a `functions/label-library-plan.js`-ben — **pontos változat-egyezést** kér (tehát a `free_link` is számít), és a **változat nélküli, régi** rekordot `mp3_128`-ként értelmezi (visszamenőleges kompatibilitás). Az `adUnlockedVariants` **változatlan** marad: az továbbra is a **lejátszható** változatok listája, amit a könyvtár használ — a kettő szándékosan **más kérdés**.
  - **⚠️ ÁLTALÁNOS TANULSÁG:** a `label-library-plan.js`-ben azért van **két** függvény, mert két külön kérdés: *„mit lehet lejátszani?"* (fájlváltozatok) és *„erre a változatra van-e jogosultság?"* (bármelyik változat, akár link). Aki ezt a kettőt újra összevonja, **visszahozza a hibát** — a `label-library-plan.test.cjs` ezért **mindkettőt** külön méri.
- **2. TABLETES FEKVŐ NÉZET — A KIADVÁNY ADATLAPJA:** a `release_detail_screen.dart` törzse eddig **teljes szélességben** rajzolt, ezért fekvő nézetben a **négyzetes borító** a képernyő szélességével együtt nőtt (1200 px-es tableten ~1200 px magas kép). **Javítás:** `LayoutBuilder → Align(topCenter) → ConstrainedBox(maxWidth: landscape ? 1100 : double.infinity)` a tartalomra, a borító pedig `Align(centerLeft) + ConstrainedBox(maxWidth: landscape ? 360 : double.infinity) + AspectRatio(1)`. Az **álló nézet szándékosan változatlan** (ott a tulajdonos szerint jó) — ugyanaz a minta, mint a 347-es hírkártyáknál.
- **3. A VILLOGÓ LISTÁK — a gyökér a Riverpod `reload`-ja:** a DJ- és szervező-listák providerei a globális `publicContentRefreshProvider`-t **`ref.watch`-olták**, ezért minden WordPress-fejcach-írás (`wordpress_head_cache.dart:137/181` → `publicContentRefreshGeneration.value++`) **újratöltést** indított, a `when()` pedig alapból `skipLoadingOnReload: false`-szal a **loading** ágra váltott → a kész lista helyett **spinner villant**. **Javítás két rétegben:** `ref.listen(...) → ref.invalidateSelf()` a négy providerben (artists, artistDetail, organizers, organizerDetail), **és** `skipLoadingOnReload: true` a négy képernyő `when(...)` hívásán — így az új adat megérkezéséig a **korábbi lista a helyén marad**. **⚠️ UGYANEZ A MINTA MÁS PROVIDEREKBEN IS MEGVAN** (news, events, games, voting, releases, prize, poll) — azokat ebben a körben **szándékosan nem** érintettem (nincs róluk panasz, és minden érintés kockázat).
- **4. A MEGVÁSÁROLT ZENÉKBŐL ELTŰNT KIADVÁNY:** eddig az ilyen kiadvány **kártyát** kapott („Ez a kiadvány már nem elérhető"), ami a listában **zaj**. **Javítás:** új tiszta `visibleLibraryItems(items, {unavailable})` a `lib/services/label_library_plan.dart`-ban, és a `my_music_screen.dart` ezt használja a **darabszámhoz, a fejléchez és a ciklushoz** is; a `_buildUnavailableCard` **teljesen törölve**. **⚠️ A feloldás nem vész el:** a szűrés **csak megjelenítés** — ha a kiadvány visszakerül a nyilvános listába, **újra megjelenik**, a letöltött fájl pedig a készüléken marad.
- **BIZONYÍTÉK:** `flutter analyze lib test` **tiszta**; `flutter test` **650/650**; a **szerver-tesztek** (`npx firebase emulators:exec --only firestore,auth,functions --project demo-huhs "node --test --test-concurrency=1 …"`, a `registration.integration.test.cjs` **kivételével**) **225/225 zöld**, benne a `label-library-plan.test.cjs` **11/11** (a `⚠️ a free_link … feloldása MŰKÖDIK` és a „pontos változat-szabály" teszt) és az `artist-claim-plan.test.cjs` **18/18**; `node tools/check-play-notes.mjs` **MINDEN ELLENŐRZÉS RENDBEN**; `check-signing-identity` 1 identitás. **Új teszt:** `test/widgets/release_landscape_and_flicker_test.dart` (**6/6**, forrás-lint a **mind a négy** javításra: fekvő szélesség-korlátok, `ref.listen` a `ref.watch` helyett, `skipLoadingOnReload`, `visibleLibraryItems`, `_claimReward('free_link')`), és **átírt** lint a `test/services/label_library_plan_test.dart`-ban (a régi a `_buildUnavailableCard`-ot követelte meg — az maga volt a hiba).
- **⚠️ KÉT, A 349-TŐL FÜGGETLEN LELET A TESZTEKNÉL (mindkettő MÉRVE, nem feltételezve):**
  1. **JAVÍTVA — `functions/article-comments.test.cjs` 3/10-ről 11/11-re.** A teszt a `vm.runInNewContext`-be az `index.js` **szeletét** tölti be (`exports.articleComments =` … `const WORDPRESS_BASE_URL`, azaz a 604–789. sor), a `recordDailyActivity` viszont a **2996.** sorban él — a 2026-09-20-i napi-aktivitás kör óta a hozzászólás-létrehozás hívja, ezért a szeletből **hiányzott**, és `ReferenceError: recordDailyActivity is not defined`-dal **mind a 7 írás-teszt elhalt**. **A javítás:** a teszt-környezet megkapta a `recordDailyActivity` **stubot** (a szelet nem tartalmazhatja a valódit), **DE** az új teszt a **VALÓDI forrásból** is igazolja a hívást és a függvény létezését — így a stub nem tudja csendben elfedni, ha a hívás eltűnik. **⚠️ Ez teszt-oldali hiba volt, NEM éles hiba:** az éles függvény a teljes fájlt tölti be, ott a `recordDailyActivity` definiálva van.
  2. **NEM JAVÍTVA, DOKUMENTÁLVA — `functions/registration.integration.test.cjs` 6/11 bukik** (`AssertionError: {"error":{"message":"Unauthenticated"}}`), mert a `checkRegistrationEligibility` **`enforceAppCheck: true`**-val fut, az emulátoros hívás pedig nem ad App Check-tokent (a fájl `GCLOUD_PROJECT` alapértéke is `hungarian-hardstyle`, nem a `demo-huhs`). **Ez környezeti/korlátozott-integrációs korlát, nem a 349 következménye** (a javított kapu a `verifyLabelPurchase`/`getLabelDownloadUrl`/`getMyLabelLibrary` útvonalon van, az `activeAdUnlock` pedig ehhez a folyamathoz hozzá sem ér). Aki zöldre akarja hozni, annak App Check debug-tokent kell adnia a hívásokhoz.
- **📌 AZ 5. PONT (vásárlások visszaállítása) — MEGTERVEZVE, DE NEM MEGÉPÍTVE:** a tulajdonos **ötletet** kért, nem kódot, ezért a döntéshez szükséges **teljes terv** a **`docs/VASARLAS-HELYREALLITAS-TERV.md`**-ben van (mért tények + a szerkezeti gyökér + három kivitel + kiviteli és teszt-terv + a tulajdonosnak szóló 4 döntési pont). **A lényeg három mondatban:** (1) a jogosultság **nem veszik el** — a Playnél a vásárlás örökre megvan, a `label_entitlements` pedig a szerveren; (2) a **valódi rés** a **félbemaradt vásárlás** (a Playnél sikerült, nálunk nem futott le a `verifyLabelPurchase`) — ezt ma csak szerencsével lehet helyrehozni (meg kell nyitni **pont azt** a kiadványt és az **árra** koppintani); (3) a gyökér **szerkezeti**: a `release_detail_screen` a **nyitott** kiadvány termékeihez szűr (`_knownProductIds`) és a **nyitott** `releaseId`-vel ellenőriz, ezért a Play visszaadott **többi** vásárlása **némán elveszik** — a helyreállításnak a **szolgáltatásba** kell kerülnie, nem a képernyőbe.
  - **⚠️ A LEGFONTOSABB TÉNY, AMIÉRT A FUNKCIÓ BIZTONSÁGOS:** a szerver **már most** megvédi a visszaéléstől — egy vásárlási token (`purchaseTokenHash`) **csak egy fiókhoz** tartozhat, más fiókkal *„Ez a vásárlás már másik felhasználóhoz tartozik."* (functions/index.js:5469-5505). Tehát a gomb **nem** adható arra, hogy egy Google-fiókkal több app-fiókot lássunk el ingyen zenével. Pontot sem ad kétszer (`release-purchase:<productId>` naplókulcs).
  - **⚠️ A KORLÁT, AMIT KEZELNI KELL:** a `verifyLabelPurchase` **10 hívás / 60 másodperc / fiók** (`allowCall(uid, 'label_purchase', 10)`, a vödör fix 60 s). **10-nél több** megvásárolt kiadványnál a naiv „mindent visszaállítok" a 11.-nél `resource-exhausted`. A terv ezért **külön szerver-kulcsot** (`label_restore`) **és** kliens-oldali **darabolást** ír elő — a kettő együtt kell.
  - **A JAVASLAT (a döntés a tulajdonosé):** kézi gomb a Beállításokban (a „Gyorsítótár" kártya mintájára) **+** automatikus háttéregyeztetés a „Megvásárolt zenéim" megnyitásakor; a „csak jelzés" változat önmagában kevés.
- **A FUNKCIÓK TELEPÍTVE** (`npx firebase deploy --only functions` → *Deploy complete!*), és az éles mérés az új kapuval **1/1 elfogadás**.
- **⚠️ ŐSZINTE KORLÁT:** futásidejű, **eszközön mért** igazolás nincs (a fekvő tabletes nézet, a villogás megszűnése és a reklám utáni feloldás a tulajdonos készülékén mérhető) — a bizonyíték futó teszt + forrás-lint + éles adatmérés + csomag-ellenőrzés. **A WordPress plugin ebben a körben nem változott (2.6.0 marad).**
