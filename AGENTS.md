...
- Current 1.0 live status (2026-08-12): all four Wellerman Play Billing products are active in Hungary with the requested 700/550 HUF base prices. WordPress release 12123 still needs the four product IDs entered; until then the public API returns `products: []`. `getLabelDownloadUrl` now reads the product-ID entitlement document written by `verifyLabelPurchase`. Do not close v1.0 until WordPress metadata, the Firebase Play secret, production AdMob IDs, and final phone release testing are complete.
