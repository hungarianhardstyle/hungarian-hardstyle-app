...
- **5–6. RÁDIÓ: nem szakad meg (wake lock) + elhallgat, ha más app szól (hangfókusz).**
  ...
  - **A javítás (3. rész):** `pauseForFocusLoss()` **nem állítja le** a szolgáltatást (nincs `stopSelf`, nincs `stopForeground`, a `KEY_PLAYING` marad) — csak elhallgat, és beindítja a figyelést. A folytatás **egy közös döntés** (`resumeAfterFocusLoss()`), amelyet **két út** hív:
