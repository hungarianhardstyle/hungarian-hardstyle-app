...
- **📌 „SOKAKNAK BÉTÁT ÍR A PLAY" — NEM a mi kiadásunk neve (a tulajdonos jelzése, 2026-09-23):** a **`beta` sáv ÜRES** (mérve: `check-play-track.mjs` → a `beta` sávnál **egyetlen kiadás sincs**), ezért a felirat **kizárólag a tesztelői jogosultságból** ered: a Play a **saját tesztelői felületén** jelöli meg azt, aki **benne van a zárt teszt listáján** — ez **Play-oldali címke**, nem a mi kiadásunk elnevezése. **Nem hiba és nem baj:** ugyanazt a **352**-t kapják, mint a nyilvános felhasználók (`alpha` = `production` = 352), a nyilvános látogatónál **nincs** ilyen felirat. **A tesztelői lista az API-ból nem mérhető megbízhatóan** (lásd a `testers` végpont tanulságát) — **a Console az irányadó**. **A felirat csak úgy tűnik el, ha az illető kikerül a tesztelői listáról, ÉS a Playben is kilép a tesztből** (addig a Play teszt-verzióként kezeli). **✅ A TULAJDONOS DÖNTÉSE (2026-09-23): „semmit most, de később térjünk vissza rá"** — a **zárt teszt marad** (ez a gyors előteszt-csatornánk a következő kiadás előtt), a `beta`/`alpha` sávhoz és a tesztelői listához **most nem nyúlunk**.
