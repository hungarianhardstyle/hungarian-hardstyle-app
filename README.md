...
> **A legfrissebb állapot az `AGENTS.md` legfelső szakaszaiban van.** Az alábbi
...
## Aktuális állapot — 2026-09-19 (AAB **333** + plugin **2.5.7**)

- **Kiadásra kész:** `build/HUHS-v1.0.0+333-release.aab` (versionCode **333**) és `build/huhs-mobile-api-2.5.7.zip` (SHA-256 `352223459F8DC5318321303B8BF835623AE8856465F15412E03D5002F744F2E9`). A Play-be másolható kiadási szöveg: `docs/PLAY-KIADASI-JEGYZET.md`. **A 333-at kell feltölteni** — a zárt teszt (alpha) sávján most a **332** él, ezt váltja; a mért állapotot a `node tools/check-play-track.mjs` mutatja.
- **A 333 újdonsága:** az **Achievement-útmutató** (Több → Achievementek) pontos leírásokat kapott (napi keretek, a lájkpont véglegessége, lemondásnál elvesző pontok), a **szintek a szerverről** jönnek, és **három új pontforrás** él: kiadvány-vásárlás +20 (változatonként, Play-ellenőrzéssel), jóváhagyott beküldés +10 (napi 3, a beküldőnek), valamint a **napi aktivitási pont 1–5** kommentért és chatért — a szerver a lezárt napot értékeli.
- **Az Achievement-útmutató szövegét kapu őrzi:** `node tools/verify-achievement-guide.mjs` a `functions/index.js`-ből kiolvasott pontértékekhez és napi keretekhez köti a szöveget (20/20 + önteszt 6/6), így nem avulhat el csendben.
- **A 332 újdonsága (a zárt teszt sávján élt):** a **görgetés közbeni villogás javítva** (a hírek, DJ-k és események képei áttűnés nélkül jelennek meg), és **a napi lájkpont-keret jelzése**: ha a napi 3 lájkpont elfogyott, az app a lájk előtt kiírja, hogy „A mai lájkpontod elfogyott.".
- **Az achievement-pontok rendben (szerveroldali, AAB nélkül is él):** a lájk visszavonása **nem vesz el pontot**, az értesítés **mindig megmondja, miért** járt a pont, és a régebben tévesen elvett **40 pont visszaállt 6 felhasználónál** (Szabó Attila 0 → 12). A saját hibámból eredő dupla `profile-complete` jóváírás korrigálva (Denoiser −30). Az invariáns (profil = ledger-összeg) 21 profilnál, 487 sorral **0 eltéréssel** tart (`node tools/verify-achievement-points.mjs`).
- **A 331 újdonsága:** a natív HUHS adminból mostantól **ÚJ kérdőív, nyereményjáték és kvíz is létrehozható** (kérdőív: kérdés + 2–6 válasz + időszak; nyereményjáték: kérdés + 3–5 válasz + helyes válasz + nyeremény + látszási napok; kvíz: típus + időszak + jutalom + kérdések 2–6 válasszal és megjelölt helyes válasszal). A `+` gomb a „Kvíz és játékok" szakaszon, illetve a Kérdőív-eredmények és a Nyereményjáték-admin képernyőn van.
...
- **A plugin 2.5.7 élesben van és igazolva** (`apiVersion=2.5.7`; a `tools/verify-admin-create-live.mjs` **11/11**-gyel mérte: mindhárom típus űrlapja lekérhető `id=0`-ra, egy **piszkozat** kvíz létrejött és a meta-körjárás megmaradt, a frissítő út pedig nem publikál véletlenül). A `prize_games`/`prize_results` valódi adatot ad UID/hash nélkül. **⚠️ A plugin forrása gitignore-olt munkafában él (`.tmp-api-*`), ezért a repóban csak a ZIP látszik.**
...
- **Új kapuk:** `tools/verify-native-admin-menu.mjs` (a natív admin menüpontjai ↔ a plugin végpontjai), `tools/check-play-notes.mjs` (a kiadási szöveg), `tools/check-play-track.mjs` (mi van tényleg a Play sávjain — a Play Developer API-t olvasásra kérdezi), `tools/verify-achievement-points.mjs` (az achievement-invariáns ÉLES mérése), `tools/restore-lost-achievement-points.mjs` (az elveszett pontok visszaállítása előnézettel és `--confirm`-mal) és `tools/verify-achievement-guide.mjs` (az Achievement-útmutató szövege egyezik-e a kóddal).
- **Élő ellenőrző eszközök** (egy paranccsal, titok nélkül a repóban): `tools/verify-live-account-deletion.mjs` (nincs-e elakadt törlés / szellem-profil), `tools/verify-wp-admin-endpoints.mjs` (a plugin végpontjai és UID/hash-szivárgás), `tools/check-play-listing.mjs` (a nyilvános Play-oldal állapota — zárt tesztnél a 404 a várt), `tools/check-play-track.mjs` (mi van tényleg a Play sávjain), `tools/run-account-cleanup.mjs` (a takarítás azonnali futtatása, `--confirm`-mal), `tools/verify-achievement-points.mjs` és `tools/restore-lost-achievement-points.mjs` (achievement-audit és -visszaállítás), valamint `tools/check-function-errors.mjs` (volt-e hiba a Cloud Function-ökben az elmúlt időszakban). Közös modul: `tools/lib/live-firebase.mjs`.
...
## Ingyenes kiadvány külső linkje
