...
Target: implement in v0.99.99 before v1.0. The current implementation is ready for phone verification: one WordPress season editor provides category-level candidate fields, WordPress manages unlimited candidates per category, DJ and organizer candidates do not use Spotify/YouTube links, the Hungarian hardstyle track category supports optional Spotify/YouTube, Flutter enforces category selection limits of 5/3/2/1/3, displays the active Home entry and voting categories with a 5-second API timeout, registered-user votes are protected by Firestore rules, the newsletter question is separate and explicit, and the native admin has a private summary. ARM64 debug APK: `build/HUHS-v0.99.99+5-arm64-debug.apk`; updated existing API package: `build/huhs-mobile-api-2.4.42.zip`. Final closure waits for owner phone verification.
