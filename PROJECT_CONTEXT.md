...
The v1.0 Label delivery path is implemented and WordPress API `2.4.45` is deployed and live-verified; the public release API returns separate Radio/Extended versions and `audio_status: ready`. Release closure is still blocked until `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` is configured in Firebase Secret Manager, Play Billing products are created, production AdMob IDs are supplied, and the new release APK is phone-tested. The Label editor has separate Radio and Extended uploads; Radio creates the preview, and both versions create private WAV/320 kbps derivatives. Apple sign-in and iOS preparation remain explicitly out of scope.
