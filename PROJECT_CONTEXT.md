...
The v1.0 Label delivery path is implemented locally, but release closure is blocked until the 2.4.43 WordPress package is deployed, `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` is configured in Firebase Secret Manager, Play Billing products are created, production AdMob IDs are supplied, and the new release APK is phone-tested. Apple sign-in and iOS preparation remain explicitly out of scope.
