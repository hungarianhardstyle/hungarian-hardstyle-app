...
The v1.0 Label delivery path is implemented and WordPress API `2.4.45` is deployed and live-verified; locally prepared follow-up package `2.4.46` adds native Release admin fields for the four Play product IDs/prices; the public release API returns separate Radio/Extended versions and `audio_status: ready`. The Play Console app is created with Android package `hu.hungarianhardstyle.app`, and Firebase has the matching Android app and release SHA certificates. As of 2026-08-12 all four Wellerman Play Billing products are active in Hungary with the requested 700/550 HUF base prices, but WordPress release 12123 still has empty product metadata and the live API returns `products: []` until the four IDs are entered. The paid-download entitlement lookup was fixed to use the product-ID document written by purchase verification. Release closure remains blocked by the WordPress metadata entry, missing `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` Firebase secret, missing production AdMob IDs, and final phone testing. Apple sign-in and iOS preparation remain explicitly out of scope.
